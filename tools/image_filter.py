...
def main():
    ...
    image_list = dict(
        [
            (filename, cv2.imread(os.path.join(IMAGE_COLLECTION_PATH, filename)))
            for filename in tqdm(
                os.listdir(IMAGE_COLLECTION_PATH), desc="loading images"
            )
        ]
    )
    kept_images = []
    ...
    if input(
        f"from the {len(image_list)} images, {len(kept_images)} were filtered. Continue? (Y/n)"
    ) not in ["n", "N"]:
