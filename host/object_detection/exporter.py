...
def parse_side_inputs(
    side_input_shapes_string, side_input_names_string, side_input_types_string
):
    ...
    if side_input_shapes_string:
        ...
        raise ValueError(
            "When using side_inputs, side_input_shapes must be "
    ...
    if side_input_names_string:
        ...
        raise ValueError(
            "When using side_inputs, side_input_names must be "
            "specified in the input flags."
        )
    if side_input_types_string:
        ...
        raise ValueError(
            "When using side_inputs, side_input_types must be "
            "specified in the input flags."
        )
...
def rewrite_nn_resize_op(is_quantized=False):
    ...
    def remove_nn():
        ...
        stack_2_pattern = graph_matcher.OpTypePattern(
            "Pack", inputs=[reshape_1_pattern, reshape_1_pattern], ordered_inputs=False
        )
        ...
        consumer_pattern1 = graph_matcher.OpTypePattern(
            "Add|AddV2|Max|Mul", inputs=[reshape_2_pattern, "*"], ordered_inputs=False
        )
...
def replace_variable_values_with_moving_averages(
    graph, current_checkpoint_file, new_checkpoint_file, no_ema_collection=None
):
...
def _side_input_tensor_placeholder(side_input_shape, side_input_name, side_input_type):
...
def add_output_tensor_nodes(
    postprocessed_tensors, output_collection_name="inference_op"
):
...
def write_graph_and_checkpoint(
    inference_graph_def, model_path, input_saver_def, trained_checkpoint_prefix
):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            saver = tf.train.Saver(saver_def=input_saver_def, save_relative_paths=True)
            saver.restore(sess, trained_checkpoint_prefix)
            saver.save(sess, model_path)


def _get_outputs_from_inputs(
    input_tensors, detection_model, output_collection_name, **side_inputs
):
...
def build_detection_graph(
    ...
    if input_shape is not None:
        if (
            ...
            and input_type != "encoded_image_string_tensor"
            and input_type != "tf_example"
            and input_type != "tf_sequence_example"
        ):
            raise ValueError(
                "Can only specify input shape for `image_tensor`, "
