...
EVAL_METRICS_CLASS_DICT = {
    ...
    "coco_keypoint_metrics": coco_evaluation.CocoKeypointEvaluator,
    "coco_mask_metrics": coco_evaluation.CocoMaskEvaluator,
    "coco_panoptic_metrics": coco_evaluation.CocoPanopticSegmentationEvaluator,
    "lvis_mask_metrics": lvis_evaluation.LVISMaskEvaluator,
    ...
    with slim.queues.QueueRunners(sess):
        try:
            for batch in range(int(num_batches)):
                if (batch + 1) % 100 == 0:
                    tf.logging.info(
                        "Running eval ops batch %d/%d", batch + 1, num_batches
                    )
            ...
            if eval_export_path and eval_export_path is not None:
                for evaluator in evaluators:
                    ...
                    ) or isinstance(evaluator, coco_evaluation.CocoMaskEvaluator):
            ...
            if process_metrics_fn and checkpoint_file:
                ...
                if not m:
                    tf.logging.error(
                        "Failed to parse checkpoint number from: %s", checkpoint_file
                    )
                else:
                    checkpoint_number = int(m.group(1))
                    process_metrics_fn(
                        checkpoint_number, all_evaluator_metrics, checkpoint_file
                    )
    sess.close()
...
def repeated_checkpoint_run(
    ...
    while True:
        ...
        if not model_path:
            ...
            tf.logging.info(
                "Found already evaluated checkpoint. Will try again in " "%d seconds",
                eval_interval_secs,
            )
        else:
...
def result_dict_for_single_example(
    image,
    key,
    detections,
    groundtruth=None,
    ...
    if groundtruth:
        max_gt_boxes = tf.shape(groundtruth[fields.InputDataFields.groundtruth_boxes])[
            0
        ]
    ...
    output_dict = {
        fields.InputDataFields.original_image: batched_output_dict[
            fields.InputDataFields.original_image
        ]
    }
    ...
    for eval_metric_fn_key in eval_metric_fn_keys:
        if eval_metric_fn_key in (
            ...
            evaluator_options[eval_metric_fn_key] = {
                ...
                "skip_predictions_for_unlabeled_class": eval_config.skip_predictions_for_unlabeled_class,
            }
...
def has_densepose(eval_dict):
    return (
        fields.DetectionResultFields.detection_masks in eval_dict
        and fields.DetectionResultFields.detection_surface_coords in eval_dict
    )
