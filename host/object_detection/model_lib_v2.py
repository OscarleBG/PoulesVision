...
def _ensure_model_is_built(model, input_dataset, unpad_groundtruth_tensors):
    ...
    def _dummy_computation_fn(features, labels):
        ...
        return _compute_losses_and_predictions_dicts(
            model, features, labels, training_step=0
        )
...
def normalize_dict(values_dict, num_replicas):
    ...
    return {
        key: tf.math.divide(loss, num_replicas) for key, loss in values_dict.items()
    }
...
def validate_tf_v2_checkpoint_restore_map(checkpoint_restore_map):
    ...
    for key, value in checkpoint_restore_map.items():
        if not (
            isinstance(key, str)
            and (isinstance(value, tf.Module) or isinstance(value, tf.train.Checkpoint))
        ):
            if isinstance(key, str) and isinstance(value, dict):
                validate_tf_v2_checkpoint_restore_map(value)
            else:
                raise TypeError(
                    RESTORE_MAP_ERROR_TEMPLATE.format(
...
def load_fine_tune_checkpoint(
    ...
    ckpt.restore(checkpoint_path).expect_partial().assert_existing_objects_matched()
    ...
    **kwargs
    ...
    with strategy.scope():
        detection_model = MODEL_BUILD_UTIL_MAP["detection_model_fn_base"](
            model_config=model_config, is_training=True, add_summaries=record_summaries
        )
        ...
        if train_config.optimizer.use_moving_average:
            _ensure_model_is_built(
                detection_model, train_input, unpad_groundtruth_tensors
            )
            ...
            with tf.compat.v2.summary.record_if(
                ...
                def _sample_and_train(strategy, train_step_fn, data_iterator):
                    ...
                    if hasattr(tf.distribute.Strategy, "run"):
                        ...
                        per_replica_losses_dict = strategy.experimental_run_v2(
                            train_step_fn, args=(features, labels)
                        )
                ...
                for _ in range(
                    ...
                    if global_step.value() - logged_step >= LOG_EVERY:
                        logged_dict_np = {
                            name: value.numpy() for name, value in logged_dict.items()
                        }
                        tf.logging.info(
    ...
    for i, (features, labels) in enumerate(eval_dataset):
        try:
            (
                ...
                groundtruth_dict,
                eval_features,
            ) = strategy.run(compute_eval_dict, args=(features, labels))
