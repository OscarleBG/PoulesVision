...
def get_inputs(
    ...
    def extract_images_and_targets(read_data):
        ...
        if merge_multiple_label_boxes:
            ...
            classes_gt = tf.cast(
                read_data[fields.InputDataFields.multiclass_scores], tf.float32
            )
        else:
...
def train(
    ...
    with tf.Graph().as_default():
        ...
        model_fn = functools.partial(
            _create_losses, create_model_fn=create_model_fn, train_config=train_config
        )
        ...
        with tf.device(deploy_config.optimizer_device()):
            ...
            total_loss, grads_and_vars = model_deploy.optimize_clones(
                clones, training_optimizer, regularization_losses=regularization_losses
            )
        ...
        for loss_tensor in tf.losses.get_losses():
            global_summaries.add(
                tf.summary.scalar("Losses/" + loss_tensor.op.name, loss_tensor)
            )
        global_summaries.add(
