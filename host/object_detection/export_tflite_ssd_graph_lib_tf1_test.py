...
class ExportTfliteGraphTest(tf.test.TestCase):
    def _save_checkpoint_from_mock_model(
        self, checkpoint_path, use_moving_averages, quantize=False, num_channels=3
    ):
        g = tf.Graph()
        with g.as_default():
    ...
    def test_export_tflite_graph_with_moving_averages(self):
        ...
        (box_encodings_np, class_predictions_np) = self._import_graph_and_run_inference(
            tflite_graph_file
        )
        self.assertAllClose(
            box_encodings_np, [[[0.0, 0.0, 0.5, 0.5], [0.5, 0.5, 0.8, 0.8]]]
        )
    ...
    def test_export_tflite_graph_with_softmax_score_conversion(self):
        ...
        self.assertAllClose(
            class_predictions_np, [[[0.524979, 0.475021], [0.710949, 0.28905]]]
        )
    ...
    def test_export_tflite_graph_with_sigmoid_score_conversion(self):
        ...
        self.assertAllClose(
            class_predictions_np, [[[0.668188, 0.645656], [0.710949, 0.5]]]
        )
