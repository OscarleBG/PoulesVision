...
class DetectionInferenceModule(tf.Module):
    ...
    def __init__(self, detection_model, use_side_inputs=False, zipped_side_inputs=None):
...
class DetectionFromImageModule(DetectionInferenceModule):
    ...
    def __init__(self, detection_model, use_side_inputs=False, zipped_side_inputs=None):
        ...
        super(DetectionFromImageModule, self).__init__(
            detection_model, use_side_inputs, zipped_side_inputs
        )
...
class DetectionFromTFExampleModule(DetectionInferenceModule):
    ...
    @tf.function(input_signature=[tf.TensorSpec(shape=[None], dtype=tf.string)])
    def __call__(self, input_tensor):
        images, true_shapes = self._preprocess_input(input_tensor, _decode_tf_example)
        return self._run_inference_on_images(images, true_shapes)
...
def export_inference_graph(
    ...
    zipped_side_inputs = []
    if use_side_inputs:
        zipped_side_inputs = _combine_side_inputs(
            side_input_shapes, side_input_types, side_input_names
        )
...
class DetectionFromImageAndBoxModule(DetectionInferenceModule):
    ...
    def __init__(self, detection_model, use_side_inputs=False, zipped_side_inputs=None):
        ...
        super(DetectionFromImageAndBoxModule, self).__init__(
            detection_model, use_side_inputs, zipped_side_inputs
        )
    ...
    def _run_segmentation_on_images(self, image, boxes, **kwargs):
        ...
        detections = self._model.predict_masks_from_boxes(
            prediction_dict, shapes, boxes
        )
        classes_field = fields.DetectionResultFields.detection_classes
        detections[classes_field] = (
            tf.cast(detections[classes_field], tf.float32) + label_id_offset
