...
def _multiclass_scores_or_one_hot_labels(
    multiclass_scores, groundtruth_boxes, groundtruth_classes, num_classes
):
    ...
    def true_fn():
        return tf.reshape(
            multiclass_scores, [tf.shape(groundtruth_boxes)[0], num_classes]
        )
...
def transform_input_data(
    ...
    if input_fields.multiclass_scores in out_tensor_dict:
        ...
        ] = _multiclass_scores_or_one_hot_labels(
    ...
    if input_fields.image_additional_channels in out_tensor_dict:
        ...
        if retain_original_image_additional_channels:
            out_tensor_dict[input_fields.image_additional_channels] = tf.cast(
    ...
    if input_fields.groundtruth_keypoints in tensor_dict:
        ...
        out_tensor_dict[input_fields.groundtruth_keypoints] = realigned_keypoints
    ...
    if input_fields.groundtruth_instance_masks in out_tensor_dict:
        ...
        out_tensor_dict[input_fields.groundtruth_instance_masks] = resized_masks

    zero_indexed_groundtruth_classes = (
        out_tensor_dict[input_fields.groundtruth_classes] - _LABEL_OFFSET
    )
    if use_multiclass_scores:
        out_tensor_dict[input_fields.groundtruth_classes] = out_tensor_dict[
            input_fields.multiclass_scores
        ]
    else:
    ...
    if input_fields.groundtruth_confidences in out_tensor_dict:
        groundtruth_confidences = out_tensor_dict[input_fields.groundtruth_confidences]
    ...
    if merge_multiple_boxes:
        ...
        out_tensor_dict[input_fields.groundtruth_confidences] = merged_confidences
    if input_fields.groundtruth_boxes in out_tensor_dict:
...
def pad_input_data_to_static_shapes(
    ...
    if input_fields.image_additional_channels in tensor_dict:
        num_additional_channels = shape_utils.get_dim_as_int(
            tensor_dict[input_fields.image_additional_channels].shape[2]
        )
    ...
    if num_additional_channels:
        ...
        if (
            input_fields.original_image in tensor_dict
            and shape_utils.get_dim_as_int(
    ...
    if input_fields.context_features in tensor_dict and (
        ...
        raise ValueError(
            "max_num_context_features must be specified in the model "
    ...
    if input_fields.groundtruth_keypoints in tensor_dict:
        tensor_shape = tensor_dict[input_fields.groundtruth_keypoints].shape
        padding_shape = [
            max_num_boxes,
    ...
    if input_fields.groundtruth_keypoint_visibilities in tensor_dict:
        tensor_shape = tensor_dict[input_fields.groundtruth_keypoint_visibilities].shape
        padding_shape = [max_num_boxes, shape_utils.get_dim_as_int(tensor_shape[1])]
        padding_shapes[input_fields.groundtruth_keypoint_visibilities] = padding_shape
    ...
    if input_fields.groundtruth_keypoint_weights in tensor_dict:
        tensor_shape = tensor_dict[input_fields.groundtruth_keypoint_weights].shape
        padding_shape = [max_num_boxes, shape_utils.get_dim_as_int(tensor_shape[1])]
        padding_shapes[input_fields.groundtruth_keypoint_weights] = padding_shape
    if input_fields.groundtruth_dp_num_points in tensor_dict:
        padding_shapes[input_fields.groundtruth_dp_num_points] = [max_num_boxes]
        padding_shapes[input_fields.groundtruth_dp_part_ids] = [
            max_num_boxes,
            max_dp_points,
        ]
        padding_shapes[input_fields.groundtruth_dp_surface_coords] = [
            max_num_boxes,
            max_dp_points,
            4,
        ]
    if input_fields.groundtruth_track_ids in tensor_dict:
        padding_shapes[input_fields.groundtruth_track_ids] = [max_num_boxes]
    ...
    if input_fields.context_features in tensor_dict:
        ...
        tensor_shape = tf.shape(tensor_dict[fields.InputDataFields.context_features])
...
def augment_input_data(tensor_dict, data_augmentation_options):
    ...
    include_label_confidences = (
        fields.InputDataFields.groundtruth_confidences in tensor_dict
    )
...
def create_train_input_fn(train_config, train_input_config, model_config):
    ...
    def _train_input_fn(params=None):
        return train_input(
            train_config, train_input_config, model_config, params=params
        )
...
def train_input(
    ...
    if not isinstance(train_config, train_pb2.TrainConfig):
        raise TypeError(
            "For training mode, the `train_config` must be a " "train_pb2.TrainConfig."
        )
    if not isinstance(train_input_config, input_reader_pb2.InputReader):
        raise TypeError(
            "The `train_input_config` must be a " "input_reader_pb2.InputReader."
        )
