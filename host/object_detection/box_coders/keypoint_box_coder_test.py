...
class KeypointBoxCoderTest(test_case.TestCase):
    def test_get_correct_relative_codes_after_encoding(self):
        boxes = np.array([[10.0, 10.0, 20.0, 15.0], [0.2, 0.1, 0.5, 0.4]], np.float32)
        keypoints = np.array(
            [[[15.0, 12.0], [10.0, 15.0]], [[0.5, 0.3], [0.2, 0.4]]], np.float32
        )
        num_keypoints = len(keypoints[0])
        anchors = np.array([[15.0, 12.0, 30.0, 18.0], [0.1, 0.0, 0.7, 0.9]], np.float32)
        expected_rel_codes = [
            [-0.5, -0.416666, -0.405465, -0.182321, -0.5, -0.5, -0.833333, 0.0],
            [
                -0.083333,
                -0.222222,
                -0.693147,
        ...
        self.assertAllClose(rel_codes_out, expected_rel_codes, rtol=1e-04, atol=1e-04)
    ...
    def test_get_correct_relative_codes_after_encoding_with_scaling(self):
        ...
        anchors = np.array([[15.0, 12.0, 30.0, 18.0], [0.1, 0.0, 0.7, 0.9]], np.float32)
        expected_rel_codes = [
            [-1.0, -1.25, -1.62186, -0.911608, -1.0, -1.5, -1.666667, 0.0],
            [
                -0.166667,
                -0.666667,
                -2.772588,
    ...
    def test_get_correct_boxes_after_decoding(self):
        ...
        self.assertAllClose(keypoints_out, expected_keypoints, rtol=1e-04, atol=1e-04)
        self.assertAllClose(boxes_out, expected_boxes, rtol=1e-04, atol=1e-04)
    ...
    def test_very_small_width_nan_after_encoding(self):
        ...
        keypoints = np.array([[[10.0, 10.0], [10.0000001, 20.0]]], np.float32)
