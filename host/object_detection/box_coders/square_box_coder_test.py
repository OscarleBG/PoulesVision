...
class SquareBoxCoderTest(test_case.TestCase):
    ...
    def test_correct_relative_codes_with_non_default_scale(self):
        ...
        rel_codes_out = self.execute(graph_fn, [boxes, anchors])
        self.assertAllClose(rel_codes_out, expected_rel_codes, rtol=1e-03, atol=1e-03)
    ...
    def test_correct_boxes_with_default_scale(self):
        anchors = np.array([[15.0, 12.0, 30.0, 18.0], [0.1, 0.0, 0.7, 0.9]], np.float32)
        rel_codes = np.array(
            [[-0.5, -0.416666, -0.405465], [-0.083333, -0.222222, -0.693147]],
            np.float32,
        )
        expected_boxes = [
    ...
    def test_correct_boxes_with_non_default_scale(self):
        anchors = np.array([[15.0, 12.0, 30.0, 18.0], [0.1, 0.0, 0.7, 0.9]], np.float32)
        rel_codes = np.array(
            [[-1.0, -1.25, -1.62186], [-0.166667, -0.666667, -2.772588]], np.float32
        )
        expected_boxes = [
            [14.594306, 7.884875, 20.918861, 14.209432],
            [0.155051, 0.102989, 0.522474, 0.470412],
