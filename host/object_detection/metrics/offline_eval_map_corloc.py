...
flags.DEFINE_string(
    "eval_config_path", None, "Path to an eval_pb2.EvalConfig config file."
)
flags.DEFINE_string(
    "input_config_path", None, "Path to an eval_pb2.InputConfig config file."
)
...
def read_data_and_evaluate(input_config, eval_config):
    ...
    if input_config.WhichOneof("input_reader") == "tf_record_input_reader":
        ...
        object_detection_evaluators = eval_util.get_evaluators(eval_config, categories)
        ...
        for input_path in _generate_filenames(input_paths):
            ...
            for string_record in record_iterator:
                tf.logging.log_every_n(
                    tf.logging.INFO, "Processed %d images...", 1000, processed_images
                )
                processed_images += 1
