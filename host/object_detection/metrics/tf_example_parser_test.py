...
class TfExampleDecoderTest(tf.test.TestCase):
    ...
    def testParseDetectionsAndGT(self):
        ...
        object_bb = np.array(
            [[0.0, 0.5, 0.3], [0.0, 0.1, 0.6], [1.0, 0.6, 0.8], [1.0, 0.6, 0.7]]
        ).transpose()
        detection_bb = np.array(
            [[0.1, 0.2], [0.0, 0.8], [1.0, 0.6], [1.0, 0.85]]
        ).transpose()
        ...
        features = {
            fields.TfExampleFields.source_id: self._BytesFeature(source_id),
            fields.TfExampleFields.object_bbox_ymin: self._FloatFeature(
                object_bb[:, 0].tolist()
            ),
            fields.TfExampleFields.object_bbox_xmin: self._FloatFeature(
                object_bb[:, 1].tolist()
            ),
            fields.TfExampleFields.object_bbox_ymax: self._FloatFeature(
                object_bb[:, 2].tolist()
            ),
            fields.TfExampleFields.object_bbox_xmax: self._FloatFeature(
                object_bb[:, 3].tolist()
            ),
            fields.TfExampleFields.detection_bbox_ymin: self._FloatFeature(
                detection_bb[:, 0].tolist()
            ),
            fields.TfExampleFields.detection_bbox_xmin: self._FloatFeature(
                detection_bb[:, 1].tolist()
            ),
            fields.TfExampleFields.detection_bbox_ymax: self._FloatFeature(
                detection_bb[:, 2].tolist()
            ),
            fields.TfExampleFields.detection_bbox_xmax: self._FloatFeature(
                detection_bb[:, 3].tolist()
            ),
            fields.TfExampleFields.detection_class_label: self._Int64Feature(
                detection_class_label
            ),
            fields.TfExampleFields.detection_score: self._FloatFeature(detection_score),
        }
        ...
        features[fields.TfExampleFields.object_class_label] = self._Int64Feature(
            object_class_label
        )
        features[fields.TfExampleFields.object_difficult] = self._Int64Feature(
            object_difficult
        )
        ...
        np_testing.assert_almost_equal(
            detection_bb, results_dict[fields.DetectionResultFields.detection_boxes]
        )
        np_testing.assert_almost_equal(
            detection_score, results_dict[fields.DetectionResultFields.detection_scores]
        )
        np_testing.assert_almost_equal(
            detection_class_label,
        ...
        np_testing.assert_almost_equal(
            object_difficult, results_dict[fields.InputDataFields.groundtruth_difficult]
        )
        np_testing.assert_almost_equal(
            object_class_label, results_dict[fields.InputDataFields.groundtruth_classes]
        )

        parser = tf_example_parser.TfExampleDetectionAndGTParser()

        features[fields.TfExampleFields.object_group_of] = self._Int64Feature(
            object_group_of
        )
        ...
        np_testing.assert_equal(
            object_group_of, results_dict[fields.InputDataFields.groundtruth_group_of]
        )

        features[fields.TfExampleFields.image_class_label] = self._Int64Feature(
            verified_labels
        )
    ...
    def testBoundingBoxParser(self):
        ...
        parser = tf_example_parser.BoundingBoxParser(
            "xmin", "ymin", "xmax", "another_ymax"
        )
        result = parser.parse(example)
        self.assertIsNone(result)
