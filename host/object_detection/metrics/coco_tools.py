    ...
    def ComputeMetrics(
        ...
        if self._iou_type in ["bbox", "segm"]:
            summary_metrics = OrderedDict(
                [
                    (name, self.stats[index])
                    for name, index in COCO_METRIC_NAMES_AND_INDEX
                ]
            )
            ...
            for metric_name, index in COCO_KEYPOINT_METRIC_NAMES_AND_INDEX:
                ...
                summary_metrics[
                    "{} ByCategory/{}".format(metric_name, category_name)
                ] = value
        ...
        for category_index, category_id in enumerate(self.GetCategoryIdList()):
            ...
            per_category_ap[
                "PerformanceByCategory/mAP/{}".format(category)
            ...
            if super_categories:
                for key in super_categories:
                    if category in super_categories[key]:
                        ...
                        if all_metrics_per_category:
                            for metric_name, index in COCO_METRIC_NAMES_AND_INDEX:
                                metric_key = "{} BySuperCategory/{}".format(
                                    metric_name, key
                                )
                                super_category_ap[metric_key] += self.category_stats[
                                    index
                                ][category_index]
        ...
        if super_categories:
            for key in super_categories:
                ...
                super_category_ap["PerformanceBySuperCategory/{}".format(key)] /= length

                if all_metrics_per_category:
                    for metric_name, _ in COCO_METRIC_NAMES_AND_INDEX:
                        super_category_ap[
                            "{} BySuperCategory/{}".format(metric_name, key)
                        ] /= length
...
def ExportGroundtruthToCOCO(
    image_ids, groundtruth_boxes, groundtruth_classes, categories, output_path=None
):
    ...
    annotation_id = 1
    for image_id, boxes, classes in zip(
        image_ids, groundtruth_boxes, groundtruth_classes
    ):
        image_export_list.append({"id": image_id})
        groundtruth_export_list.extend(
            ExportSingleImageGroundtruthToCoco(
                image_id, annotation_id, category_id_set, boxes, classes
            )
        )
        ...
        if detection_classes[i] in category_id_set:
            ...
            if detection_keypoints is not None:
                ...
                if detection_keypoint_visibilities is None:
                    detection_keypoint_visibilities = np.full(
...
def ExportDetectionsToCOCO(
    ...
    for image_id, boxes, scores, classes in zip(
        image_ids, detection_boxes, detection_scores, detection_classes
    ):
        detections_export_list.extend(
            ExportSingleImageDetectionBoxesToCoco(
                image_id, category_id_set, boxes, scores, classes
            )
        )
    if output_path:
        with tf.gfile.GFile(output_path, "w") as fid:
...
def ExportSegmentsToCOCO(
    ...
    segment_export_list = []
    for image_id, masks, scores, classes in zip(
        image_ids, detection_masks, detection_scores, detection_classes
    ):
