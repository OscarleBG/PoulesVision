...
def _get_categories_list():
    return [
        {"id": 1, "name": "person", "frequency": "f"},
        {"id": 2, "name": "dog", "frequency": "c"},
        {"id": 3, "name": "cat", "frequency": "r"},
    ]
...
class LvisMaskEvaluationTest(tf.test.TestCase):
    def testGetOneMAPWithMatchingGroundtruthAndDetections(self):
        ...
        lvis_evaluator.add_single_ground_truth_image_info(
            image_id=1,
            groundtruth_dict={
                fields.InputDataFields.groundtruth_boxes: np.array(
                    [[100.0, 100.0, 200.0, 200.0]]
                ),
                fields.InputDataFields.groundtruth_classes: np.array([1]),
                fields.InputDataFields.groundtruth_instance_masks: masks1,
                fields.InputDataFields.groundtruth_verified_neg_classes: np.array(
                    [0, 0, 0, 0]
                ),
                fields.InputDataFields.groundtruth_not_exhaustive_classes: np.array(
                    [0, 0, 0, 0]
                ),
            },
        )
        lvis_evaluator.add_single_detected_image_info(
            ...
            detections_dict={
                ...
                fields.DetectionResultFields.detection_scores: np.array([0.8]),
...
class LVISMaskEvaluationPyFuncTest(tf.test.TestCase):
    def testAddEvalDict(self):
        ...
        groundtruth_boxes = tf.constant(
            np.array([[100.0, 100.0, 200.0, 200.0], [50.0, 50.0, 100.0, 100.0]]),
            dtype=tf.float32,
        )
        groundtruth_classes = tf.constant(np.array([1, 2]), dtype=tf.float32)
        groundtruth_masks = tf.constant(
            np.stack(
                [
                    np.pad(
                        np.ones([100, 100], dtype=np.uint8),
        ...
        original_image_spatial_shapes = tf.constant(
            [[120, 120], [120, 120]], dtype=tf.int32
        )
        groundtruth_verified_neg_classes = tf.constant(
            np.array([0, 0, 0, 0]), dtype=tf.float32
        )
        groundtruth_not_exhaustive_classes = tf.constant(
            np.array([0, 0, 0, 0]), dtype=tf.float32
        )
        detection_scores = tf.constant(np.array([0.9, 0.8]), dtype=tf.float32)
        detection_classes = tf.constant(np.array([2, 1]), dtype=tf.float32)
        detection_masks = tf.constant(
            np.stack(
                [
                    np.pad(
                        np.ones([50, 50], dtype=np.uint8),
