...
def build_groundtruth_dictionary(data, class_label_map):
    ...
    dictionary = {
        standard_fields.InputDataFields.groundtruth_boxes: data_location[
            ["YMin", "XMin", "YMax", "XMax"]
        ]
        .to_numpy()
        .astype(float),
        standard_fields.InputDataFields.groundtruth_classes: data_location["LabelName"]
        .map(lambda x: class_label_map[x])
        .to_numpy(),
        standard_fields.InputDataFields.groundtruth_group_of: data_location["IsGroupOf"]
        .to_numpy()
        .astype(int),
        standard_fields.InputDataFields.groundtruth_image_classes: data_labels[
            "LabelName"
        ]
        .map(lambda x: class_label_map[x])
        .to_numpy(),
...
def build_predictions_dictionary(data, class_label_map):
    ...
    """
    dictionary = {
        standard_fields.DetectionResultFields.detection_classes: data["LabelName"]
        .map(lambda x: class_label_map[x])
        .to_numpy(),
        standard_fields.DetectionResultFields.detection_scores: data["Score"]
        .to_numpy()
        .astype(float),
    }
    ...
    if "Mask" in data:
        ...
        dictionary[standard_fields.DetectionResultFields.detection_boxes] = (
            data[["YMin", "XMin", "YMax", "XMax"]].to_numpy().astype(float)
        )
