...
class CalibrationLibTest(tf.test.TestCase):
    ...
    def test_expected_calibration_error_all_bins_filled(self):
        ...
        with self.test_session() as sess:
            ...
            sess.run(
                update_op,
                feed_dict={
                    y_pred: np.array([0.0, 0.2, 0.4, 0.5, 1.0]),
    ...
    def test_expected_calibration_error_with_multiple_data_streams(self):
        ...
        with self.test_session() as sess:
            ...
            sess.run(
                update_op,
                feed_dict={y_pred: np.array([0.0, 0.2]), y_true: np.array([0, 0])},
            )
            sess.run(
                update_op,
                feed_dict={y_pred: np.array([0.4, 0.5]), y_true: np.array([1, 0])},
            )
            sess.run(
                update_op, feed_dict={y_pred: np.array([1.0]), y_true: np.array([1])}
            )
        actual_ece = 0.08 + 0.1
        expected_ece = sess.run(expected_ece_op)
        self.assertAlmostEqual(actual_ece, expected_ece)
