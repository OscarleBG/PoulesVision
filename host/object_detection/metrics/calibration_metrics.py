...
def expected_calibration_error(y_true, y_pred, nbins=20):
    ...
    with tf.control_dependencies([bin_ids]):
        update_bin_counts_op = tf.assign_add(
            bin_counts, tf.cast(tf.bincount(bin_ids, minlength=nbins), dtype=tf.float32)
        )
        update_bin_true_sum_op = tf.assign_add(
            bin_true_sum,
            tf.cast(
                tf.bincount(bin_ids, weights=y_true, minlength=nbins), dtype=tf.float32
            ),
        )
        update_bin_preds_sum_op = tf.assign_add(
            bin_preds_sum,
            tf.cast(
                tf.bincount(bin_ids, weights=y_pred, minlength=nbins), dtype=tf.float32
            ),
        )
