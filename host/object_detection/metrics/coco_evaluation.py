...
class CocoDetectionEvaluator(object_detection_evaluation.DetectionEvaluator):
    ...
    def add_eval_dict(self, eval_dict):
        ...
        def update_op(
            ...
            groundtruth_labeled_classes_batched,
        ...
        if groundtruth_labeled_classes is None:
            groundtruth_labeled_classes = tf.reduce_max(
                tf.one_hot(
                    tf.cast(detection_classes, tf.int32), len(self._category_id_set) + 1
                ),
                axis=-2,
            )
        ...
        if not image_id.shape.as_list():
            ...
            groundtruth_labeled_classes = tf.expand_dims(groundtruth_labeled_classes, 0)
...
class CocoKeypointEvaluator(CocoDetectionEvaluator):
    ...
    def __init__(self, category_id, category_keypoints, class_text, oks_sigmas=None):
        """Constructor.
        ...
        if oks_sigmas:
            ...
            tf.logging.warning(
                "No default keypoint OKS sigmas provided. Will use " "0.05"
            )
        tf.logging.info(
            "Using the following keypoint OKS sigmas: {}".format(self._oks_sigmas)
        )
        self._metrics = None
        super(CocoKeypointEvaluator, self).__init__(
            [{"id": self._category_id, "name": class_text}]
        )
    ...
    def add_single_ground_truth_image_info(self, image_id, groundtruth_dict):
        ...
        class_indices = [
            idx
            for idx, gt_class_id in enumerate(groundtruth_classes)
            if gt_class_id == self._category_id
        ]
        filtered_groundtruth_classes = np.take(
            groundtruth_classes, class_indices, axis=0
        )
        filtered_groundtruth_boxes = np.take(groundtruth_boxes, class_indices, axis=0)
        ...
        filtered_groundtruth_dict[
            standard_fields.InputDataFields.groundtruth_classes
        ] = filtered_groundtruth_classes
        filtered_groundtruth_dict[
            standard_fields.InputDataFields.groundtruth_boxes
        ] = filtered_groundtruth_boxes
        filtered_groundtruth_dict[
            standard_fields.InputDataFields.groundtruth_keypoints
        ] = filtered_groundtruth_keypoints
        ...
        if (
            ...
            filtered_groundtruth_is_crowd = np.take(
                groundtruth_is_crowd, class_indices, 0
            )
            filtered_groundtruth_dict[
                standard_fields.InputDataFields.groundtruth_is_crowd
            ] = filtered_groundtruth_is_crowd
        if standard_fields.InputDataFields.groundtruth_area in groundtruth_dict.keys():
        ...
        if (
            ...
            filtered_groundtruth_keypoint_visibilities = np.take(
                filtered_groundtruth_keypoint_visibilities, self._keypoint_ids, axis=1
            )
            filtered_groundtruth_dict[
                standard_fields.InputDataFields.groundtruth_keypoint_visibilities
            ] = filtered_groundtruth_keypoint_visibilities

        super(CocoKeypointEvaluator, self).add_single_ground_truth_image_info(
            image_id, filtered_groundtruth_dict
        )
    ...
    def add_single_detected_image_info(self, image_id, detections_dict):
        ...
        class_indices = [
            idx
            for idx, class_id in enumerate(detection_classes)
            if class_id == self._category_id
        ]
        filtered_detection_classes = np.take(detection_classes, class_indices, axis=0)
        ...
        filtered_detections_dict[
            standard_fields.DetectionResultFields.detection_classes
        ] = filtered_detection_classes
        filtered_detections_dict[
            standard_fields.DetectionResultFields.detection_boxes
        ] = filtered_detection_boxes
        filtered_detections_dict[
            standard_fields.DetectionResultFields.detection_scores
        ] = filtered_detection_scores
        filtered_detections_dict[
            standard_fields.DetectionResultFields.detection_keypoints
        ] = filtered_detection_keypoints

        super(CocoKeypointEvaluator, self).add_single_detected_image_info(
            image_id, filtered_detections_dict
        )
    ...
    def evaluate(self):
        ...
        keypoint_metrics = {
            "Keypoints_" + key: value for key, value in iter(keypoint_metrics.items())
        }
                ...
                if is_annotated:
                    self.add_single_ground_truth_image_info(
                        image_id,
                        {
                            ...
                            "groundtruth_keypoint_visibilities": gt_keyp_vis[
                                :num_gt_box
                            ],
                        },
                    )
...
class CocoMaskEvaluator(object_detection_evaluation.DetectionEvaluator):
    ...
    def add_single_ground_truth_image_info(self, image_id, groundtruth_dict):
        ...
        groundtruth_instance_masks = convert_masks_to_binary(groundtruth_instance_masks)
        self._groundtruth_list.extend(
            coco_tools.ExportSingleImageGroundtruthToCoco(
                image_id=image_id,
                next_annotation_id=self._annotation_id,
        ...
        if groundtruth_masks_shape[1:] != detection_masks.shape[1:]:
            raise ValueError(
                ...
                "are incompatible: {} vs {}".format(
                    groundtruth_masks_shape, detection_masks.shape
                )
            )
    ...
    def evaluate(self):
        ...
        mask_metrics = {
            "DetectionMasks_" + key: value for key, value in mask_metrics.items()
        }
        return mask_metrics
        ...
        def update_op(
            ...
            for (
                ...
                self.add_single_ground_truth_image_info(
                    image_id,
                    {
                        "groundtruth_boxes": groundtruth_boxes[:num_gt_box],
    ...
    def get_estimator_eval_metric_ops(self, eval_dict):
        ...
        if self._include_metrics_per_category:
            for category_dict in self._categories:
                metric_names.append(
                    "DetectionMasks_PerformanceByCategory/mAP/" + category_dict["name"]
                )
...
class CocoPanopticSegmentationEvaluator(object_detection_evaluation.DetectionEvaluator):
    ...
    def evaluate(self):
        ...
        mask_metrics = self._compute_panoptic_metrics(
            sum_tp_iou, sum_num_tp, sum_num_fp, sum_num_fn
        )
        return mask_metrics
        ...
        def update_op(
            ...
            for (
                ...
                self.add_single_ground_truth_image_info(
                    image_id,
                    {
                        "groundtruth_classes": groundtruth_classes[:num_gt_mask],
                        "groundtruth_instance_masks": groundtruth_instance_masks[
                            :num_gt_mask
                        ],
                        "groundtruth_is_crowd": groundtruth_is_crowd[:num_gt_mask],
                    },
                )
                self.add_single_detected_image_info(
                    image_id,
    ...
    def _evaluate_all_masks(self):
        ...
        for image_id in self._groundtruth_class_labels:
            ...
            (
                ...
                crowd_gt_masks,
                crowd_gt_classes,
            ) = self._separate_normal_and_crowd_labels(
                crowd_gt_indices,
                self._groundtruth_masks[image_id],
            ...
            (
                ...
                pred_matched,
                gt_matched,
            ) = self._match_predictions_to_groundtruths(
                predicted_masks,
            ...
            for (class_id, is_matched, overlap) in zip(
                predicted_class_labels, pred_matched, overlaps
            ):
                if is_matched:
    ...
    def _compute_panoptic_metrics(self, sum_tp_iou, sum_num_tp, sum_num_fp, sum_num_fn):
        """Compute PQ metric for each category and average over all classes.
        ...
        for category in self._categories:
            ...
            if panoptic_quality is not None:
                ...
                if self._include_metrics_per_category:
                    mask_metrics[
                        "PanopticQuality@%.2fIOU_ByCategory/%s"
                        % (self._iou_threshold, category["name"])
                    ] = panoptic_quality
        mask_metrics["PanopticQuality@%.2fIOU" % self._iou_threshold] = (
            sum_pq / num_valid_classes
        )
        mask_metrics["SegmentationQuality@%.2fIOU" % self._iou_threshold] = (
            sum_sq / num_valid_classes
        )
        mask_metrics["RecognitionQuality@%.2fIOU" % self._iou_threshold] = (
            sum_rq / num_valid_classes
        )
    ...
    def _compute_panoptic_metrics_single_class(
        self, sum_tp_iou, num_tp, num_fp, num_fn
    ):
    ...
    def _separate_normal_and_crowd_labels(
        self, crowd_gt_indices, groundtruth_masks, groundtruth_classes
    ):
        ...
        if groundtruth_masks.shape[0] != groundtruth_classes.shape[0]:
            raise ValueError("The number of masks doesn't match the number of labels.")
    ...
    def _match_predictions_to_groundtruths(
        self,
        predicted_masks,
        predicted_classes,
        groundtruth_masks,
            ...
            for gid in range(groundtruth_classes.shape[0]):
                if predicted_classes[pid] == groundtruth_classes[gid]:
                    if (not with_replacement) and gt_matched[gid]:
                        continue
                    if not is_crowd:
                        overlap = np_mask_ops.iou(
                            predicted_masks[pid : pid + 1],
                            groundtruth_masks[gid : gid + 1],
                        )[0, 0]
                    else:
                        overlap = np_mask_ops.ioa(
                            groundtruth_masks[gid : gid + 1],
                            predicted_masks[pid : pid + 1],
                        )[0, 0]
    ...
    def _unpack_evaluation_dictionary_items(self, eval_dict):
        ...
        detection_classes = eval_dict[detection_fields.detection_classes]
        detection_masks = eval_dict[detection_fields.detection_masks]
        num_det_masks_per_image = eval_dict.get(detection_fields.num_detections, None)
        if groundtruth_is_crowd is None:
            groundtruth_is_crowd = tf.zeros_like(groundtruth_classes, dtype=tf.bool)
