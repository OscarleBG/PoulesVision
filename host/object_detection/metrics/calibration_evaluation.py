...
class CalibrationDetectionEvaluator(object_detection_evaluation.DetectionEvaluator):
    ...
    def match_single_image_info(self, image_info):
        ...
        one_hot_depth = tf.cast(
            tf.add(tf.reduce_max(groundtruth_classes), 1), dtype=tf.int32
        )
        ...
        is_class_matched = tf.cast(
            tf.equal(tf.cast(cls_targets, tf.int64), detection_classes), dtype=tf.int64
        )
    ...
    def get_estimator_eval_metric_ops(self, eval_dict):
        ...
        image_info = [
            tf.boolean_mask(tensor, is_annotated_batched)
            for tensor in [
                groundtruth_boxes,
                groundtruth_classes,
