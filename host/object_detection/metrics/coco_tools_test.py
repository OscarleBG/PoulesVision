...
class CocoToolsTest(tf.test.TestCase):
    ...
    def testExportKeypointsToCOCO(self):
        ...
        detection_scores = [
            np.array([0.8, 0.2], np.float),
            np.array([0.7, 0.3], np.float),
        ]
        detection_classes = [np.array([1, 1], np.int32), np.array([1, 1], np.int32)]

        categories = [
            {"id": 1, "name": "person", "num_keypoints": 3},
            {"id": 2, "name": "cat"},
            {"id": 3, "name": "dog"},
        ]
    ...
    def testSingleImageGroundtruthExportWithKeypoints(self):
        ...
        visibilities = np.array([[2, 2, 2], [2, 2, 0], [2, 0, 0]], dtype=np.int32)
        areas = np.array([15.0, 16.0, 17.0])
