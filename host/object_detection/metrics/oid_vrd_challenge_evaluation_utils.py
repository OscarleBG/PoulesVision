...
def build_groundtruth_vrd_dictionary(data, class_label_map, relationship_label_map):
    ...
    boxes["subject"] = data_boxes[["YMin1", "XMin1", "YMax1", "XMax1"]].to_numpy()
    ...
    labels["subject"] = (
        data_boxes["LabelName1"].map(lambda x: class_label_map[x]).to_numpy()
    )
    labels["object"] = (
        data_boxes["LabelName2"].map(lambda x: class_label_map[x]).to_numpy()
    )
    labels["relation"] = (
        data_boxes["RelationshipLabel"]
        .map(lambda x: relationship_label_map[x])
        .to_numpy()
    )

    return {
        standard_fields.InputDataFields.groundtruth_boxes: boxes,
        standard_fields.InputDataFields.groundtruth_classes: labels,
...
def build_predictions_vrd_dictionary(data, class_label_map, relationship_label_map):
    ...
    return {
        ...
        standard_fields.DetectionResultFields.detection_scores: data_boxes[
            "Score"
        ].to_numpy(),
    }
