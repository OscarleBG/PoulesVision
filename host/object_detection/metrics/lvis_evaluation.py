...
class LVISMaskEvaluator(object_detection_evaluation.DetectionEvaluator):
    ...
    def __init__(
        self, categories, include_metrics_per_category=False, export_path=None
    ):
        """Constructor.
        ...
        if include_metrics_per_category:
            raise ValueError(
                "include_metrics_per_category not yet supported "
    ...
    def add_single_ground_truth_image_info(self, image_id, groundtruth_dict):
        ...
        self._image_id_to_verified_neg_classes[image_id] = np.flatnonzero(
            groundtruth_dict[fields.InputDataFields.groundtruth_verified_neg_classes]
            == 1
        ).tolist()
        self._image_id_to_not_exhaustive_classes[image_id] = np.flatnonzero(
            groundtruth_dict[fields.InputDataFields.groundtruth_not_exhaustive_classes]
            == 1
        ).tolist()
    ...
    def add_single_detected_image_info(self, image_id, detections_dict):
        ...
        detection_masks = detections_dict[fields.DetectionResultFields.detection_masks]
    ...
    def evaluate(self):
        ...
        tf.logging.info(
            "Performing evaluation on %d images.",
            ...
            "images": [
                {
                    ...
                    "height": shape[1],
                    "width": shape[2],
                    "neg_category_ids": self._image_id_to_verified_neg_classes[
                        image_id
                    ],
                    "not_exhaustive_category_ids": self._image_id_to_not_exhaustive_classes[
                        image_id
                    ],
                }
        ...
        detections = lvis_results.LVISResults(
            lvis_wrapped_groundtruth, self._detection_masks_list
        )
        ...
        mask_metrics = {
            "DetectionMasks_" + key: value for key, value in iter(mask_metrics.items())
        }
        return mask_metrics
            ...
            for (
                ...
                self.add_single_ground_truth_image_info(
                    image_id,
                    {
                        ...
                        input_data_fields.groundtruth_verified_neg_classes: groundtruth_verified_neg_classes,
                        input_data_fields.groundtruth_not_exhaustive_classes: groundtruth_not_exhaustive_classes,
                    },
                )
    ...
    def dump_detections_to_json_file(self, json_output_path):
        ...
        if json_output_path and json_output_path is not None:
            ...
            tf.logging.info(
                "Dumping detections to output json file: %s", json_output_path
            )
