...
tf.flags.DEFINE_string(
    "input_tfrecord_paths", None, "A comma separated list of paths to input TFRecords."
)
tf.flags.DEFINE_string("output_tfrecord_path", None, "Path to the output TFRecord.")
tf.flags.DEFINE_string(
    "inference_graph", None, "Path to the inference graph with embedded weights."
)
...
def main(_):
    ...
    with tf.Session() as sess:
        ...
        ) = detection_inference.build_inference_graph(
            ...
            try:
                for counter in itertools.count():
                    tf.logging.log_every_n(
                        tf.logging.INFO, "Processed %d images...", 10, counter
                    )
                    tf_example = (
                        detection_inference.infer_detections_and_add_to_example(
