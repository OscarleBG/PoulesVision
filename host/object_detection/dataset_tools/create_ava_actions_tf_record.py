...
SPLITS = {
    "train": {"shards": 1000, "examples": 862663, "csv": "", "excluded-csv": ""},
    "val": {"shards": 100, "examples": 243029, "csv": "", "excluded-csv": ""},
    # Test doesn't have ground truth, so TF Records can't be created
    "test": {"shards": 100, "examples": 0, "csv": "", "excluded-csv": ""},
}
...
class Ava(object):
    ...
    def generate_and_write_records(
        self,
        ...
        for key in splits_to_process.split(","):
            ...
            all_metadata = list(
                example_function(
                    download_output[0][key][0],
                    download_output[0][key][1],
                    download_output[1],
            ...
            shard_names = [
                os.path.join(
                    ...
                    FILEPATTERN % key + "-%05d-of-%05d" % (i, shards),
                )
    ...
    def _generate_sequence_examples(
        self,
        annotation_file,
        excluded_file,
        label_map,
        ...
        with open(annotation_file, "r") as annotations:
            ...
            for media_id in ids:
                ...
                while middle_frame_time < POSSIBLE_TIMESTAMPS[-1]:
                        ...
                        for row in frame_annotations[(media_id, windowed_timestamp)]:
                            if len(row) > 2 and int(row["action_label"]) in label_map:
                                ...
                                label_strings.append(
                                    label_map[int(row["action_label"])]
                                )
                                confidences.append(1)
                            else:
                                logging.warning(
                                    "Unknown label: %s", row["action_label"]
                                )
                    ...
                    while (
                        frames_mv < hop_between_sequences + frames_excluded_count
    ...
    def _generate_examples(
        ...
        with open(annotation_file, "r") as annotations:
            ...
            for media_id in ids:
                ...
                while middle_frame_time < POSSIBLE_TIMESTAMPS[-1]:
                    cur_vid.set(
                        cv2.CAP_PROP_POS_MSEC, middle_frame_time * SECONDS_TO_MILLI
                    )
                    _, image = cur_vid.read()
                    _, buffer = cv2.imencode(".jpg", image)
                    ...
                    for row in frame_annotations[(media_id, middle_frame_time)]:
                        if len(row) > 2 and int(row["action_label"]) in label_map:
                            ...
                            areas.append(
                                float((xmaxs[-1] - xmins[-1]) * (ymaxs[-1] - ymins[-1]))
                                / 2
                            )
                            labels.append(int(row["action_label"]))
                    ...
                    date_captured_feature = "2020-06-17 00:%02d:%02d" % (
                    ...
                    context_feature_dict = {
                        ...
                        "image/encoded": dataset_util.bytes_feature(bufstring),
                        ...
                        "image/location": dataset_util.bytes_feature(
                            media_id.encode("utf8")
                        ),
                        "image/date_captured": dataset_util.bytes_feature(
                            date_captured_feature.encode("utf8")
                        ),
                        "image/seq_num_frames": dataset_util.int64_feature(
                            total_non_excluded
                        ),
                        "image/seq_frame_num": dataset_util.int64_feature(
                            cur_frame_num
                        ),
                        "image/seq_id": dataset_util.bytes_feature(
                            media_id.encode("utf8")
                        ),
                    }
    ...
    def _download_data(self):
        ...
        zip_path = os.path.join(
            self.path_to_data_download, ANNOTATION_URL.split("/")[-1]
        )
        ...
        for split in ["train", "test", "val"]:
            csv_path = os.path.join(
                self.path_to_data_download, "ava_%s_v2.2.csv" % split
            )
...
def main(argv):
    ...
    Ava(
        flags.FLAGS.path_to_output_dir, flags.FLAGS.path_to_download_data
...
if __name__ == "__main__":
    flags.DEFINE_string(
        "path_to_download_data", "", "Path to directory to download data to."
    )
    flags.DEFINE_string("path_to_output_dir", "", "Path to directory to write data to.")
    flags.DEFINE_string(
        "splits_to_process",
        "train,val",
