...
class SeqExampleUtilTest(tf.test.TestCase):
    ...
    def test_make_unlabeled_example(self):
        ...
        images = tf.cast(
            tf.random.uniform(
                [num_frames, image_height, image_width, 3], maxval=256, dtype=tf.int32
            ),
            dtype=tf.uint8,
        )
        ...
        self.assertEqual(
            0, context_feature_dict["clip/start/timestamp"].int64_list.value[0]
        )
        self.assertEqual(
            num_frames - 1,
        ...
        self.assertEqual(
            num_frames, context_feature_dict["clip/frames"].int64_list.value[0]
        )
        self.assertEqual(3, context_feature_dict["image/channels"].int64_list.value[0])
        self.assertEqual(
            b"JPEG", context_feature_dict["image/format"].bytes_list.value[0]
        )
        self.assertEqual(
            image_height, context_feature_dict["image/height"].int64_list.value[0]
        )
        self.assertEqual(
            image_width, context_feature_dict["image/width"].int64_list.value[0]
        )
        self.assertEqual(
            video_id, context_feature_dict["clip/media_id"].bytes_list.value[0]
        )

        seq_feature_dict = seq_example.feature_lists.feature_list
        self.assertLen(seq_feature_dict["image/encoded"].feature[:], num_frames)
        timestamps = [
            feature.int64_list.value[0]
            for feature in seq_feature_dict["image/timestamp"].feature
        ]
        self.assertAllEqual(list(range(num_frames)), timestamps)
        source_ids = [
            feature.bytes_list.value[0]
            for feature in seq_feature_dict["image/source_id"].feature
        ]
        self.assertAllEqual(
            [six.ensure_binary(str(idx)) for idx in range(num_frames)], source_ids
        )
    ...
    def test_make_labeled_example(self):
        ...
        label_strings = [np.array(labels), np.array([]), np.array([])]
    ...
    def test_make_labeled_example_with_context_features(self):
        ...
        label_strings = [np.array(labels), np.array([])]
        ...
        self.assertEqual(
            ...
            context_feature_dict["image/context_feature_length"].int64_list.value[0],
        )
        self.assertEqual(
    ...
    def test_make_labeled_example_with_predictions(self):
        ...
        encoded_images = self.materialize_tensors(encoded_images_list)
        bboxes = [
            np.array([[0.0, 0.0, 0.75, 0.75], [0.0, 0.0, 1.0, 1.0]], dtype=np.float32),
            np.array([[0.0, 0.25, 0.5, 0.75]], dtype=np.float32),
        ]
        label_strings = [np.array(["cat", "frog"]), np.array(["cat"])]
        detection_bboxes = [
            np.array([[0.0, 0.0, 0.75, 0.75]], dtype=np.float32),
            np.zeros([0, 4], dtype=np.float32),
        ]
        detection_classes = [
        ...
        self.assertEqual(
            1, context_feature_dict["clip/end/timestamp"].int64_list.value[0]
        )
        self.assertEqual(
            num_frames, context_feature_dict["clip/frames"].int64_list.value[0]
        )
        ...
        # Frame 0.
        self.assertAllEqual(
            1, seq_feature_dict["region/is_annotated"].feature[0].int64_list.value[0]
        )
        self.assertAllClose(
            [0.0, 0.0],
            seq_feature_dict["region/bbox/ymin"].feature[0].float_list.value[:],
        ...
        self.assertAllClose(
            [0.0],
            seq_feature_dict["predicted/region/bbox/ymin"]
            .feature[0]
            .float_list.value[:],
        )
        self.assertAllClose(
            [0.0],
            seq_feature_dict["predicted/region/bbox/xmin"]
            .feature[0]
            .float_list.value[:],
        )
        self.assertAllClose(
        ...
        self.assertAllEqual(
            [5],
            seq_feature_dict["predicted/region/label/index"]
            .feature[0]
            .int64_list.value[:],
        )
        self.assertAllClose(
            [0.9],
            seq_feature_dict["predicted/region/label/confidence"]
            .feature[0]
            .float_list.value[:],
        )
        ...
        # Frame 1.
        self.assertAllEqual(
            1, seq_feature_dict["region/is_annotated"].feature[1].int64_list.value[0]
        )
        self.assertAllClose(
            [0.0], seq_feature_dict["region/bbox/ymin"].feature[1].float_list.value[:]
        )
        self.assertAllClose(
            [0.25], seq_feature_dict["region/bbox/xmin"].feature[1].float_list.value[:]
        )
        self.assertAllClose(
            [0.5], seq_feature_dict["region/bbox/ymax"].feature[1].float_list.value[:]
        )
        self.assertAllClose(
            [0.75], seq_feature_dict["region/bbox/xmax"].feature[1].float_list.value[:]
        )
        self.assertAllEqual(
            [b"cat"],
            seq_feature_dict["region/label/string"].feature[1].bytes_list.value[:],
