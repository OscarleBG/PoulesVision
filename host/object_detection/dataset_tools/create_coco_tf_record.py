...
tf.flags.DEFINE_string("val_annotations_file", "", "Validation annotations JSON file.")
tf.flags.DEFINE_string(
    "testdev_annotations_file", "", "Test-dev annotations JSON file."
)
tf.flags.DEFINE_string(
    "train_keypoint_annotations_file", "", "Training annotations JSON file."
)
tf.flags.DEFINE_string(
    "val_keypoint_annotations_file", "", "Validation annotations JSON file."
)
...
tf.flags.DEFINE_boolean(
    ...
    False,
    "Whether to " "remove all annotations for non-person objects.",
)
tf.flags.DEFINE_boolean(
    "remove_non_person_images",
    False,
    "Whether to " "remove all examples that do not contain a person.",
)
...
_COCO_PART_NAMES = [
    ...
    b"left_foot",
    b"right_foot",
    b"right_upper_leg_back",
    b"left_upper_leg_back",
    b"right_upper_leg_front",
    b"left_upper_leg_front",
    b"right_lower_leg_back",
    b"left_lower_leg_back",
    b"right_lower_leg_front",
    b"left_lower_leg_front",
    b"left_upper_arm_back",
    b"right_upper_arm_back",
    b"left_upper_arm_front",
    b"right_upper_arm_front",
    b"left_lower_arm_back",
    b"right_lower_arm_back",
    b"left_lower_arm_front",
    b"right_lower_arm_front",
    b"right_face",
    b"left_face",
]
...
def create_tf_example(
    ...
    for object_annotations in annotations_list:
        ...
        if include_densepose:
            annotation_id = object_annotations["id"]
            if annotation_id in densepose_annotations_dict and all(
                key in densepose_annotations_dict[annotation_id]
                ...
                dp_x_box_rel = [
                    clip_to_unit(val / 256.0) for val in dp_annotations["dp_x"]
                ]
                dp_x_norm = [
                    (float(x) + x_box_rel * width) / image_width
                    for x_box_rel in dp_x_box_rel
                ]
                dp_y_box_rel = [
                    clip_to_unit(val / 256.0) for val in dp_annotations["dp_y"]
                ]
                dp_y_norm = [
                    (float(y) + y_box_rel * height) / image_height
    ...
    if remove_non_person_images and not any(
        name == b"person" for name in category_names
    ):
        return (
            key,
            None,
    ...
    if include_keypoint:
        feature_dict["image/object/keypoint/x"] = dataset_util.float_list_feature(
            keypoints_x
        )
        feature_dict["image/object/keypoint/y"] = dataset_util.float_list_feature(
            keypoints_y
        )
        feature_dict["image/object/keypoint/num"] = dataset_util.int64_list_feature(
            num_keypoints
        )
        feature_dict[
            "image/object/keypoint/visibility"
        ] = dataset_util.int64_list_feature(keypoints_visibility)
        feature_dict["image/object/keypoint/text"] = dataset_util.bytes_list_feature(
            keypoints_name
        )
        num_keypoint_annotation_skipped = (
    ...
    if include_densepose:
        feature_dict["image/object/densepose/num"] = dataset_util.int64_list_feature(
            dp_num_points
        )
        feature_dict[
            "image/object/densepose/part_index"
        ] = dataset_util.int64_list_feature(dp_part_index)
        feature_dict["image/object/densepose/x"] = dataset_util.float_list_feature(dp_x)
        feature_dict["image/object/densepose/y"] = dataset_util.float_list_feature(dp_y)
        feature_dict["image/object/densepose/u"] = dataset_util.float_list_feature(dp_u)
        feature_dict["image/object/densepose/v"] = dataset_util.float_list_feature(dp_v)
        num_densepose_annotation_skipped = (
...
def _create_tf_record_from_coco_annotations(
    ...
    with contextlib2.ExitStack() as tf_record_close_stack, tf.gfile.GFile(
        ...
        for idx, image in enumerate(images):
            ...
            total_num_densepose_annotations_skipped += num_densepose_annotations_skipped
