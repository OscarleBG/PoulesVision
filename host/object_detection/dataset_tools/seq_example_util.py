...
def sequence_strings_feature(strings):
    ...
    for single_str in strings:
        new_str_arr.append(
            tf.train.Feature(
                bytes_list=tf.train.BytesList(value=[single_str.encode("utf8")])
            )
        )
...
def make_sequence_example(
    ...
    if image_source_ids is not None:
        if use_strs_for_source_id:
            feature_list["image/source_id"] = sequence_strings_feature(image_source_ids)
        else:
    ...
    if label_strings is not None:
        feature_list["region/label/string"] = sequence_bytes_feature(label_strings)
    ...
    if context_features is not None:
        context_dict["image/context_features"] = context_float_feature(context_features)
    ...
    if context_features_image_id_list is not None:
        context_dict["image/context_features_image_id_list"] = context_bytes_feature(
