...
tf.app.flags.DEFINE_string(
    "label_map_path", "data/kitti_label_map.pbtxt", "Path to label map proto."
)
tf.app.flags.DEFINE_integer(
    "validation_set_size", "500", "Number of images to" "be used as a validation set."
)
FLAGS = tf.app.flags.FLAGS


def convert_kitti_to_tfrecords(
    data_dir, output_path, classes_to_use, label_map_path, validation_set_size
):
    ...
    for img_name in images:
        ...
        img_anno = read_annotation_file(
            os.path.join(annotation_dir, str(img_num).zfill(6) + ".txt")
        )
...
def filter_annotations(img_all_annotations, used_classes):
    ...
    for key in img_all_annotations.keys():
        img_filtered_annotations[key] = img_all_annotations[key][
            relevant_annotation_indices
        ]

    if "dontcare" in used_classes:
        dont_care_indices = [
            i for i, x in enumerate(img_filtered_annotations["type"]) if x == "dontcare"
        ]
