...
class GenerateDetectionDataFn(beam.DoFn):
    ...
    def _run_inference_and_generate_detections(self, tfrecord_entry):
        ...
        if input_example.features.feature["image/object/bbox/ymin"].float_list.value:
        ...
        if len(image_class_labels) == 1:
            ...
            for idx, score in enumerate(detection_scores[0]):
                if score >= self._confidence_threshold and idx < num_detections:
                    ...
                    ].float_list.value.extend([detection_boxes[0, idx, 0]])
                    example.features.feature[
                        "image/object/bbox/xmin"
                    ].float_list.value.extend([detection_boxes[0, idx, 1]])
                    example.features.feature[
                        "image/object/bbox/ymax"
                    ].float_list.value.extend([detection_boxes[0, idx, 2]])
                    example.features.feature[
                        "image/object/bbox/xmax"
                    ].float_list.value.extend([detection_boxes[0, idx, 3]])

                    # Add box scores and class texts and labels.
                    example.features.feature[
                        "image/object/class/score"
                    ].float_list.value.extend([score])

                    example.features.feature[
                        "image/object/class/label"
                    ].int64_list.value.extend([image_class_labels[0]])

                    example.features.feature[
                        "image/object/class/text"
                    ].bytes_list.value.extend([image_class_texts[0]])
...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        required=True,
        help="TFRecord containing images in tf.Example format for object " "detection.",
    )
    parser.add_argument(
