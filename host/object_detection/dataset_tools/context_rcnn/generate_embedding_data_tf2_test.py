...
if tf_version.is_tf2():
    from object_detection.dataset_tools.context_rcnn import (
...
class FakeModel(model.DetectionModel):
    def __init__(self, conv_weight_scalar=1.0):
        ...
        self._conv = tf.keras.layers.Conv2D(
            ...
            strides=(1, 1),
            padding="valid",
            kernel_initializer=tf.keras.initializers.Constant(value=conv_weight_scalar),
        )
    ...
    def postprocess(self, prediction_dict, true_image_shapes):
        with tf.control_dependencies(prediction_dict.values()):
            ...
            classifier_feature = np.ones(
                (2, feature_dims, feature_dims, num_features), dtype=np.float32
            ).tolist()
            postprocessed_tensors = {
                "detection_boxes": tf.constant(
                    [[[0.0, 0.1, 0.5, 0.6], [0.5, 0.5, 0.8, 0.8]]], tf.float32
                ),
                "detection_scores": tf.constant([[0.95, 0.6]], tf.float32),
                "detection_multiclass_scores": tf.constant(
                    [[[0.1, 0.7, 0.2], [0.3, 0.1, 0.6]]], tf.float32
                ),
                "detection_classes": tf.constant([[0, 1]], tf.float32),
                "num_detections": tf.constant([2], tf.float32),
                "detection_features": tf.constant([classifier_feature], tf.float32),
            }
...
class GenerateEmbeddingData(tf.test.TestCase):
    ...
    def _create_tf_example(self):
        encoded_image = tf.io.encode_jpeg(
            tf.constant(np.ones((4, 4, 3)).astype(np.uint8))
        ).numpy()
    ...
    def assert_expected_example(self, example, topk=False, botk=False):
        ...
        if topk or botk:
            self.assertEqual(
                len(example.features.feature["image/embedding"].float_list.value), 218
            )
            self.assertAllEqual(
                example.features.feature["image/embedding_count"].int64_list.value, [2]
            )
        else:
            self.assertEqual(
                len(example.features.feature["image/embedding"].float_list.value), 109
            )
            self.assertAllEqual(
                example.features.feature["image/embedding_count"].int64_list.value, [1]
            )

        self.assertAllEqual(
            example.features.feature["image/embedding_length"].int64_list.value, [109]
        )

        # Check annotations
        self.assertAllClose(
            example.features.feature["image/object/bbox/ymin"].float_list.value, [0.0]
        )
        self.assertAllClose(
            example.features.feature["image/object/bbox/xmin"].float_list.value, [0.1]
        )
        self.assertAllClose(
            example.features.feature["image/object/bbox/ymax"].float_list.value, [0.5]
        )
        self.assertAllClose(
            example.features.feature["image/object/bbox/xmax"].float_list.value, [0.6]
        )
        self.assertAllClose(
            example.features.feature["image/object/class/score"].float_list.value,
            [0.95],
        )
        self.assertAllClose(
            example.features.feature["image/object/class/label"].int64_list.value, [5]
        )
        self.assertAllEqual(
        ...
        self.assertAllClose(
            example.features.feature["image/class/label"].int64_list.value, [5]
        )
        self.assertAllEqual(
            example.features.feature["image/class/text"].bytes_list.value, [b"hyena"]
        )
        ...
        self.assertAllEqual(
            example.features.feature["image/source_id"].bytes_list.value, [b"image_id"]
        )
        self.assertTrue(example.features.feature["image/encoded"].bytes_list.value)
    ...
    def test_generate_embedding_data_fn(self):
        ...
        self.assertAllEqual(
            tf.train.Example.FromString(generated_example)
            .features.feature["image/object/class/label"]
            .int64_list.value,
            [5],
        )
        self.assertAllEqual(
            tf.train.Example.FromString(generated_example)
            .features.feature["image/object/class/text"]
            .bytes_list.value,
            [b"hyena"],
        )
    ...
    def test_beam_pipeline(self):
        with InMemoryTFRecord([self._create_tf_example()]) as input_tfrecord:
            ...
            self.assert_expected_example(tf.train.Example.FromString(actual_output[0]))
