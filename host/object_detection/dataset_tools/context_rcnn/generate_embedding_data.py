...
def get_bb_embedding(detection_features, detection_boxes, detection_scores, index):
...
class GenerateEmbeddingDataFn(beam.DoFn):
    ...
    def _run_inference_and_generate_embedding(self, tfexample_key_value):
        ...
        try:
            ...
            unix_time = (
                date_captured - datetime.datetime.fromtimestamp(0)
            ).total_seconds()
        ...
        for index in range(
            ...
            max(-1, num_detections - 1 - self._bottom_k_embedding_count),
            -1,
        ):
            bb_embedding, score = get_bb_embedding(
                detection_features, detection_boxes, detection_scores, index
        ...
        example.features.feature["image/embedding"].float_list.value.extend(embed_all)
...
def construct_pipeline(
    ...
    output_collection = input_collection | "ExtractEmbedding" >> beam.ParDo(
        GenerateEmbeddingDataFn(
            model_dir, top_k_embedding_count, bottom_k_embedding_count, embedding_type
        )
    )
    output_collection = output_collection | "Reshuffle" >> beam.Reshuffle()
    _ = (
        output_collection
        | "DropKeys" >> beam.Map(drop_keys)
        | "WritetoDisk"
