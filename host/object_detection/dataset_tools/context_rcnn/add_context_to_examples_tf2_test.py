...
class GenerateContextDataTest(tf.test.TestCase):
        ...
        example = tf.train.Example(
            features=tf.train.Features(
                feature={
                    ...
                    "image/object/class/text": BytesListFeature(
    ...
    def assert_expected_examples(self, tf_example_list):
        ...
        self.assertAllClose(
            tf_example_list[0]
            .features.feature["image/context_features"]
            .float_list.value,
    ...
    def assert_expected_sequence_example(self, tf_sequence_example_list):
        ...
        self.assertAllEqual(
            tf_sequence_example.context.feature["clip/media_id"].bytes_list.value[0],
            six.ensure_binary("01_0"),
        )
        self.assertAllClose(
        ...
        # Second example.
        self.assertAllClose(
            [0.2], seq_feature_dict["region/bbox/ymin"].feature[1].float_list.value[:]
        )
        self.assertAllClose(
            [0.0], seq_feature_dict["region/bbox/xmin"].feature[1].float_list.value[:]
        )
        self.assertAllClose(
            [0.3], seq_feature_dict["region/bbox/ymax"].feature[1].float_list.value[:]
        )
        self.assertAllClose(
            [0.1], seq_feature_dict["region/bbox/xmax"].feature[1].float_list.value[:]
        )
        self.assertAllEqual(
            [six.ensure_binary("hyena")],
    ...
    def test_sliding_window(self):
        ...
        out_list = [
            list(i)
            for i in add_context_to_examples.get_sliding_window(
                example_list, max_clip_length, stride_length
            )
        ]
        self.assertAllEqual(out_list, [["a", "b", "c"], ["d", "e", "f"], ["g"]])
    ...
    def test_rekey_data_fn(self):
        ...
        rekey_fn = add_context_to_examples.ReKeyDataFn(
            sequence_key, time_horizon, reduce_image_size, max_dim
        )
    ...
    def test_beam_pipeline(self):
        with InMemoryTFRecord(
            [self._create_first_tf_example(), self._create_second_tf_example()]
        ) as input_tfrecord:
            ...
            self.assert_expected_examples(
                [
                    tf.train.Example.FromString(tf_example)
