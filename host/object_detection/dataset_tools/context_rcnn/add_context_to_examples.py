...
class ReKeyDataFn(beam.DoFn):
    ...
    def __init__(
        self, sequence_key, time_horizon, reduce_image_size, max_image_dimension
    ):
        ...
        if time_horizon is None or time_horizon in {
            "year",
            "month",
            "week",
            "day",
    ...
    def _rekey_examples(self, tfrecord_entry):
        ...
        new_key = input_example.features.feature[self._sequence_key].bytes_list.value[0]
...
class SortGroupedDataFn(beam.DoFn):
    ...
    def __init__(
        self, sequence_key, sorted_image_ids, max_num_elements_in_context_features
    ):
        """Initialization function.
...
def get_sliding_window(example_list, max_clip_length, stride_length):
    ...
    if len(example_list) < max_clip_length:
        ...
        starting_values = [
            ...
            if len(example_list) > i * stride_length
        ]
        for start in starting_values:
            result = tuple(
                itertools.islice(
                    example_list, start, min(start + max_clip_length, len(example_list))
                )
            )
            yield result
...
class GenerateContextFn(beam.DoFn):
    ...
    def __init__(
        ...
        self._keep_context_features_image_id_list = keep_context_features_image_id_list
    ...
    def _build_context_features(self, example_list):
        ...
        for idx, example in enumerate(example_list):
            ...
            if self._keep_only_positives:
                if (
                    example.features.feature["image/embedding_score"].float_list.value[
                        0
                    ]
                    < self._context_features_score_threshold
                ):
                    example.features.feature[
                        "context_features_idx"
                    ].int64_list.value.append(
        ...
        if example_list and (
            ...
            feature_length = (
                example_list[0]
                .features.feature["image/embedding_length"]
                .int64_list.value[0]
            )
    ...
    def _add_context_to_example(self, grouped_entry):
        ...
        if self._output_type == "tf_sequence_example":
            ...
            for clip_num, clip_list in enumerate(clips):
                ...
                seq_example.context.feature[
                    "clip/end/timestamp"
                ].int64_list.value.append(len(clip_list))
                ...
                seq_example.context.feature["image/height"].int64_list.value.append(
                    context_example.features.feature["image/height"].int64_list.value[0]
                )
                ...
                ].int64_list.value.append(feature_length)
                seq_example.context.feature[
                    "image/context_features"
                ].float_list.value.extend(context_features)
                if self._keep_context_features_image_id_list:
                    seq_example.context.feature[
                        "image/context_features_image_id_list"
                    ].bytes_list.value.extend(context_features_image_id_list)
...
def construct_pipeline(
    ...
    output_collection = output_collection | "ReshuffleExamples" >> beam.Reshuffle()
...
def parse_args(argv):
    ...
    parser.add_argument(
        "--num_shards", dest="num_shards", default=0, help="Number of output shards."
    )
