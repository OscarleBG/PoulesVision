...
class ParseImage(beam.DoFn):
    ...
    def __init__(self, image_directory, images, annotations, categories, keep_bboxes):
    ...
    def process(self, image_id):
        ...
        feature_dict = {
            ...
            "image/location": dataset_util.bytes_feature(
                str(image_location_id).encode("utf8")
            ),
            "image/seq_num_frames": dataset_util.int64_feature(
                image_sequence_num_frames
            ),
            "image/seq_frame_num": dataset_util.int64_feature(image_sequence_frame_num),
            "image/seq_id": dataset_util.bytes_feature(
                image_sequence_id.encode("utf8")
            ),
            "image/date_captured": dataset_util.bytes_feature(
                image_datetime.encode("utf8")
            ),
        }
        ...
        if annotations:
            ...
            if not category_ids:
                feature_dict.update(
                    {
                        "image/class/label": dataset_util.int64_list_feature([0]),
                ...
                feature_dict.update(
                    {
                        "image/class/label": dataset_util.int64_list_feature(
                            [category_ids[0]]
                        ),
                        "image/class/text": dataset_util.bytes_list_feature(
                            [category_names[0]]
...
def create_pipeline(
    ...
    image_examples = (
        pipeline
        | ("CreateCollections") >> beam.Create([im["id"] for im in data["images"]])
        | ("ParseImage")
        >> beam.ParDo(
            ParseImage(
                image_directory,
