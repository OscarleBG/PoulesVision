...
class CreateCOCOCameraTrapsTfexampleTest(tf.test.TestCase):
    ...
    def assert_expected_example_bbox(self, example):
        self.assertAllClose(
            example.features.feature["image/object/bbox/ymin"].float_list.value, [0.1]
        )
        self.assertAllClose(
            example.features.feature["image/object/bbox/xmin"].float_list.value, [0.0]
        )
        self.assertAllClose(
            example.features.feature["image/object/bbox/ymax"].float_list.value, [0.6]
        )
        self.assertAllClose(
            example.features.feature["image/object/bbox/xmax"].float_list.value, [0.5]
        )
        self.assertAllClose(
        ...
        self.assertAllClose(
            example.features.feature["image/class/label"].int64_list.value, [1]
        )
        self.assertAllEqual(
            example.features.feature["image/class/text"].bytes_list.value, [b"animal"]
        )
        ...
        self.assertAllEqual(
            example.features.feature["image/width"].int64_list.value, [self.IMAGE_WIDTH]
        )
        self.assertAllEqual(
            example.features.feature["image/source_id"].bytes_list.value, [b"im_0"]
        )
    ...
    def test_beam_pipeline(self):
        ...
        create_cococameratraps_tfexample_main.create_pipeline(
            p, temp_dir, json_path, output_tfrecord_prefix=output_tfrecord
        )
        p.run()
    ...
    def test_beam_pipeline_bbox(self):
        ...
        for record in record_iterator:
            actual_output.append(record)
        self.assertEqual(len(actual_output), num_frames)
        self.assert_expected_example_bbox(tf.train.Example.FromString(actual_output[0]))
