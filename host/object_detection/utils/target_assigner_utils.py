...
def _coordinates_to_heatmap_dense(
    y_grid,
    x_grid,
    y_coordinates,
    x_coordinates,
    ...
    num_instances, num_channels = shape_utils.combined_static_and_dynamic_shape(
        channel_onehot
    )
    ...
    reshaped_channel_onehot = tf.reshape(
        channel_onehot, (1, 1, num_instances, num_channels)
    )
...
def _coordinates_to_heatmap_sparse(
    ...
    _, num_channels = shape_utils.combined_static_and_dynamic_shape(channel_onehot)
    ...
    heatmap = tf.tensor_scatter_nd_max(heatmap_init, channel_indices, gaussian_map)
...
def compute_floor_offsets_with_indices(
    y_source, x_source, y_target=None, x_target=None
):
...
def coordinates_to_iou(y_grid, x_grid, blist, channels_onehot, weights=None):
    ...
    per_pixel_xmin = x_grid[tf.newaxis, :, :] - (width[:, tf.newaxis, tf.newaxis] / 2.0)
    per_pixel_ymax = y_grid[tf.newaxis, :, :] + (
        height[:, tf.newaxis, tf.newaxis] / 2.0
    )
    per_pixel_xmax = x_grid[tf.newaxis, :, :] + (width[:, tf.newaxis, tf.newaxis] / 2.0)

    # [num_boxes, height, width] -> [num_boxes * height * width]
    per_pixel_ymin = tf.reshape(per_pixel_ymin, [num_pixels * num_boxes])
    ...
    per_pixel_blist = box_list.BoxList(
        tf.stack(
            [per_pixel_ymin, per_pixel_xmin, per_pixel_ymax, per_pixel_xmax], axis=1
        )
    )

    target_boxes = tf.tile(blist.get()[:, tf.newaxis, :], [1, num_pixels, 1])
    # [num_boxes, height * width, 4] -> [num_boxes * height * wdith, 4]
    target_boxes = tf.reshape(target_boxes, [num_pixels * num_boxes, 4])
    ...
    per_class_iou = (
        ious[:, :, :, tf.newaxis] * channels_onehot[:, tf.newaxis, tf.newaxis, :]
    )

    if weights is not None:
        per_class_iou = per_class_iou * weights[:, tf.newaxis, tf.newaxis, tf.newaxis]

    per_class_iou = tf.maximum(per_class_iou, 0.0)
    return tf.reduce_max(per_class_iou, axis=0)


def get_valid_keypoint_mask_for_class(
    ...
    keypoints_nan_to_zeros = tf.where(
        not_nan, keypoint_coordinates, tf.zeros_like(keypoint_coordinates)
    )
...
def blackout_pixel_weights_by_box_regions(
    height, width, boxes, blackout, weights=None, boxes_scale=1.0
):
    ...
    # 0.0 elsewhere.
    weights_tiled = tf.tile(weights[:, tf.newaxis, tf.newaxis], [1, height, width])
    weights_3d = tf.where(in_boxes, weights_tiled, tf.zeros_like(weights_tiled))
    weights_2d = tf.math.maximum(tf.math.reduce_max(weights_3d, axis=0), 0.0)

    # Add 1.0 to all regions outside a box.
    weights_2d = tf.where(
        tf.math.reduce_any(in_boxes, axis=0), weights_2d, tf.ones_like(weights_2d)
    )
    ...
    keep_region_3d = tf.where(
        in_boxes, keep_region_tiled, tf.ones_like(keep_region_tiled)
    )
...
def _get_yx_indices_offset_by_radius(radius):
    ...
    return (
        tf.constant(y_offsets, dtype=tf.float32),
