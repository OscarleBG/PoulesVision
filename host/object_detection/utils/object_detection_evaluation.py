...
class PrecisionAtRecallDetectionEvaluator(ObjectDetectionEvaluator):
    ...
    def add_single_ground_truth_image_info(self, image_id, groundtruth_dict):
        ...
        super(
            PrecisionAtRecallDetectionEvaluator, self
    ...
    def add_single_detected_image_info(self, image_id, detections_dict):
        ...
        if groundtruth_labeled_classes is not None:
            ...
            allowed = np.isin(
                detections_dict[detection_classes_key], groundtruth_labeled_classes
            )
            ...
            detections_dict[detected_scores_key] = detections_dict[detected_scores_key][
                allowed
            ]

        super(PrecisionAtRecallDetectionEvaluator, self).add_single_detected_image_info(
            image_id, detections_dict
        )
...
class OpenImagesChallengeEvaluator(OpenImagesDetectionEvaluator):
    ...
    def add_single_ground_truth_image_info(self, image_id, groundtruth_dict):
        ...
        Raises:
          ValueError: On adding groundtruth for an image more than once.
        """
        super(OpenImagesChallengeEvaluator, self).add_single_ground_truth_image_info(
            image_id, groundtruth_dict
        )
        input_fields = standard_fields.InputDataFields
        groundtruth_classes = (
            groundtruth_dict[input_fields.groundtruth_classes] - self._label_id_offset
        )
        ...
        if input_fields.groundtruth_image_classes in groundtruth_dict:
            ...
            logging.warning(
                "No image classes field found for image with id %s!", image_id
            )
    ...
    def _update_ground_truth_statistics(
        ...
        for class_index in range(self.num_class):
            ...
            num_groupof_gt_instances = self.group_of_weight * np.sum(
                groundtruth_class_labels[
                    groundtruth_is_group_of_list & ~groundtruth_is_difficult_list
                ]
                == class_index
            )
            self.num_gt_instances_per_class[class_index] += (
                num_gt_instances + num_groupof_gt_instances
            )
    ...
    def evaluate(self):
        ...
        for class_index in range(self.num_class):
            ...
            recall_within_bound_indices = [
                ...
                if value >= self.recall_lower_bound and value <= self.recall_upper_bound
            ]
        ...
        if self.use_weighted_mean_ap:
            ...
            recall_within_bound = recall[recall_within_bound_indices]
            precision_within_bound = precision[recall_within_bound_indices]
            mean_ap = metrics.compute_average_precision(
                precision_within_bound, recall_within_bound
            )
        else:
