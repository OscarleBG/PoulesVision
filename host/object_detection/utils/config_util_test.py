...
class ConfigUtilTest(tf.test.TestCase):
    ...
    def test_get_configs_from_pipeline_file(self):
        ...
        self.assertProtoEquals(
            pipeline_config.train_input_reader, configs["train_input_config"]
        )
        self.assertProtoEquals(pipeline_config.eval_config, configs["eval_config"])
        self.assertProtoEquals(
            pipeline_config.eval_input_reader, configs["eval_input_configs"]
        )
    ...
    def test_create_pipeline_proto_from_configs(self):
        ...
        configs = config_util.get_configs_from_pipeline_file(pipeline_config_path)
        pipeline_config_reconstructed = config_util.create_pipeline_proto_from_configs(
            configs
        )
    ...
    def _assertOptimizerWithNewLearningRate(self, optimizer_name):
        ...
        self.assertAlmostEqual(
            hparams.learning_rate, exponential_lr.initial_learning_rate
        )
        ...
        self.assertAlmostEqual(
            hparams.learning_rate * warmup_scale_factor, cosine_lr.warmup_learning_rate
        )
    ...
    def testGenericConfigOverride(self):
        ...
        self.assertEqual(
            "/some/other/path", configs["train_input_config"].label_map_path
        )
    ...
    def testOverwriteSampleFromDatasetWeights(self):
        ...
        self.assertListEqual(
            [0.5, 0.5], list(configs["train_input_config"].sample_from_datasets_weights)
        )
    ...
    def testOverwriteSampleFromDatasetWeightsWrongLength(self):
        ...
        configs = config_util.get_configs_from_pipeline_file(pipeline_config_path)
        hparams = contrib_training.HParams(sample_from_datasets_weights=[0.5, 0.5, 0.5])
    ...
    def testNewLabelMapPath(self):
        ...
        self.assertEqual(
            new_label_map_path, configs["train_input_config"].label_map_path
        )
    ...
    def testDontOverwriteEmptyLabelMapPath(self):
        ...
        self.assertEqual(
            original_label_map_path, configs["train_input_config"].label_map_path
        )
        self.assertEqual(
    ...
    def testOverwriteAllEvalSampling(self):
        ...
        for eval_input_config in configs["eval_input_configs"]:
            self.assertEqual(
                new_num_eval_examples, eval_input_config.sample_1_of_n_examples
            )
    ...
    def testUpdateInputReaderConfigErrors(self):
        ...
        with self.assertRaisesRegexp(
            ValueError, "Duplicate input name found when overriding."
        ):
            config_util.update_input_reader_config(
                configs,
    ...
    def testOverWriteRetainOriginalImageAdditionalChannels(self):
        ...
        configs = config_util.get_configs_from_pipeline_file(pipeline_config_path)
        override_dict = {
            "retain_original_image_additional_channels_in_eval": desired_retain_original_image_additional_channels
        }
        configs = config_util.merge_external_params_with_configs(
            configs, kwargs_dict=override_dict
        )
    ...
    def testRemoveUnnecessaryEma(self):
        input_dict = {
            ...
            "FeatureExtractor/MobilenetV2_2/expanded_conv_5/expand/act_quant/min": 2,
            "expanded_conv_10/expand/BatchNorm/gamma/min/ExponentialMovingAverage": 3,
            "expanded_conv_3/depthwise/BatchNorm/beta/max/ExponentialMovingAverage": 4,
    ...
    def testUpdateRescoreInstances(self):
        ...
        self.assertEqual(True, cn_config.keypoint_estimation_task[0].rescore_instances)

        config_util.merge_external_params_with_configs(
            configs, kwargs_dict={"rescore_instances": False}
        )
        cn_config = configs["model"].center_net
        self.assertEqual(False, cn_config.keypoint_estimation_task[0].rescore_instances)
    ...
    def testUpdateRescoreInstancesWithMultipleTasks(self):
        ...
        cn_config = configs["model"].center_net
        self.assertEqual(True, cn_config.keypoint_estimation_task[0].rescore_instances)
        self.assertEqual(True, cn_config.keypoint_estimation_task[1].rescore_instances)
