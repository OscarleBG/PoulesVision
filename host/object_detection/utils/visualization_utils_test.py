...
class VisualizationUtilsTest(test_case.TestCase):
    ...
    def test_draw_bounding_boxes_on_image_tensors(self):
        ...
        def graph_fn():
            ...
            boxes = tf.constant(
                [
                    [[0.4, 0.25, 0.75, 0.75], [0.5, 0.3, 0.6, 0.9]],
    ...
    def test_draw_mask_on_image_array(self):
        ...
        visualization_utils.draw_mask_on_image_array(
            test_image, mask, color="Blue", alpha=0.5
        )
    ...
    def test_draw_part_mask_on_image_array(self):
        ...
        mask = np.asarray([[0, 1], [1, 6]], dtype=np.uint8)

        visualization_utils.draw_part_mask_on_image_array(test_image, mask, alpha=0.5)
    ...
    def test_draw_float_channel_on_image_array(self):
        ...
        channel = np.asarray([[0.0, 0.5], [0.0, 1.0]], dtype=np.float32)
        mask = np.asarray([[0, 1], [1, 1]], dtype=np.uint8)
        ...
        expected_result = np.asarray(
            [[[0, 0, 0], [255, 254, 254]], [[0, 0, 255], [255, 0, 0]]], dtype=np.uint8
        )
        self.assertAllEqual(test_image, expected_result)
    ...
    def test_draw_heatmaps_on_image_array(self):
        ...
        heatmap1 = np.asarray([[1, 0], [0, 1]], dtype=np.float)
        heatmap2 = np.asarray([[0, 1], [1, 0]], dtype=np.float)
        ...
        self.assertAllEqual(
            output_image,
            np.array(
                [[[240, 248, 255], [127, 255, 0]], [[127, 255, 0], [240, 248, 255]]]
            ),
        )
    ...
    def test_draw_heatmaps_on_image_tensors(self):
        ...
        def graph_fn():
            ...
            output_image = visualization_utils.draw_heatmaps_on_image_tensors(
                images=image_tensor, heatmaps=heatmaps_tensor, apply_sigmoid=False
            )
    ...
    def test_add_hist_image_summary(self):
        def graph_fn():
            ...
            visualization_utils.add_hist_image_summary(
                values, bins, "ScoresDistribution"
            )
    ...
    def test_eval_metric_ops(self):
        if self.is_tf2():
            self.skipTest(
                "This test is only compatible with Tensorflow 1.X, "
        ...
        detection_boxes = tf.random_uniform(
            [4, 20, 4], minval=0.0, maxval=1.0, dtype=tf.float32
        )
        detection_classes = tf.random_uniform(
            [4, 20], minval=1, maxval=3, dtype=tf.int64
        )
        detection_scores = tf.random_uniform(
            [4, 20], minval=0.0, maxval=1.0, dtype=tf.float32
        )
        groundtruth_boxes = tf.random_uniform(
            [4, 8, 4], minval=0.0, maxval=1.0, dtype=tf.float32
        )
        num_groundtruth_boxes = tf.constant([3, 8, 0, 2], tf.int32)
        groundtruth_classes = tf.random_uniform(
        ...
        eval_dict = {
            ...
            fields.InputDataFields.num_groundtruth_boxes: num_groundtruth_boxes,
        }
                ...
                sess.run(
                    ...
                    feed_dict={
                        original_image: np.random.randint(
                            low=0, high=256, size=(4, 6 + i, 7 + i, 3), dtype=np.uint8
                        ),
                        original_image_spatial_shape: [
                            [6 + i, 7 + i],
                            [6 + i, 7 + i],
    ...
    def test_visualize_boxes_and_labels_on_image_array(self):
        ...
        detections = np.array(
            [[0.8, 0.1, 0.9, 0.1, 1.0, 0.1], [0.1, 0.3, 0.8, 0.7, 1.0, 0.6]]
        )
