    ...
    def test_invalid_box_mask_data(self):
        with self.assertRaises(ValueError):
            np_box_mask_list.BoxMaskList(
                box_data=[0, 0, 1, 1], mask_data=np.zeros([1, 3, 3], dtype=np.uint8)
            )
...
class AddExtraFieldTest(tf.test.TestCase):
    ...
    def test_add_already_existed_field_bbox(self):
        with self.assertRaises(ValueError):
            self.box_mask_list.add_field(
                "boxes", np.array([[0, 0, 0, 1, 0]], dtype=float)
            )
    ...
    def test_add_invalid_field_data(self):
        ...
        with self.assertRaises(ValueError):
            self.box_mask_list.add_field(
                "scores", np.array([0.5, 0.7, 0.9, 0.1], dtype=float)
            )
    ...
    def test_get_extra_fields(self):
        ...
        self.assertItemsEqual(
            box_mask_list.get_extra_fields(), ["masks", "scores", "labels"]
        )
    ...
    def test_num_boxes(self):
        boxes = np.array(
            [[0.0, 0.0, 100.0, 100.0], [10.0, 30.0, 50.0, 70.0]], dtype=float
        )
