...
def get_image_resizer_config(model_config):
    ...
    if hasattr(meta_architecture_config, "image_resizer"):
        ...
        raise ValueError("{} has no image_reszier_config".format(meta_architecture))
...
def get_spatial_image_size(image_resizer_config):
    ...
    if image_resizer_config.HasField(
        "identity_resizer"
    ) or image_resizer_config.HasField("conditional_shape_resizer"):
        return [-1, -1]
...
def clear_fine_tune_checkpoint(pipeline_config_path, new_pipeline_config_path):
...
def update_input_reader_config(
    configs,
    key_name=None,
    input_name=None,
    field_name=None,
...
def _update_momentum_optimizer_value(configs, momentum):
    ...
    if optimizer_type == "rms_prop_optimizer":
        ...
        raise TypeError(
            "Optimizer type must be one of `rms_prop_optimizer` or "
            "`momentum_optimizer`."
        )
...
def remove_unnecessary_ema(variables_to_restore, no_ema_collection=None):
    ...
    for key in variables_to_restore:
        if "ExponentialMovingAverage" in key and any(
            [name in key for name in no_ema_collection]
        ):
...
def _update_sample_from_datasets_weights(input_reader_config, weights):
    ...
    if len(weights) != len(input_reader_config.sample_from_datasets_weights):
        raise ValueError(
            ...
            " ({}) than the configured dataset weights ({}).".format(
                len(input_reader_config.sample_from_datasets_weights), len(weights)
            )
        )
...
def _update_score_distance_multiplier(model_config, score_distance_multiplier):
    ...
    if meta_architecture == "center_net":
        ...
        raise ValueError("Unsupported meta_architecture type: %s" % meta_architecture)
