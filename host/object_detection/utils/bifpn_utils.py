    ...
    if apply_batchnorm:
        layers.append(
            conv_hyperparams.build_batch_norm(
                training=(is_training and not freeze_batchnorm), name=name + "batchnorm"
            )
        )
...
def create_downsample_feature_map_ops(
    scale, downsample_method, conv_hyperparams, is_training, freeze_batchnorm, name
):
    ...
    if downsample_method == "max_pooling":
        ...
        layers.append(
            conv_hyperparams.build_activation_layer(name=name + "downsample_activation")
        )
    else:
...
def create_resample_feature_map_ops(
    input_scale_factor,
    output_scale_factor,
    downsample_method,
    ...
    if input_scale_factor < output_scale_factor:
        if output_scale_factor % input_scale_factor != 0:
            raise ValueError(
                "Invalid scale factor: input scale 1/{} not divisible by"
                "output scale 1/{}".format(input_scale_factor, output_scale_factor)
            )
        scale = output_scale_factor // input_scale_factor
        return create_downsample_feature_map_ops(
            scale,
            downsample_method,
            conv_hyperparams,
            is_training,
        ...
        if input_scale_factor % output_scale_factor != 0:
            raise ValueError(
                "Invalid scale factor: input scale 1/{} not a divisor of"
                "output scale 1/{}".format(input_scale_factor, output_scale_factor)
            )
...
class BiFPNCombineLayer(tf.keras.layers.Layer):
    ...
    def _combine_attention(self, inputs):
        ...
        return tf.squeeze(
            tf.linalg.matmul(tf.stack(inputs, axis=-1), normalized_weights), axis=[-1]
        )
    ...
    def build(self, input_shape):
        if not isinstance(input_shape, list):
            raise ValueError(
                "A BiFPN combine layer should be called " "on a list of inputs."
            )
        if len(input_shape) < 2:
            raise ValueError(
                "A BiFPN combine layer should be called "
