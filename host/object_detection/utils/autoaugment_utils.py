...
def _apply_bbox_augmentation_wrapper(
    image, bbox, new_bboxes, prob, augmentation_func, func_changes_bbox, *args
):
...
def _apply_multi_bbox_augmentation(
    ...
    body = lambda _idx, _images_and_bboxes: [
        _idx + 1,
        wrapped_aug_func(
            _images_and_bboxes[0], loop_bboxes[_idx], _images_and_bboxes[1]
        ),
    ]
    # pylint:enable=g-long-lambda
...
def distort_image_with_autoaugment(image, bboxes, augmentation_name):
    ...
    augmented_image, augmented_bbox = build_and_apply_nas_policy(
        policy, image, bboxes, augmentation_hparams
    )
