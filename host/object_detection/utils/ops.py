...
def filter_groundtruth_with_nan_box_coordinates(tensor_dict):
    ...
    nan_indicator_vector = tf.greater(
        tf.reduce_sum(
            tf.cast(tf.is_nan(groundtruth_boxes), dtype=tf.int32), reduction_indices=[1]
        ),
        0,
    )
...
def filter_unrecognized_classes(tensor_dict):
    ...
    recognized_indices = tf.where(
        tf.greater(
            tensor_dict[fields.InputDataFields.groundtruth_classes], unrecognized_label
        )
    )
...
def merge_boxes_with_multiple_labels(
    ...
    box_confidence_shape_assert = shape_utils.assert_shape_equal_along_first_dimension(
    ...
    with tf.control_dependencies(
        ...
        return (merged_boxes, class_encodings, confidence_encodings, merged_box_indices)


def nearest_neighbor_upsampling(
    input_tensor,
    scale=None,
    height_scale=None,
    width_scale=None,
    ...
    if not scale and (height_scale is None or width_scale is None):
        raise ValueError(
            "Provide either `scale` or `height_scale` and" " `width_scale`."
        )
    with tf.name_scope(name):
        ...
        output_tensor = tf.reshape(
            output_tensor, [batch_size, height, width * w_scale, channels]
        )
        output_tensor = tf.stack([output_tensor] * h_scale, axis=2, name="h_stack")
        return tf.reshape(
            output_tensor, [batch_size, height * h_scale, width * w_scale, channels]
        )
...
def fpn_feature_levels(num_levels, unit_scale_index, image_ratio, boxes):
    ...
    assert (
        unit_scale_index < num_levels and unit_scale_index >= 0
    ...
    levels = tf.cast(
        tf.floordiv(tf.log(areas_sqrt * image_ratio), log_2) + unit_scale_index,
        dtype=tf.int32,
    )
...
EqualizationLossConfig = collections.namedtuple(
    "EqualizationLossConfig", ["weight", "exclude_prefixes"]
)
...
def giou(boxes1, boxes2):
    ...
    intersection_area = tf.maximum(0.0, y2_i - y1_i) * tf.maximum(0.0, x2_i - x1_i)
    ...
    iou = tf.where(
        tf.equal(union_area, 0.0),
        tf.zeros_like(union_area),
        intersection_area / union_area,
    )
    giou_ = iou - tf.where(hull_area > 0.0, (hull_area - union_area) / hull_area, iou)
    return giou_
