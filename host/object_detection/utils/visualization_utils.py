...
def draw_heatmaps_on_image(image, heatmaps):
    ...
    for c in range(channel):
        ...
        bitmap.convert("1")
        draw.bitmap(xy=[(0, 0)], bitmap=bitmap, fill=STANDARD_COLORS[c])
...
def draw_heatmaps_on_image_tensors(images, heatmaps, apply_sigmoid=False):
    ...
    def draw_heatmaps(image_and_heatmaps):
        ...
        image_with_heatmaps = tf.py_function(
            draw_heatmaps_on_image_array, image_and_heatmaps, tf.uint8
        )
...
def draw_side_by_side_evaluation_image(
    ...
    if input_data_fields.num_groundtruth_boxes in eval_dict:
        num_gt_boxes = tf.cast(
            eval_dict[input_data_fields.num_groundtruth_boxes], tf.int32
        )
    for indx in range(eval_dict[input_data_fields.original_image].shape[0]):
        ...
        images_with_detections = draw_bounding_boxes_on_image_tensors(
            ...
            tf.expand_dims(eval_dict[detection_fields.detection_boxes][indx], axis=0),
            tf.expand_dims(eval_dict[detection_fields.detection_classes][indx], axis=0),
            tf.expand_dims(eval_dict[detection_fields.detection_scores][indx], axis=0),
            category_index,
        ...
        images_with_groundtruth = draw_bounding_boxes_on_image_tensors(
            tf.expand_dims(eval_dict[input_data_fields.original_image][indx], axis=0),
            tf.expand_dims(
                eval_dict[input_data_fields.groundtruth_boxes][indx][:num_gt_boxes_i],
                axis=0,
            ),
            tf.expand_dims(
                eval_dict[input_data_fields.groundtruth_classes][indx][:num_gt_boxes_i],
                axis=0,
            ),
            tf.expand_dims(
                tf.ones_like(
...
def draw_densepose_visualizations(
    ...
    if dp_coord_to_visualize not in (0, 1):
        raise ValueError(
            "`dp_coord_to_visualize` must be either 0 for v "
    ...
    for indx in range(eval_dict[input_data_fields.original_image].shape[0]):
        ...
        original_shape = eval_dict[input_data_fields.original_image_spatial_shape][indx]
            ...
            for i, (score, surface_coord, mask) in enumerate(
                ...
                if score > min_score_thresh:
                    ...
                    draw_float_channel_on_image_array(
                        surface_coord_image,
                        surface_coord[:, :, dp_coord_to_visualize],
                        mask,
                    )
        ...
        images_with_detections_list.append(image_with_densepose[tf.newaxis, :, :, :])
    return images_with_detections_list
...
def draw_keypoints_on_image(
    ...
    if keypoint_scores is not None:
        ...
        valid_kpt = np.where(
            np.any(np.isnan(keypoints), axis=1),
...
def draw_float_channel_on_image_array(image, channel, mask, alpha=0.9, cmap="YlGn"):
    ...
    if image.shape[:2] != channel.shape:
        raise ValueError(
            "The image has spatial dimensions %s but the channel has "
    ...
    pil_colored_channel = Image.fromarray(np.uint8(colored_channel * 255)).convert(
        "RGBA"
    )
    pil_mask = Image.fromarray(np.uint8(255.0 * alpha * (mask > 0))).convert("L")
