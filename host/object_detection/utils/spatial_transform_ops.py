...
def _valid_indicator(feature_grid_y, feature_grid_x, true_feature_shapes):
    ...
    valid_indicator = tf.logical_and(
        tf.expand_dims(
            tf.logical_and(feature_grid_y >= 0, tf.less(feature_grid_y, height)), 3
        ),
        tf.expand_dims(
            tf.logical_and(feature_grid_x >= 0, tf.less(feature_grid_x, width)), 2
        ),
    )
...
def ravel_indices(
    feature_grid_y, feature_grid_x, num_levels, height, width, box_levels
):
    ...
    width_indices = tf.reshape(
        feature_grid_x, [batch_size, num_boxes, 1, size_x]
...
def pad_to_max_size(features):
    ...
    """
    if len(features) == 1:
        return tf.expand_dims(features[0], 1), tf.expand_dims(
            tf.shape(features[0])[1:3], 0
        )
    ...
    features_all = [
        tf.image.pad_to_bounding_box(feature, 0, 0, max_height, max_width)
...
def _gather_valid_indices(tensor, indices, padding_value=0.0):
    ...
    padded_tensor = tf.concat(
        [
            padding_value * tf.ones([1, tf.shape(tensor)[-1]], dtype=tensor.dtype),
            tensor,
        ],
        axis=0,
    )
...
def multilevel_roi_align(
    ...
    with tf.name_scope(scope, "MultiLevelRoIAlign"):
        ...
        (
            ...
            feature_grid_y1,
            feature_grid_x1,
        ) = feature_grid_coordinate_vectors(box_grid_y, box_grid_x)
        ...
        valid_indices = _valid_indicator(
...
def multilevel_native_crop_and_resize(images, boxes, box_levels, crop_size, scope=None):
...
def native_crop_and_resize(image, boxes, crop_size, scope=None):
    ...
    with tf.name_scope(scope, "CropAndResize"):
        cropped_regions = tf.image.crop_and_resize(
            image,
            tf.reshape(boxes, [-1] + boxes.shape.as_list()[2:]),
            get_box_inds(boxes),
            crop_size,
        )
        final_shape = tf.concat(
            [tf.shape(boxes)[:2], tf.shape(cropped_regions)[1:]], axis=0
        )
        return tf.reshape(cropped_regions, final_shape)


def multilevel_matmul_crop_and_resize(
    images, boxes, box_levels, crop_size, extrapolation_value=0.0, scope=None
):
...
def matmul_crop_and_resize(
    image, boxes, crop_size, extrapolation_value=0.0, scope=None
):
