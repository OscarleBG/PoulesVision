...
class MockMaskHead(object):
    ...
    def predict(self, features):
        ...
        return tf.zeros(
            (batch_size, 1, self._num_classes, DEFAULT_MASK_SIZE, DEFAULT_MASK_SIZE),
            dtype=tf.float32,
        )
...
class MockKerasBoxPredictor(box_predictor.KerasBoxPredictor):
    ...
    def __init__(self, is_training, num_classes, add_background_class=True):
        ...
        self._dummy_var = tf.Variable(0.0, trainable=True, name="box_predictor_var")
...
def keypoints_with_dynamic_shape(num_instances, num_keypoints, num_coordinates):
    ...
    n = tf.random.uniform(
        [], minval=num_instances, maxval=num_instances + 1, dtype=tf.int32
    )
