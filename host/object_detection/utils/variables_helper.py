...
def get_variables_available_in_checkpoint(
    variables, checkpoint_path, include_global_step=True
):
...
def get_global_variables_safely():
    ...
    with tf.init_scope():
        if tf.executing_eagerly():
            raise ValueError(
                "Global variables collection is not tracked when "
    ...
    if checkpoint_type in DEPRECATED_CHECKPOINT_MAP:
        ...
        for var_name, _ in variables:
            if var_name.startswith(blocked_prefix):
                tf.logging.error(
                    "Found variable name - %s with prefix %s", var_name, blocked_prefix
                )
