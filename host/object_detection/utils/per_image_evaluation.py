...
class PerImageEvaluation(object):
    ...
    def _compute_cor_loc(
        ...
        """
        if (detected_masks is not None and groundtruth_masks is None) or (
            detected_masks is None and groundtruth_masks is not None
        ):
            raise ValueError(
    ...
    def _compute_tp_fp_for_single_class(
        ...
        def compute_match_iou(iou, groundtruth_nongroup_of_is_difficult_list, is_box):
            ...
            for i in range(num_detected_boxes):
                ...
                is_evaluatable = (
                    not tp_fp_labels[i]
                    and not is_matched_to_difficult[i]
                    and iou[i, gt_id] >= self.matching_iou_threshold
        ...
        def compute_match_ioa(ioa, is_box):
            ...
            for i in range(num_detected_boxes):
                ...
                is_evaluatable = (
                    not tp_fp_labels[i]
                    and not is_matched_to_difficult[i]
                    and ioa[i, gt_id] >= self.matching_iou_threshold
                    and not is_matched_to_group_of[i]
                )
                if is_evaluatable:
        ...
        if mask_mode:
            ...
            valid_entries = (
                ~is_matched_to_difficult & ~is_matched_to_group_of & ~is_matched_to_box
            )
            return np.concatenate(
                (scores[valid_entries], scores_mask_group_of)
            ), np.concatenate(
                (tp_fp_labels[valid_entries].astype(float), tp_fp_labels_mask_group_of)
            )
        else:
