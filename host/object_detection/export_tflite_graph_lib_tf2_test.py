    ...
    def postprocess(self, prediction_dict, true_image_shapes):
        ...
        with tf.control_dependencies(list(prediction_dict.values())):
            postprocessed_tensors = {
                ...
                "detection_keypoints": tf.zeros([2, 17, 2], tf.float32),
                "detection_keypoint_scores": tf.zeros([2, 17], tf.float32),
            }
  ...
  center_net {
    ...
    def test_postprocess_implements_signature(self):
        ...
        with mock.patch.object(model_builder, "build", autospec=True) as mock_builder:
            ...
            self.assertEqual(
                expected_signature, detection_module.postprocess_implements_signature()
            )
