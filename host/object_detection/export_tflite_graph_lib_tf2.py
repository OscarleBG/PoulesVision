...
class SSDModule(tf.Module):
    ...
    def __init__(
        self, pipeline_config, detection_model, max_detections, use_regular_nms
    ):
        """Initialization.
    ...
    def postprocess_implements_signature(self):
        ...
        implements_signature = [
            ...
            % self._nms_iou_threshold,
            'attr { key: "y_scale" value { f: %f } }' % self._scale_values["y_scale"],
            'attr { key: "x_scale" value { f: %f } }' % self._scale_values["x_scale"],
            'attr { key: "h_scale" value { f: %f } }' % self._scale_values["h_scale"],
            'attr { key: "w_scale" value { f: %f } }' % self._scale_values["w_scale"],
    ...
    def inference_fn(self, image):
        ...
        return self._get_postprocess_fn(num_anchors, self._num_classes)(
...
class CenterNetModule(tf.Module):
    ...
    def __init__(
        self, pipeline_config, max_detections, include_keypoints, label_map_path=""
    ):
        """Initialization.
    ...
    def inference_fn(self, image):
        ...
        if self._include_keypoints:
            model_outputs = (
                detections[field_names.detection_boxes],
                classes,
                detections[field_names.detection_scores],
                num_detections,
...
def export_tflite_model(
    ...
    if pipeline_config.model.WhichOneof("model") == "ssd":
        ...
        detection_module = SSDModule(
            pipeline_config, detection_model, max_detections, use_regular_nms
        )
        ...
        raise ValueError(
            "Only ssd or center_net models are supported in tflite. "
            "Found {} in config".format(pipeline_config.model.WhichOneof("model"))
        )
    ...
    # Export SavedModel.
    tf.saved_model.save(
        detection_module, output_saved_model_directory, signatures=concrete_function
    )
