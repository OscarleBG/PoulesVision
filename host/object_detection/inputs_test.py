...
def _get_configs_for_model(model_name):
    ...
    label_map_path = os.path.join(
        tf.resource_loader.get_data_files_path(), "data/pet_label_map.pbtxt"
    )
    data_path = os.path.join(
        tf.resource_loader.get_data_files_path(), "test_data/pets_examples.record"
    )
...
def _get_configs_for_model_sequence_example(model_name, frame_index=-1):
    """Returns configurations for model."""
    fname = os.path.join(
        tf.resource_loader.get_data_files_path(), "test_data/" + model_name + ".config"
    )
    label_map_path = os.path.join(
        tf.resource_loader.get_data_files_path(),
...
class InputFnTest(test_case.TestCase, parameterized.TestCase):
    def test_faster_rcnn_resnet50_train_input(self):
        ...
        self.assertAllEqual(
            [1, None, None, 3], features[fields.InputDataFields.image].shape.as_list()
        )
        ...
        self.assertEqual(
            tf.float32, labels[fields.InputDataFields.groundtruth_classes].dtype
        )
        self.assertAllEqual(
            [1, 100], labels[fields.InputDataFields.groundtruth_weights].shape.as_list()
        )
        self.assertEqual(
            tf.float32, labels[fields.InputDataFields.groundtruth_weights].dtype
        )
        self.assertAllEqual(
            [1, 100, model_config.faster_rcnn.num_classes],
        ...
        self.assertEqual(
            tf.float32, labels[fields.InputDataFields.groundtruth_confidences].dtype
        )
    ...
    def test_faster_rcnn_resnet50_train_input_with_additional_channels(self):
        ...
        self.assertAllEqual(
            [1, None, None, 5], features[fields.InputDataFields.image].shape.as_list()
        )
        self.assertAllEqual(
            [1, None, None, 3],
    ...
    def test_faster_rcnn_resnet50_eval_input(self, eval_batch_size=1):
        ...
        self.assertEqual(
            tf.uint8, features[fields.InputDataFields.original_image].dtype
        )
        self.assertAllEqual(
            [eval_batch_size], features[inputs.HASH_KEY].shape.as_list()
        )
        self.assertEqual(tf.int32, features[inputs.HASH_KEY].dtype)
        self.assertAllEqual(
        ...
        self.assertEqual(
            tf.float32, labels[fields.InputDataFields.groundtruth_area].dtype
        )
        self.assertAllEqual(
            [eval_batch_size, 100],
    ...
    def test_context_rcnn_resnet50_train_input_with_sequence_example(
        ...
        """Tests the training input function for FasterRcnnResnet50."""
        configs = _get_configs_for_model_sequence_example("context_rcnn_camera_trap")
        model_config = configs["model"]
        ...
        self.assertAllEqual(
            [train_batch_size, 640, 640, 3],
            features[fields.InputDataFields.image].shape.as_list(),
        )
        self.assertEqual(tf.float32, features[fields.InputDataFields.image].dtype)
        self.assertAllEqual(
            [train_batch_size], features[inputs.HASH_KEY].shape.as_list()
        )
        self.assertEqual(tf.int32, features[inputs.HASH_KEY].dtype)
        self.assertAllEqual(
    ...
    def test_ssd_inceptionV2_train_input(self):
        ...
        self.assertEqual(
            tf.int32, labels[fields.InputDataFields.num_groundtruth_boxes].dtype
        )
        self.assertAllEqual(
            [batch_size, 100, 4],
            labels[fields.InputDataFields.groundtruth_boxes].shape.as_list(),
    ...
    def test_ssd_inceptionV2_eval_input_with_additional_channels(
        ...
        self.assertAllEqual(
            [eval_batch_size, 300, 300, 1],
            features[fields.InputDataFields.image_additional_channels].shape.as_list(),
        )
        self.assertEqual(
            tf.uint8, features[fields.InputDataFields.image_additional_channels].dtype
        )
        self.assertAllEqual(
            [eval_batch_size], features[inputs.HASH_KEY].shape.as_list()
        )
...
class DataAugmentationFnTest(test_case.TestCase):
    def test_apply_image_and_box_augmentation(self):
        ...
        def graph_fn():
            tensor_dict = {
                fields.InputDataFields.image: tf.constant(
            ...
            return (
                augmented_tensor_dict[fields.InputDataFields.image],
    ...
    def test_apply_image_and_box_augmentation_with_scores(self):
        ...
        def graph_fn():
            tensor_dict = {
                ...
                fields.InputDataFields.groundtruth_weights: tf.constant(
    ...
    def test_include_masks_in_data_augmentation(self):
        ...
        def graph_fn():
            tensor_dict = {
                ...
                fields.InputDataFields.groundtruth_instance_masks: tf.constant(
                    np.zeros([2, 10, 10], np.uint8)
                ),
                fields.InputDataFields.groundtruth_instance_mask_weights: tf.constant(
                    [1.0, 0.0], np.float32
                ),
            }
            augmented_tensor_dict = data_augmentation_fn(tensor_dict=tensor_dict)
    ...
    def test_include_keypoints_in_data_augmentation(self):
        ...
        def graph_fn():
            tensor_dict = {
                ...
                fields.InputDataFields.groundtruth_keypoints: tf.constant(
                    np.array([[[0.5, 1.0], [0.5, 0.5]]], np.float32)
                ),
            }
            augmented_tensor_dict = data_augmentation_fn(tensor_dict=tensor_dict)
            return (
                augmented_tensor_dict[fields.InputDataFields.image],
                augmented_tensor_dict[fields.InputDataFields.groundtruth_boxes],
                augmented_tensor_dict[fields.InputDataFields.groundtruth_keypoints],
            )
...
class DataTransformationFnTest(test_case.TestCase, parameterized.TestCase):
    def test_combine_additional_channels_if_present(self):
        ...
        self.assertAllClose(
            out_image, np.concatenate((image, additional_channels), axis=2)
        )
    ...
    def test_use_multiclass_scores_when_present(self):
        def graph_fn():
            tensor_dict = {
                ...
                fields.InputDataFields.groundtruth_boxes: tf.constant(
                    np.array([[0.5, 0.5, 1, 1], [0.5, 0.5, 1, 1]], np.float32)
                ),
                fields.InputDataFields.multiclass_scores: tf.constant(
    ...
    @unittest.skipIf(
        tf_version.is_tf2(), ("Skipping due to different behaviour " "in TF 2.X")
    )
    def test_use_multiclass_scores_when_not_present(self):
        def graph_fn():
            zero_num_elements = tf.random.uniform(
        ...
        groundtruth_classes = self.execute_cpu(graph_fn, [])
        self.assertAllClose(
            np.array([[0, 1, 0], [0, 0, 1]], np.float32), groundtruth_classes
        )
    ...
    def test_use_labeled_classes(self, labeled_classes):
        def compute_fn(
            ...
            tensor_dict = {
                ...
                fields.InputDataFields.groundtruth_labeled_classes: groundtruth_labeled_classes,
            }
    ...
    def test_returns_correct_class_label_encodings(self):
        def graph_fn():
            tensor_dict = {
                ...
                fields.InputDataFields.groundtruth_boxes: tf.constant(
                    np.array([[0, 0, 1, 1], [0.5, 0.5, 1, 1]], np.float32)
                ),
                fields.InputDataFields.groundtruth_classes: tf.constant(
    ...
    def test_returns_correct_labels_with_unrecognized_class(self):
        def graph_fn():
            tensor_dict = {
                ...
                fields.InputDataFields.groundtruth_boxes: tf.constant(
                    np.array(
                        [[0, 0, 1, 1], [0.2, 0.2, 4, 4], [0.5, 0.5, 1, 1]], np.float32
                    )
                ),
                fields.InputDataFields.groundtruth_area: tf.constant(
                    np.array([0.5, 0.4, 0.3])
                ),
                fields.InputDataFields.groundtruth_classes: tf.constant(
                    np.array([3, -1, 1], np.int32)
                ),
                fields.InputDataFields.groundtruth_keypoints: tf.constant(
                    np.array([[[0.1, 0.1]], [[0.2, 0.2]], [[0.5, 0.5]]], np.float32)
                ),
                fields.InputDataFields.groundtruth_keypoint_visibilities: tf.constant(
                    [[True, True], [False, False], [True, True]]
                ),
                fields.InputDataFields.groundtruth_instance_masks: tf.constant(
                    np.random.rand(3, 4, 4).astype(np.float32)
                ),
                fields.InputDataFields.groundtruth_is_crowd: tf.constant(
                    [False, True, False]
                ),
                fields.InputDataFields.groundtruth_difficult: tf.constant(
            ...
            return (
                ...
                transformed_inputs[fields.InputDataFields.groundtruth_instance_masks],
        ...
        self.assertAllClose(groundtruth_area, [0.5, 0.3])
        self.assertAllEqual(groundtruth_confidences, [[0, 0, 1], [1, 0, 0]])
        self.assertAllClose(groundtruth_boxes, [[0, 0, 1, 1], [0.5, 0.5, 1, 1]])
        self.assertAllClose(groundtruth_keypoints, [[[0.1, 0.1]], [[0.5, 0.5]]])
        self.assertAllEqual(
            groundtruth_keypoint_visibilities, [[True, True], [True, True]]
        )
    ...
    def test_returns_correct_merged_boxes(self):
        ...
        self.assertAllClose(groundtruth_boxes, [[0.5, 0.5, 1.0, 1.0]])
        self.assertAllClose(groundtruth_classes, [[1, 0, 1]])
        self.assertAllClose(groundtruth_confidences, [[1, 0, 1]])
        self.assertAllClose(num_groundtruth_boxes, 1)
    ...
    def test_returns_correct_groundtruth_confidences_when_input_present(self):
        def graph_fn():
            tensor_dict = {
                ...
                fields.InputDataFields.groundtruth_confidences: tf.constant(
    ...
    def test_returns_resized_masks(self):
        def graph_fn():
            tensor_dict = {
                ...
                fields.InputDataFields.original_image_spatial_shape: tf.constant(
            ...
            transformed_inputs = input_transformation_fn(tensor_dict)
            return (
                transformed_inputs[fields.InputDataFields.original_image],
                transformed_inputs[fields.InputDataFields.original_image_spatial_shape],
                transformed_inputs[fields.InputDataFields.groundtruth_instance_masks],
            )
    ...
    def test_applies_data_augmentation_fn_to_tensor_dict(self):
        ...
        self.assertAllEqual(groundtruth_classes, [[0, 0, 0, 1], [0, 1, 0, 0]])
    ...
    def test_resize_with_padding(self):
        def graph_fn():
            tensor_dict = {
                fields.InputDataFields.image: tf.constant(
                    np.random.rand(100, 50, 3).astype(np.float32)
                ),
                fields.InputDataFields.groundtruth_boxes: tf.constant(
                    np.array([[0.5, 0.5, 1, 1], [0.0, 0.0, 0.5, 0.5]], np.float32)
                ),
                fields.InputDataFields.groundtruth_classes: tf.constant(
                    np.array([1, 2], np.int32)
                ),
        ...
        self.assertAllClose(
            groundtruth_boxes, [[0.5, 0.25, 1.0, 0.5], [0.0, 0.0, 0.5, 0.25]]
        )
        self.assertAllClose(groundtruth_keypoints, [[[0.1, 0.1]], [[0.3, 0.2]]])
    ...
    def test_groundtruth_keypoint_weights(self):
        ...
        self.assertAllClose(
            groundtruth_keypoints, [[[0.1, 0.1], [0.3, 0.2]], [[0.5, 0.3], [0.7, 0.4]]]
        )
        self.assertAllClose(groundtruth_keypoint_weights, [[1.0, 0.0], [1.0, 2.0]])
    ...
    def test_groundtruth_keypoint_weights_default(self):
        ...
        self.assertAllClose(groundtruth_keypoint_weights, [[1.0, 1.0], [1.0, 1.0]])
    ...
    def test_groundtruth_dense_pose(self):
        def graph_fn():
            tensor_dict = {
                ...
                fields.InputDataFields.groundtruth_dp_num_points: tf.constant(
                    [0, 2], dtype=tf.int32
                ),
                fields.InputDataFields.groundtruth_dp_part_ids: tf.constant(
                    [[0, 0], [4, 23]], dtype=tf.int32
                ),
                fields.InputDataFields.groundtruth_dp_surface_coords: tf.constant(
                    [
                        [
                            [
                                0.0,
            ...
            return (
                transformed_dp_num_points,
                transformed_dp_part_ids,
                transformed_dp_surface_coords,
            )

        dp_num_points, dp_part_ids, dp_surface_coords = self.execute_cpu(graph_fn, [])
    ...
    def test_groundtruth_keypoint_depths(self):
        def graph_fn():
            tensor_dict = {
                ...
                fields.InputDataFields.groundtruth_keypoint_depths: tf.constant(
                    [[1.0, 0.9], [0.8, 0.7]]
                ),
                fields.InputDataFields.groundtruth_keypoint_depth_weights: tf.constant(
                    [[0.7, 0.8], [0.9, 1.0]]
                ),
            }
            ...
            transformed_inputs = input_transformation_fn(tensor_dict=tensor_dict)
            return (
                transformed_inputs[fields.InputDataFields.groundtruth_keypoint_depths],
                transformed_inputs[
        ...
        self.assertAllClose(keypoint_depths, [[1.0, 0.9], [0.8, 0.7]])
        self.assertAllClose(keypoint_depth_weights, [[0.7, 0.8], [0.9, 1.0]])


class PadInputDataToStaticShapesFnTest(test_case.TestCase):
    def test_pad_images_boxes_and_classes(self):
        input_tensor_dict = {
            fields.InputDataFields.image: tf.random.uniform([3, 3, 3]),
            fields.InputDataFields.groundtruth_boxes: tf.random.uniform([2, 4]),
            fields.InputDataFields.groundtruth_classes: tf.random.uniform(
                [2, 3], minval=0, maxval=2, dtype=tf.int32
            ),
            fields.InputDataFields.true_image_shape: tf.constant([3, 3, 3]),
            fields.InputDataFields.original_image_spatial_shape: tf.constant([3, 3]),
        }
        ...
        self.assertAllEqual(
            padded_tensor_dict[fields.InputDataFields.image].shape.as_list(), [5, 6, 3]
        )
        self.assertAllEqual(
            padded_tensor_dict[fields.InputDataFields.true_image_shape].shape.as_list(),
            [3],
        )
        self.assertAllEqual(
            padded_tensor_dict[
    ...
    def test_clip_boxes_and_classes(self):
        def graph_fn():
            input_tensor_dict = {
                fields.InputDataFields.groundtruth_boxes: tf.random.uniform([5, 4]),
                fields.InputDataFields.groundtruth_classes: tf.random.uniform(
                    [2, 3], maxval=10, dtype=tf.int32
                ),
                fields.InputDataFields.num_groundtruth_boxes: tf.constant(5),
            }
            padded_tensor_dict = inputs.pad_input_data_to_static_shapes(
                tensor_dict=input_tensor_dict,
                max_num_boxes=3,
    ...
    def test_images_and_additional_channels(self):
        input_tensor_dict = {
            fields.InputDataFields.image: test_utils.image_with_dynamic_shape(4, 3, 5),
            fields.InputDataFields.image_additional_channels: test_utils.image_with_dynamic_shape(
                4, 3, 2
            ),
        }
        padded_tensor_dict = inputs.pad_input_data_to_static_shapes(
        ...
        self.assertAllEqual(
            padded_tensor_dict[fields.InputDataFields.image].shape.as_list(), [5, 6, 5]
        )
        self.assertAllEqual(
            padded_tensor_dict[
    ...
    def test_images_and_additional_channels_errors(self):
        input_tensor_dict = {
            fields.InputDataFields.image: test_utils.image_with_dynamic_shape(
                10, 10, 3
            ),
            fields.InputDataFields.image_additional_channels: test_utils.image_with_dynamic_shape(
                10, 10, 2
            ),
            fields.InputDataFields.original_image: test_utils.image_with_dynamic_shape(
                10, 10, 3
            ),
        }
    ...
    def test_gray_images(self):
        input_tensor_dict = {
            fields.InputDataFields.image: test_utils.image_with_dynamic_shape(4, 4, 1),
        }
        padded_tensor_dict = inputs.pad_input_data_to_static_shapes(
            tensor_dict=input_tensor_dict,
            max_num_boxes=3,
        ...
        self.assertAllEqual(
            padded_tensor_dict[fields.InputDataFields.image].shape.as_list(), [5, 6, 1]
        )

    def test_gray_images_and_additional_channels(self):
        input_tensor_dict = {
            fields.InputDataFields.image: test_utils.image_with_dynamic_shape(4, 4, 3),
            fields.InputDataFields.image_additional_channels: test_utils.image_with_dynamic_shape(
                4, 4, 2
            ),
        }
    ...
    def test_keypoints(self):
        ...
        visibilities = tf.cast(
            tf.random.uniform(
                tf.shape(keypoints)[:-1], minval=0, maxval=2, dtype=tf.int32
            ),
            tf.bool,
        )
        input_tensor_dict = {
            fields.InputDataFields.groundtruth_keypoints: test_utils.keypoints_with_dynamic_shape(
                10, 16, 4
            ),
            fields.InputDataFields.groundtruth_keypoint_visibilities: visibilities,
        }
        padded_tensor_dict = inputs.pad_input_data_to_static_shapes(
            tensor_dict=input_tensor_dict,
            max_num_boxes=3,
    ...
    def test_pad_input_data_to_static_shapes_for_trackid(self):
        input_tensor_dict = {
            fields.InputDataFields.groundtruth_track_ids: tf.constant(
                [0, 1], dtype=tf.int32
            ),
        }
    ...
    def test_context_features(self):
        ...
        def graph_fn():
            input_tensor_dict = {
                fields.InputDataFields.context_features: tf.ones(
                    [context_memory_size, context_feature_length]
                ),
                fields.InputDataFields.context_feature_length: tf.constant(
...
class NegativeSizeTest(test_case.TestCase):
    ...
    def test_negative_size_error(self):
        ...
        def graph_fn():
            tensors = {
                ...
                fields.InputDataFields.groundtruth_boxes: tf.constant(
                    [[0.5, 0.5, 0.4, 0.5]], tf.float32
                ),
            }
            tensors = inputs.transform_input_data(
                tensors,
                _fake_model_preprocessor_fn,
                _fake_image_resizer_fn,
                num_classes=10,
            )
    ...
    def test_negative_size_no_assert(self):
        ...
        tensors = {
            fields.InputDataFields.image: tf.zeros((128, 128, 3)),
            fields.InputDataFields.groundtruth_classes: tf.constant([1, 1], tf.int32),
            fields.InputDataFields.groundtruth_boxes: tf.constant(
                [[0.5, 0.5, 0.4, 0.5], [0.5, 0.5, 0.6, 0.6]], tf.float32
            ),
        }
        ...
        with mock.patch.object(tf, "Assert") as tf_assert:
            ...
            self.assertAllClose(
                tensors[fields.InputDataFields.groundtruth_boxes],
                [[0.5, 0.5, 0.6, 0.6]],
            )
