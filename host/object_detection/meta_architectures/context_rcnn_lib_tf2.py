...
class ContextProjection(tf.keras.layers.Layer):
    ...
    def __init__(self, projection_dimension, **kwargs):
        self.batch_norm = freezable_batch_norm.FreezableBatchNorm(
            epsilon=0.001, center=True, scale=True, momentum=0.97, trainable=True
        )
        self.projection = tf.keras.layers.Dense(
            units=projection_dimension, use_bias=True
        )
    ...
    def call(self, input_features, is_training=False):
        return tf.nn.relu6(
            self.batch_norm(self.projection(input_features), is_training)
        )
...
class AttentionBlock(tf.keras.layers.Layer):
    ...
    def call(self, box_features, context_features, valid_context_size, num_proposals):
        ...
        keys_values_valid_mask = compute_valid_mask(valid_context_size, context_size)
        ...
        box_features = tf.reshape(
            box_features, [batch_size, self._max_num_proposals, height, width, channels]
        )
        ...
        queries_valid_mask = compute_valid_mask(num_proposals, box_features.shape[1])

        queries = project_features(
            box_features,
            self._bottleneck_dimension,
            self._is_training,
            self._query_proj,
            normalize=True,
        )
...
def filter_weight_value(weights, values, valid_mask):
    ...
    if w_batch_size != v_batch_size or v_batch_size != m_batch_size:
        raise ValueError(
            "Please make sure the first dimension of the input" " tensors are the same."
        )
...
def project_features(
    features, bottleneck_dimension, is_training, layer, normalize=True
):
    ...
    projected_features = tf.reshape(
        projected_features, [batch_size, -1, bottleneck_dimension]
    )
