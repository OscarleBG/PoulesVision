...
class FasterRCNNMetaArch(model.DetectionModel):
        ...
        if isinstance(
            ...
            self._first_stage_box_predictor_first_conv = tf.keras.Sequential(
                [
                    ...
                    tf.keras.layers.Lambda(tf.nn.relu6, name="RPNActivation"),
                ],
    ...
    def _predict_first_stage(self, preprocessed_inputs):
        ...
        clip_window = tf.cast(
            tf.stack([0, 0, image_shape[1], image_shape[2]]), dtype=tf.float32
        )
        ...
        prediction_dict = {
            ...
            "rpn_objectness_predictions_with_background": tf.cast(
                rpn_objectness_predictions_with_background, dtype=tf.float32
            ),
    ...
    def _image_batch_shape_2d(self, image_batch_shape_1d):
        ...
        """
        return tf.tile(
            tf.expand_dims(image_batch_shape_1d[1:], 0), [image_batch_shape_1d[0], 1]
        )
    ...
    def _predict_second_stage(
        ...
        **side_inputs
        ...
        prediction_dict = self._box_prediction(
            rpn_features_to_crop,
    ...
    def _box_prediction(
        ...
        **side_inputs
        ...
        flattened_proposal_feature_maps = self._compute_second_stage_input_feature_maps(
            rpn_features_to_crop, proposal_boxes_normalized, image_shape, **side_inputs
        )
    ...
    def _raw_detections_and_feature_map_inds(
        ...
        return {
            fields.PredictionFields.raw_detection_boxes: raw_detection_boxes_normalized,
    ...
    def _predict_third_stage(self, prediction_dict, image_shapes):
        ...
        if self._is_training:
            ...
            prediction_dict[fields.DetectionResultFields.detection_masks] = tf.reshape(
                tf.sigmoid(detection_masks),
    ...
    def _predict_rpn_proposals(self, rpn_box_predictor_features):
        ...
        objectness_predictions_with_background = tf.concat(
            box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND], axis=1
        )
        return (
    ...
    def _flatten_first_two_dimensions(self, inputs):
        ...
        flattened_shape = tf.stack(
            [combined_shape[0] * combined_shape[1]] + combined_shape[2:]
        )
            ...
            if self._number_of_stages == 1:
                ...
                return {
                    ...
                    fields.DetectionResultFields.num_detections: tf.cast(
                        num_proposals, dtype=tf.float32
                    ),
    ...
    def _add_detection_box_rpn_features_output_node(
        self, detection_boxes, rpn_features_to_crop, image_shape
    ):
        """Add detection features to outputs.
    ...
    def _postprocess_rpn(
        ...
        if self._is_training:
            ...
            if not self._hard_example_miner:
                ...
                (
                    ...
                    proposal_scores,
                    num_proposals,
                ) = self._sample_box_classifier_batch(
                    proposal_boxes,
                    proposal_scores,
                    num_proposals,
                    groundtruth_boxlists,
        ...
        raw_normalized_proposal_boxes = shape_utils.static_or_dynamic_map_fn(
            normalize_boxes, elems=[raw_proposal_boxes, image_shapes], dtype=tf.float32
        )
        proposal_multiclass_scores = (
            nmsed_additional_fields.get("multiclass_scores")
            if nmsed_additional_fields
            else None
        )
        return (
            normalized_proposal_boxes,
            proposal_scores,
            proposal_multiclass_scores,
            num_proposals,
    ...
    def _sample_box_classifier_batch(
        ...
        for (
            ...
            single_image_boxlist.add_field(
                fields.BoxListFields.scores, single_image_proposal_scores
            )
            ...
            single_image_num_proposals_sample.append(
                tf.minimum(sampled_boxlist.num_boxes(), self._second_stage_batch_size)
            )
    ...
    def _sample_box_classifier_minibatch_single_image(
        ...
        return box_list_ops.boolean_mask(
            ...
            indicator_sum=(
                self._second_stage_batch_size if self._use_static_shapes else None
            ),
        )

    def _compute_second_stage_input_feature_maps(
        self, features_to_crop, proposal_boxes_normalized, image_shape, **side_inputs
    ):
    ...
    def _postprocess_box_classifier(
        ...
        if refined_decoded_boxes_batch.shape[2] > 1:
            class_ids = tf.expand_dims(
                tf.argmax(
                    class_predictions_with_background_batch[:, :, 1:],
                    axis=2,
        ...
        detections = {
            ...
            fields.DetectionResultFields.detection_multiclass_scores: nmsed_additional_fields[
                "multiclass_scores"
            ],
    ...
    def _normalize_and_clip_boxes(self, boxes_and_image_shape):
        ...
        if boxes_contains_classes_dim:
            (
                ...
                num_classes,
                _,
            ) = shape_utils.combined_static_and_dynamic_shape(boxes_and_image_shape[0])
    ...
    def _loss_rpn(
        ...
        with tf.name_scope("RPNLoss"):
            ...
            loss_dict = {
                "Loss/RPNLoss/localization_loss": localization_loss,
                "Loss/RPNLoss/objectness_loss": objectness_loss,
            }
        return loss_dict

    def _loss_box_classifier(
        self,
        refined_box_encodings,
        class_predictions_with_background,
        proposal_boxes,
        ...
        with tf.name_scope("BoxClassifierLoss"):
            ...
            normalizer = (
                tf.tile(num_proposals_or_one, [1, self.max_num_proposals]) * batch_size
            )
            ...
            second_stage_cls_losses = (
                ops.reduce_sum_trailing_dimensions(
            ...
            second_stage_loc_loss = tf.reduce_sum(
                second_stage_loc_losses * tf.cast(paddings_indicator, dtype=tf.float32)
            )
            second_stage_cls_loss = tf.reduce_sum(
                second_stage_cls_losses * tf.cast(paddings_indicator, dtype=tf.float32)
            )
            ...
            loss_dict = {
                "Loss/BoxClassifierLoss/localization_loss": localization_loss,
            ...
            if prediction_masks is not None:
                ...
                mask_losses_weights = batch_mask_target_weights * tf.cast(
                    paddings_indicator, dtype=tf.float32
                )
    ...
    def _get_mask_proposal_boxes_and_classes(
        ...
        proposal_boxes = tf.reshape(
            box_list_ops.to_absolute_coordinates(
                box_list.BoxList(tf.reshape(detection_boxes, [-1, 4])),
                image_shape[1],
                image_shape[2],
            ).get(),
            [batch, max_num_detections, 4],
        )
        proposal_boxlists = [
    ...
    def _get_refined_encodings_for_postitive_class(
        self, refined_box_encodings, flat_cls_targets_with_background, batch_size
    ):
    ...
    def _unpad_proposals_and_apply_hard_mining(
        ...
        for (
            ...
            proposal_boxlist = box_list.BoxList(
                tf.slice(
                    proposal_boxlist.get(), [0, 0], [single_image_num_proposals, -1]
                )
            )
            single_image_loc_loss = tf.slice(
                single_image_loc_loss, [0], [single_image_num_proposals]
            )
            single_image_cls_loss = tf.slice(
                single_image_cls_loss, [0], [single_image_num_proposals]
            )
    ...
    def restore_from_objects(self, fine_tune_checkpoint_type="detection"):
        ...
        if fine_tune_checkpoint_type == "classification":
            ...
            fake_model = tf.train.Checkpoint(
                _feature_extractor_for_box_classifier_features=self._feature_extractor_for_box_classifier_features,
                _feature_extractor_for_proposal_features=self._feature_extractor_for_proposal_features,
            )
    ...
    def updates(self):
        ...
        if isinstance(self._first_stage_box_predictor_first_conv, tf.keras.Model):
            update_ops.extend(
                self._first_stage_box_predictor_first_conv.get_updates_for(None)
            )
            update_ops.extend(
