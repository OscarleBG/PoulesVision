...
class CenterNetFeatureExtractor(tf.keras.Model):
    ...
    def __init__(
        ...
        if (
            channel_means is None or len(channel_means) == 0
        ):  # pylint:disable=g-explicit-length-test
            channel_means = [0.0, 0.0, 0.0]

        if (
            channel_stds is None or len(channel_stds) == 0
        ):  # pylint:disable=g-explicit-length-test
            channel_stds = [1.0, 1.0, 1.0]
...
def make_prediction_net(
    ...
    out_conv = tf.keras.layers.Conv2D(
        num_out_channels, kernel_size=1, name="conv1" if tf_version.is_tf1() else None
    )
    ...
    layers = []
    for idx, (kernel_size, num_filter) in enumerate(zip(kernel_sizes, num_filters)):
        layers.append(
            conv_fn(
                num_filter,
...
def _flatten_spatial_dimensions(batch_images):
    ...
    return tf.reshape(batch_images, [batch_size, height * width, channels])


def _multi_range(limit, value_repetitions=1, range_repetitions=1, dtype=tf.int32):
    ...
    return tf.reshape(
        tf.tile(
            ...
            multiples=[range_repetitions, value_repetitions],
        ),
        [-1],
    )


def top_k_feature_map_locations(
    feature_map, max_pool_kernel_size=3, k=100, per_channel=False
):
    ...
    if per_channel:
        if k == 1:
            ...
            feature_map_peaks_transposed = tf.transpose(
                feature_map_peaks, perm=[0, 3, 1, 2]
            )
            ...
            peak_flat_indices = tf.pad(
                peak_flat_indices, [(0, 0), (0, 0), (0, k - safe_k)]
            )
            scores = tf.ensure_shape(scores, (batch_size, num_channels, k))
            peak_flat_indices = tf.ensure_shape(
                peak_flat_indices, (batch_size, num_channels, k)
            )
...
def prediction_tensors_to_boxes(
    y_indices, x_indices, height_width_predictions, offset_predictions
):
...
def prediction_tensors_to_keypoint_candidates(
    ...
    selected_offsets_flat = tf.gather_nd(keypoint_heatmap_offsets, combined_indices)
    selected_offsets = tf.reshape(selected_offsets_flat, [batch_size, num_indices, -1])
    ...
    if num_channels > 2:
        ...
        reshaped_offsets = tf.reshape(
            selected_offsets, [batch_size, num_indices, -1, 2]
        )
    ...
    keypoint_candidates = tf.stack(
        [y_indices + y_offsets, x_indices + x_offsets], axis=2
    )
    keypoint_candidates = tf.reshape(
        keypoint_candidates, [batch_size, num_keypoints, max_candidates, 2]
    )
    keypoint_candidates = tf.transpose(keypoint_candidates, [0, 2, 1, 3])
    keypoint_scores = tf.reshape(
        keypoint_scores, [batch_size, num_keypoints, max_candidates]
    )
    ...
    if keypoint_depths is not None:
        ...
        selected_depth = tf.reshape(selected_depth_flat, [batch_size, num_indices, -1])
        ...
        depth_candidates = tf.reshape(
            depth, [batch_size, num_keypoints, max_candidates]
        )
...
def argmax_feature_map_locations(feature_map):
    ...
    y_indices, x_indices = row_col_indices_from_flattened_indices(
        peak_flat_indices, width
    )
    channel_indices = tf.tile(tf.range(num_channels)[tf.newaxis, :], [batch_size, 1])
    return y_indices, x_indices, channel_indices


def prediction_tensors_to_single_instance_kpts(
    keypoint_heatmap_predictions, keypoint_heatmap_offsets, keypoint_score_heatmap=None
):
    """Convert keypoint heatmap predictions and offsets to keypoint candidates.
    ...
    if keypoint_score_heatmap is None:
        keypoint_scores = tf.gather_nd(keypoint_heatmap_predictions, combined_indices)
    else:
        keypoint_scores = tf.gather_nd(keypoint_score_heatmap, combined_indices)
    keypoint_scores = tf.expand_dims(tf.expand_dims(keypoint_scores, axis=0), axis=0)
    return keypoint_candidates, keypoint_scores


def _score_to_distance_map(
    y_grid, x_grid, heatmap, points_y, points_x, score_distance_offset
):
...
def prediction_to_single_instance_keypoints(
    ...
    if kp_params.candidate_ranking_mode == "score_distance_ratio":
        ...
        raise ValueError(
            "Unsupported candidate_ranking_mode: %s" % kp_params.candidate_ranking_mode
        )
    ...
    (keypoint_candidates, keypoint_scores) = prediction_tensors_to_single_instance_kpts(
...
def _gaussian_weighted_map_const_multi(
    y_grid, x_grid, heatmap, points_y, points_x, boxes, gaussian_denom_ratio
):
    """Rescores heatmap using the distance information.
    ...
    # doesn't like it.
    y_diff = tf.reshape(y_grid, [height, width, 1, 1]) - tf.reshape(
        points_y, [1, 1, num_instances, num_keypoints]
    )
    x_diff = tf.reshape(x_grid, [height, width, 1, 1]) - tf.reshape(
        points_x, [1, 1, num_instances, num_keypoints]
    )
    ...
    in_boxes = tf.math.logical_and(
        tf.math.logical_and(
            tf.reshape(y_grid, [height, width, 1])
            >= tf.reshape(y_min, [1, 1, num_instances]),
            tf.reshape(y_grid, [height, width, 1])
            < tf.reshape(y_max, [1, 1, num_instances]),
        ),
        tf.math.logical_and(
            tf.reshape(x_grid, [height, width, 1])
            >= tf.reshape(x_min, [1, 1, num_instances]),
            tf.reshape(x_grid, [height, width, 1])
            < tf.reshape(x_max, [1, 1, num_instances]),
        ),
    )
    in_boxes = tf.cast(in_boxes, dtype=tf.float32)

    gaussian_denom = (
        tf.cast(tf.minimum(height, width), dtype=tf.float32) * gaussian_denom_ratio
    )
    ...
    return (
        ...
        * gaussian_map
        * tf.reshape(in_boxes, [height, width, num_instances, 1])
    )
...
def prediction_tensors_to_multi_instance_kpts(
    ...
    # [height * width, num_instances * num_keypoints].
    feature_map_flattened = tf.reshape(
        keypoint_heatmap_predictions, [-1, num_instances * num_keypoints]
    )
    ...
    kpts_idx = _multi_range(
        limit=num_keypoints, value_repetitions=1, range_repetitions=num_instances
    )
    combined_indices = tf.stack([y_indices, x_indices, kpts_idx], axis=1)
...
def prediction_to_keypoints_argmax(
    prediction_dict, object_y_indices, object_x_indices, boxes, task_name, kp_params
):
    ...
    """
    keypoint_heatmap = tf.squeeze(
        tf.nn.sigmoid(
            prediction_dict[get_keypoint_name(task_name, KEYPOINT_HEATMAP)][-1]
        ),
        axis=0,
    )
    keypoint_offset = tf.squeeze(
        prediction_dict[get_keypoint_name(task_name, KEYPOINT_OFFSET)][-1], axis=0
    )
    keypoint_regression = tf.squeeze(
        prediction_dict[get_keypoint_name(task_name, KEYPOINT_REGRESSION)][-1], axis=0
    )
    ...
    combined_obj_indices = tf.stack(
        [tf.reshape(object_y_indices, [-1]), tf.reshape(object_x_indices, [-1])], axis=1
    )

    # Select the regression vectors from the object center.
    selected_regression_flat = tf.gather_nd(keypoint_regression, combined_obj_indices)
    ...
    y_regressed = (
        tf.cast(tf.reshape(object_y_indices, [num_instances, 1]), dtype=tf.float32)
        + y_reg
    )
    x_regressed = (
        tf.cast(tf.reshape(object_x_indices, [num_instances, 1]), dtype=tf.float32)
        + x_reg
    )
    ...
    (keypoint_candidates, keypoint_scores) = prediction_tensors_to_multi_instance_kpts(
...
def regressed_keypoints_at_object_centers(
    regressed_keypoint_predictions, y_indices, x_indices
):
    ...
    relative_regressed_keypoints = tf.gather_nd(
        regressed_keypoint_predictions, combined_indices
    )
    relative_regressed_keypoints = tf.reshape(
        relative_regressed_keypoints, [batch_size, num_instances, -1, 2]
    )
...
def sdr_scaled_ranking_score(
    ...
    # Shape: [batch_size, num_instances].
    offsets = tf.math.maximum(ymax - ymin, xmax - xmin) * score_distance_multiplier
...
def gaussian_weighted_score(keypoint_scores, distances, keypoint_std_dev, bboxes):
    ...
    # shape: [batch_size, num_instances]
    sigma = (
        cn_assigner._compute_std_dev_from_box_size(  # pylint: disable=protected-access
    ...
    # shape: [batch_size, num_instances, max_candidates, num_keypoints]
    sigma = tf.tile(sigma[:, :, tf.newaxis, :], multiples=[1, 1, max_candidates, 1])
...
def refine_keypoints(
    ...
    num_candidates_tiled = tf.tile(
        tf.expand_dims(num_keypoint_candidates, 1), [1, max_candidates, 1]
    )
    ...
    keypoint_candidates_expanded = tf.expand_dims(keypoint_candidates, axis=1)
    ...
    if candidate_ranking_mode == "min_distance":
        ...
        tiled_keypoint_scores = tf.tile(
            tf.expand_dims(keypoint_scores, axis=1), multiples=[1, num_instances, 1, 1]
        )
    ...
    if bboxes is None:
        ...
        # keypoint score.
        mask = tf.cast(nearby_candidate_scores < keypoint_score_threshold, tf.int32)
    else:
        ...
        mask = (
            tf.cast(nearby_candidate_coords[:, :, :, 0] < ymin, tf.int32)
            ...
            # Filter out the chosen candidate with score lower than unmatched
            # keypoint score.
            tf.cast(nearby_candidate_scores < keypoint_score_threshold, tf.int32)
    ...
    if nearby_candidate_depths is not None:
        refined_depths = tf.where(
            mask, tf.zeros_like(nearby_candidate_depths), nearby_candidate_depths
        )

    return refined_keypoints, refined_scores, refined_depths


def _pad_to_full_keypoint_dim(
    keypoint_coords, keypoint_scores, keypoint_inds, num_total_keypoints
):
    ...
    Returns:
      ...
      keypoint_scores_padded: a [batch_size, num_instances, num_total_keypoints]
        float32 tensor.
    """
    batch_size, num_instances, _, _ = shape_utils.combined_static_and_dynamic_shape(
        keypoint_coords
    )
...
def _pad_to_full_instance_dim(
    keypoint_coords, keypoint_scores, instance_inds, max_instances
):
    ...
    Returns:
      ...
      keypoint_scores_padded: a [batch_size, max_instances, num_keypoints]
        float32 tensor.
    """
    batch_size, _, num_keypoints, _ = shape_utils.combined_static_and_dynamic_shape(
        keypoint_coords
    )
...
def _gather_candidates_at_indices(
    keypoint_candidates, keypoint_scores, indices, keypoint_depth_candidates=None
):
    ...
    nearby_candidate_coords_transposed = tf.reshape(
        nearby_candidate_coords_transposed, [batch_size, num_keypoints, num_indices, -1]
    )

    nearby_candidate_scores_transposed = tf.gather_nd(
        keypoint_scores_transposed, combined_indices
    )
    nearby_candidate_scores_transposed = tf.reshape(
        nearby_candidate_scores_transposed, [batch_size, num_keypoints, num_indices]
    )
    ...
    new_shape = tf.concat((original_shape[:3], [tf.maximum(original_shape[3], 2)]), 0)
    gathered_keypoint_candidates = tf.reshape(gathered_keypoint_candidates, new_shape)
    gathered_keypoint_scores = tf.transpose(
        nearby_candidate_scores_transposed, [0, 2, 1]
    )

    gathered_keypoint_depths = None
    if keypoint_depth_candidates is not None:
        keypoint_depths_transposed = tf.transpose(keypoint_depth_candidates, [0, 2, 1])
        ...
        nearby_candidate_depths_transposed = tf.reshape(
            nearby_candidate_depths_transposed, [batch_size, num_keypoints, num_indices]
        )
        gathered_keypoint_depths = tf.transpose(
            nearby_candidate_depths_transposed, [0, 2, 1]
        )
    return (
...
def convert_strided_predictions_to_normalized_keypoints(
    ...
    if clip_out_of_frame_keypoints:
        ...
        valid_indices = tf.logical_and(
            tf.logical_and(
                keypoint_coords_normalized[:, :, :, 0] >= 0.0,
                keypoint_coords_normalized[:, :, :, 0] <= 1.0,
            ),
            tf.logical_and(
                keypoint_coords_normalized[:, :, :, 1] >= 0.0,
        ...
        keypoint_coords_normalized = shape_utils.static_or_dynamic_map_fn(
            clip_to_window,
            [keypoint_coords_normalized, batch_window],
            dtype=tf.float32,
            back_prop=False,
        )
        keypoint_scores = tf.where(
            valid_indices, keypoint_scores, tf.zeros_like(keypoint_scores)
        )
...
def crop_and_threshold_masks(
    ...
    part_heatmap_4d = tf.tile(
        part_heatmap[tf.newaxis, :, :, :], multiples=[num_classes, 1, 1, 1]
    )
    surf_coords_4d = tf.tile(
        surf_coords[tf.newaxis, :, :, :], multiples=[num_classes, 1, 1, 1]
    )
    feature_maps_concat = tf.concat(
        [masks_4d, part_heatmap_4d, surf_coords_4d], axis=-1
    )
    ...
    instance_masks_without_parts = tf.math.multiply(
        instance_masks_int, det_without_parts
    )
    ...
    part_mask_int_zero_indexed = tf.math.argmax(
        part_heatmap_cropped, axis=-1, output_type=tf.int32
    )[:, :, :, tf.newaxis]
    ...
    surface_coords = gather_surface_coords_for_parts(
        surface_coords_cropped, part_mask_int_zero_indexed
    )
...
def gather_surface_coords_for_parts(surface_coords_cropped, highest_scoring_part):
    ...
    flattened_lookup_indices = (
        num_parts * tf.range(max_detections * height * width) + flattened_part_ids
    )

    vu_coords_flattened = tf.gather(
        flattened_surface_coords, flattened_lookup_indices, axis=0
    )
...
def mask_from_true_image_shape(data_shape, true_image_shapes):
    ...
    """
    mask_h = tf.cast(
        tf.range(data_shape[1]) < true_image_shapes[:, tf.newaxis, 0], tf.float32
    )
    mask_w = tf.cast(
        tf.range(data_shape[2]) < true_image_shapes[:, tf.newaxis, 1], tf.float32
    )
    mask = tf.expand_dims(mask_h[:, :, tf.newaxis] * mask_w[:, tf.newaxis, :], 3)
    return mask
...
class ObjectDetectionParams(
    ...
    def __new__(
        ...
        """
        return super(ObjectDetectionParams, cls).__new__(
            cls,
            localization_loss,
            scale_loss_weight,
            offset_loss_weight,
...
class ObjectCenterParams(
    ...
    def __new__(
        ...
        """
        return super(ObjectCenterParams, cls).__new__(
            cls,
            classification_loss,
            object_center_loss_weight,
            heatmap_bias_init,
...
class MaskParams(
    ...
    def __new__(
        ...
        """
        return super(MaskParams, cls).__new__(
            cls,
            classification_loss,
            task_loss_weight,
            mask_height,
...
class DensePoseParams(
    ...
    def __new__(
        ...
        """
        return super(DensePoseParams, cls).__new__(
            cls,
            class_id,
            classification_loss,
            localization_loss,
...
class TrackParams(
    ...
    def __new__(
        ...
        """
        return super(TrackParams, cls).__new__(
            cls,
            num_track_ids,
            reid_embed_size,
            num_fc_layers,
...
class TemporalOffsetParams(
    collections.namedtuple(
        "TemporalOffsetParams", ["localization_loss", "task_loss_weight"]
    )
):
    ...
    def __new__(cls, localization_loss, task_loss_weight=1.0):
        ...
        """
        return super(TemporalOffsetParams, cls).__new__(
            cls, localization_loss, task_loss_weight
        )
...
class CenterNetMetaArch(model.DetectionModel):
    ...
    def __init__(
        ...
        if densepose_params is not None and mask_params is None:
            raise ValueError(
                "To run DensePose prediction, `mask_params` must also " "be supplied."
            )
    ...
    def batched_prediction_tensor_names(self):
        if not self._batched_prediction_tensor_names:
            raise RuntimeError(
                "Must call predict() method to get batched prediction " "tensor names."
            )
        return self._batched_prediction_tensor_names

    def _make_prediction_net_list(
        self,
        num_feature_outputs,
        num_out_channels,
        kernel_sizes=(3),
    ...
    def _construct_prediction_heads(
        self,
        num_classes,
        num_feature_outputs,
        ...
        if self._kp_params_dict is not None:
            for task_name, kp_params in self._kp_params_dict.items():
                ...
                prediction_heads[
                    get_keypoint_name(task_name, KEYPOINT_HEATMAP)
                ] = self._make_prediction_net_list(
                    num_feature_outputs,
                    num_keypoints,
                ...
                prediction_heads[
                    get_keypoint_name(task_name, KEYPOINT_REGRESSION)
                ] = self._make_prediction_net_list(
                    num_feature_outputs,
                ...
                if kp_params.per_keypoint_offset:
                    prediction_heads[
                        get_keypoint_name(task_name, KEYPOINT_OFFSET)
                    ] = self._make_prediction_net_list(
                        num_feature_outputs,
                        NUM_OFFSET_CHANNELS * num_keypoints,
                ...
                if kp_params.predict_depth:
                    ...
                    prediction_heads[
                        get_keypoint_name(task_name, KEYPOINT_DEPTH)
                    ] = self._make_prediction_net_list(
                        num_feature_outputs,
                        num_depth_channel,
                        name="kpt_depth",
    ...
    def _initialize_target_assigners(self, stride, min_box_overlap_iou):
        ...
        """
        target_assigners = {}
        keypoint_weights_for_center = self._center_params.keypoint_weights_for_center
        ...
        if self._od_params is not None:
            target_assigners[DETECTION_TASK] = cn_assigner.CenterNetBoxTargetAssigner(
                stride
            )
        if self._kp_params_dict is not None:
            for task_name, kp_params in self._kp_params_dict.items():
        ...
        if self._track_params is not None:
            target_assigners[TRACK_TASK] = cn_assigner.CenterNetTrackTargetAssigner(
    ...
    def _compute_object_center_loss(
        self,
        input_height,
        input_width,
    ...
    def _compute_box_scale_and_offset_loss(
        self,
        input_height,
        input_width,
        scale_predictions,
    ...
    def _compute_keypoint_estimation_losses(
        self, task_name, input_height, input_width, prediction_dict, per_pixel_weights
    ):
    ...
    def _compute_kp_heatmap_loss(
        ...
        (
            keypoint_heatmap,
            num_instances_per_kp_type,
            valid_mask_batch,
        ) = assigner.assign_keypoint_heatmap_targets(
            height=input_height,
            width=input_width,
            gt_keypoints_list=gt_keypoints_list,
            gt_weights_list=gt_weights_list,
        ...
        num_instances = tf.maximum(
            tf.cast(tf.reduce_sum(num_instances_per_kp_type), dtype=tf.float32), 1.0
        )
        loss = 0.0
        # Loop through each feature output head.
        for pred in heatmap_predictions:
            ...
            unweighted_loss = classification_loss_fn(
                pred, flattened_heapmap_targets, weights=tf.ones_like(per_pixel_weights)
            )
    ...
    def _compute_kp_offset_loss(
        ...
        (
            ...
            batch_offsets,
            batch_weights,
        ) = assigner.assign_keypoints_offset_targets(
            height=input_height,
            width=input_width,
            gt_keypoints_list=gt_keypoints_list,
            gt_weights_list=gt_weights_list,
    ...
    def _compute_kp_regression_loss(
        self,
        input_height,
        input_width,
        task_name,
        ...
        (
            ...
            batch_weights,
        ) = assigner.assign_joint_regression_targets(
            height=input_height,
            width=input_width,
            gt_keypoints_list=gt_keypoints_list,
            gt_classes_list=gt_classes_list,
    ...
    def _compute_kp_depth_loss(
        ...
        (
            ...
            batch_depths,
            batch_weights,
        ) = assigner.assign_keypoints_depth_targets(
            height=input_height,
            width=input_width,
            gt_keypoints_list=gt_keypoints_list,
            gt_weights_list=gt_weights_list,
    ...
    def _compute_mask_loss(self, segmentation_predictions, per_pixel_weights):
        ...
        total_pixels_in_loss = tf.math.maximum(tf.reduce_sum(per_pixel_weights), 1)
    ...
    def _compute_densepose_part_and_coordinate_losses(
        self, input_height, input_width, part_predictions, surface_coord_predictions
    ):
        ...
        for part_pred, surface_coord_pred in zip(
            part_predictions, surface_coord_predictions
        ):
    ...
    def _compute_track_embedding_loss(
        self, input_height, input_width, object_reid_predictions
    ):
    ...
    def _compute_temporal_offset_loss(self, input_height, input_width, prediction_dict):
        ...
        (
            ...
            batch_offset_targets,
            batch_weights,
        ) = assigner.assign_temporal_offset_targets(
            height=input_height,
            width=input_width,
            gt_boxes_list=gt_boxes_list,
    ...
    def _rescore_instances(self, classes, scores, keypoint_scores):
        ...
        classes_tiled = tf.tile(
            classes[:, :, tf.newaxis], multiples=[1, 1, total_num_keypoints]
        )
        ...
        for kp_params in self._kp_params_dict.values():
            ...
            kpt_mask_tiled = tf.tile(
                kpt_mask[tf.newaxis, tf.newaxis, :],
                multiples=[batch, max_detections, 1],
            )
            class_and_keypoint_mask = tf.math.logical_and(
                classes_tiled == class_id, kpt_mask_tiled == 1.0
            )
            class_and_keypoint_mask_float = tf.cast(
                class_and_keypoint_mask, dtype=tf.float32
            )
            ...
            num_visible_keypoints = tf.reduce_sum(
                class_and_keypoint_mask_float * tf.cast(visible_keypoints, tf.float32),
                axis=-1,
            )
            num_visible_keypoints = tf.math.maximum(num_visible_keypoints, 1.0)
            scores_for_class = (1.0 / num_visible_keypoints) * (
                tf.reduce_sum(
            ...
            scores = tf.where(classes == class_id, scores_for_class, scores)
        return scores
    ...
    def loss(
        ...
        output_height, output_width = (
            tf.maximum(input_height // self._stride, 1),
        ...
        losses = {
            OBJECT_CENTER: self._center_params.object_center_loss_weight
            * object_center_loss
        }
        if self._od_params is not None:
        ...
        if self._mask_params is not None:
            seg_losses = self._compute_segmentation_losses(
                prediction_dict=prediction_dict, per_pixel_weights=valid_anchor_weights
            )
        ...
        return dict(
            [("%s/%s" % (LOSS_KEY_PREFIX, key), val) for key, val in losses.items()]
        )
    ...
    def postprocess(self, prediction_dict, true_image_shapes, **params):
        ...
        if true_image_shapes is None:
            ...
            true_image_shapes = tf.stack(
                [
                    strided_height * self._stride,
                    strided_width * self._stride,
                    tf.constant(len(self._feature_extractor._channel_means)),
                ]
        ...
        boxes_strided = None
        if self._od_params:
            boxes_strided = prediction_tensors_to_boxes(
                y_indices,
                x_indices,
        ...
        if self._kp_params_dict:
            ...
            if len(self._kp_params_dict) == 1 and self._num_classes == 1:
                ...
                if kp_params.argmax_postprocessing:
                    keypoints, keypoint_scores = prediction_to_keypoints_argmax(
                        prediction_dict,
                    ...
                    (
                        ...
                        keypoint_scores,
                        keypoint_depths,
                    ) = self._postprocess_keypoints_single_class(
                        prediction_dict,
                        channel_indices,
                        y_indices,
                        x_indices,
                ...
                if keypoint_depths is not None:
                    postprocess_dict.update(
                        {
                            fields.DetectionResultFields.detection_keypoint_depths: keypoint_depths
                        }
                    )
            else:
        ...
        if self._mask_params:
            ...
            (
                ...
                densepose_surface_coords,
                stride=self._stride,
                mask_height=self._mask_params.mask_height,
        ...
        if self._track_params:
            embeddings = self._postprocess_embeddings(
                prediction_dict, y_indices, x_indices
            )
            postprocess_dict.update(
        ...
        if self._temporal_offset_params:
            offsets = prediction_tensors_to_temporal_offsets(
                y_indices, x_indices, prediction_dict[TEMPORAL_OFFSET][-1]
            )
        ...
        if self._non_max_suppression_fn:
            ...
            postprocess_dict[fields.DetectionResultFields.detection_boxes] = nmsed_boxes
            postprocess_dict[
    ...
    def postprocess_single_instance_keypoints(self, prediction_dict, true_image_shapes):
        ...
        keypoint_depths = None
        if kp_params.predict_depth:
            keypoint_depths = prediction_dict[
                get_keypoint_name(task_name, KEYPOINT_DEPTH)
            ][-1]
        (
            keypoints,
    ...
    def _scatter_keypoints_to_batch(
        self,
        num_ind,
        ...
        def left_fn(
            kpt_coords_for_example, kpt_scores_for_example, instance_inds_for_example
        ):
        ...
        def right_fn():
            ...
            return (kpt_coords_for_example_all_det, kpt_scores_for_example_all_det)

        left_fn = functools.partial(
            left_fn,
            kpt_coords_for_example,
            kpt_scores_for_example,
            instance_inds_for_example,
    ...
    def _postprocess_keypoints_multi_class(
        self, prediction_dict, classes, y_indices, x_indices, boxes, num_detections
    ):
        ...
        """
        total_num_keypoints = sum(
            len(kp_dict.keypoint_indices) for kp_dict in self._kp_params_dict.values()
        )
        ...
        for ex_ind in range(batch_size):
            ...
            for task_name, kp_params in self._kp_params_dict.items():
                ...
                kpt_coords_for_example_all_det = tf.tensor_scatter_nd_add(
                    kpt_coords_for_example_all_det, combined_inds, kpt_coords_for_class
                )
                kpt_scores_for_example_all_det = tf.tensor_scatter_nd_add(
                    kpt_scores_for_example_all_det, combined_inds, kpt_scores_for_class
                )
    ...
    def _postprocess_keypoints_single_class(
        self, prediction_dict, classes, y_indices, x_indices, boxes, num_detections
    ):
        ...
        if kp_params.predict_depth:
            keypoint_depth_predictions = prediction_dict[
                get_keypoint_name(task_name, KEYPOINT_DEPTH)
            ][-1]
    ...
    def _postprocess_keypoints_for_class_and_image(
        ...
        if keypoint_depth_predictions is not None:
            keypoint_depths = keypoint_depth_predictions[
    ...
    def restore_from_objects(self, fine_tune_checkpoint_type="detection"):
        ...
        if fine_tune_checkpoint_type == "detection":
            ...
            return {
                "feature_extractor": self._feature_extractor.classification_backbone
            }
            ...
            raise ValueError(
                "Unknown fine tune checkpoint type {}".format(fine_tune_checkpoint_type)
            )
