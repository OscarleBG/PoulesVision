...
class SsdMetaArchTest(
    ssd_meta_arch_test_lib.SSDMetaArchTestBase, parameterized.TestCase
):
    def _create_model(
        self,
    ...
    def test_predict_result_shapes_on_image_with_dynamic_shape(self):
        ...
        self.assertAllEqual(
            box_encodings_out.shape, (batch_size, num_anchors, code_size)
        )
        self.assertAllEqual(final_anchors.shape, (batch_size, num_anchors, code_size))
        self.assertAllEqual(
    ...
    def test_predict_with_raw_output_fields(self):
        ...
        ) = self.execute(graph_fn, [input_image], graph=g)
        self.assertAllEqual(box_encodings.shape, expected_box_encodings_shape)
        self.assertAllEqual(class_predictions.shape, expected_class_predictions_shape)
        self.assertAllEqual(final_anchors.shape, final_anchors_shape)
        self.assertAllEqual(
            raw_detection_boxes.shape, expected_raw_detection_boxes_shape
        )
        self.assertAllEqual(
            raw_detection_feature_map_indices, np.zeros((batch_size, num_anchors))
        )
    ...
    def test_postprocess_results_are_correct_static(self):
        with test_utils.GraphContextOrNone() as g:
            model, _, _, _ = self._create_model(
                use_static_shapes=True, nms_max_size_per_class=4
            )
        ...
        expected_boxes = [
            ...
            [[0, 0, 0.5, 0.5], [0, 0.5, 0.5, 1], [0.5, 0, 1, 0.5], [0, 0, 0, 0]],
        ]  # padding
    ...
    def test_postprocess_results_are_correct_with_calibration(self):
        ...
        detection_scores, raw_detection_scores = self.execute_cpu(graph_fn, [], graph=g)
        self.assertAllClose(detection_scores, expected_scores)
        self.assertAllEqual(raw_detection_scores, expected_raw_detection_scores)
    ...
    def test_loss_results_are_correct(self):
        ...
        def graph_fn(
            ...
            return (
                self._get_value_for_matching_key(loss_dict, "Loss/localization_loss"),
                self._get_value_for_matching_key(loss_dict, "Loss/classification_loss"),
            )
    ...
    def test_loss_results_are_correct_with_normalize_by_codesize_true(self):
        ...
        localization_loss = self.execute(
            graph_fn,
            [
                preprocessed_input,
                groundtruth_boxes1,
    ...
    def test_loss_results_are_correct_with_losses_mask(self):
        ...
        # not annotated.
        expected_classification_loss = (
            (batch_size - 1) * num_anchors * (num_classes + 1) * np.log(2.0)
        )
        (localization_loss, classification_loss) = self.execute(
            graph_fn,
            [
    ...
    def test_restore_map_for_detection_ckpt(self):
        ...
        model.predict(
            tf.constant(
                np.array([[[[0, 0], [1, 1]], [[1, 0], [0, 1]]]], dtype=np.float32)
            ),
    ...
    def test_load_checkpoint_vars_tf2(self):
        ...
        detection_var_names = sorted(
            [
                var.name
                for var in model.restore_from_objects("detection")[
                    "model"
                ]._feature_extractor.weights
            ]
        )
        ...
        full_var_names = sorted(
            [var.name for var in model.restore_from_objects("full")["model"].weights]
        )
    ...
    def test_loss_results_are_correct_with_random_example_sampling(self):
        with test_utils.GraphContextOrNone() as g:
            model, num_classes, _, _ = self._create_model(random_example_sampling=True)
