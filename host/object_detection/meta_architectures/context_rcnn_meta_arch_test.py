...
class ContextRCNNMetaArchTest(test_case.TestCase, parameterized.TestCase):
    ...
    def test_prediction_mock_tf1(self, mock_context_rcnn_lib_v1):
        ...
        mock_context_rcnn_lib_v1._compute_box_context_attention.return_value = (
            mock_tensor
        )
        ...
        context_features = tf.random_uniform(
            (2, 20, 10), minval=0, maxval=255, dtype=tf.float32
        )
        valid_context_size = tf.random_uniform(
            (2,), minval=0, maxval=10, dtype=tf.int32
        )
        features = {
