...
def filter_weight_value(weights, values, valid_mask):
    ...
    very_negative_mask = (
        tf.ones(weights.shape, dtype=weights.dtype) * _NEGATIVE_PADDING_VALUE
    )
    valid_weight_mask = tf.tile(
        tf.transpose(valid_mask, perm=[0, 2, 1]), [1, weights.shape[1], 1]
    )
    weights = tf.where(valid_weight_mask, x=weights, y=very_negative_mask)
...
def project_features(features, projection_dimension, is_training, normalize):
    ...
    projected_features = tf.reshape(
        projected_features, [batch_size, -1, projection_dimension]
    )
...
def attention_block(
    ...
    with tf.variable_scope(block_name):
        ...
        weights = tf.identity(
            tf.nn.softmax(weights / attention_temperature),
...
def _compute_box_context_attention(
    box_features,
    num_proposals,
    context_features,
    valid_context_size,
    ...
    box_features = tf.reshape(
        box_features, [batch_size, max_num_proposals, height, width, channels]
    )
