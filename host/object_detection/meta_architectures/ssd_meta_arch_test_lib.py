            ...
            box_list.BoxList(
                tf.constant(
                    [
                        ...
                        [0, 0.5, 0.5, 1],
                        [0.5, 0, 1, 0.5],
                        [1.0, 1.0, 1.5, 1.5],  # Anchor that is outside clip_window.
                    ],
                    tf.float32,
                )
            )
    ...
    def _create_model(
      ...
      function_approximation {
        x_y_pairs {
            ...
            score_conversion_fn = post_processing_builder._build_calibrated_score_converter(  # pylint: disable=protected-access
        ...
        if predict_mask:
            kwargs.update(
                {
                    "mask_prediction_fn": test_utils.MockMaskHead(
                        num_classes=1
                    ).predict,
                }
            )
