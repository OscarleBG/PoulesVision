...
class SSDMetaArch(model.DetectionModel):
    ...
    @deprecated_args(
        None,
        "NMS is always placed on TPU; do not use nms_on_host " "as it has no effect.",
        "nms_on_host",
    )
    def __init__(
        self,
        ...
        if add_background_class and explicit_background_class:
            raise ValueError(
                "Cannot have both 'add_background_class' and"
        ...
        if self._feature_extractor.is_keras_model:
            ...
            with slim.arg_scope(
                ...
                with tf.variable_scope(
                    None, self._extract_features_scope, [preprocessed_inputs]
                ):
    ...
    def _raw_detections_and_feature_map_inds(self, box_encodings, boxlist_list):
        ...
        feature_map_indices = self._anchor_generator.anchor_index_to_feature_map_index(
            boxlist_list
        )
        feature_map_indices_batched = tf.tile(
            tf.expand_dims(feature_map_indices, 0), multiples=[batch_size, 1]
        )
        return {
    ...
    def postprocess(self, prediction_dict, true_image_shapes):
        ...
        with tf.name_scope("Postprocessor"):
            ...
            if self._anchors is not None:
                num_boxes = (
                    self._anchors.num_boxes_static() or self._anchors.num_boxes()
                )
    ...
    def loss(self, prediction_dict, true_image_shapes, scope=None):
        ...
        with tf.name_scope(scope, "Loss", prediction_dict.values()):
            ...
            if self._random_example_sampler:
                ...
                batch_reg_weights = tf.multiply(
                    batch_sampled_indicator, batch_reg_weights
                )
                batch_cls_weights = tf.multiply(
                    tf.expand_dims(batch_sampled_indicator, -1), batch_cls_weights
                )
            ...
            if self._expected_loss_weights_fn:
                ...
                unmatched_targets = (
                    tf.ones([batch_size, num_anchors, 1]) * self._unmatched_class_label
                )
                ...
                if cls_losses.get_shape().ndims == 3:
                    ...
                    unmatched_cls_losses = tf.reshape(
                        unmatched_cls_losses, [batch_size, -1]
                    )
                ...
                foreground_weights, background_weights = self._expected_loss_weights_fn(
                    batch_cls_targets
                )
            ...
            if self._normalize_loss_by_num_matches:
                normalizer = tf.maximum(
                    tf.cast(tf.reduce_sum(batch_reg_weights), dtype=tf.float32), 1.0
                )
            ...
            localization_loss = tf.multiply(
                (self._localization_loss_weight / localization_loss_normalizer),
                localization_loss,
    ...
    def _summarize_anchor_classification_loss(self, class_ids, cls_losses):
        ...
        visualization_utils.add_cdf_image_summary(
            positive_anchor_cls_loss, "PositiveAnchorLossCDF"
        )
        ...
        visualization_utils.add_cdf_image_summary(
            negative_anchor_cls_loss, "NegativeAnchorLossCDF"
        )
    ...
    def _summarize_target_assignment(self, groundtruth_boxes_list, match_list):
        ...
        tf.summary.scalar(
            "AvgNumPositiveAnchorsPerImage", avg_pos_anchors, family="TargetAssignment"
        )
        tf.summary.scalar(
            "AvgNumNegativeAnchorsPerImage", avg_neg_anchors, family="TargetAssignment"
        )
        tf.summary.scalar(
            "AvgNumIgnoredAnchorsPerImage",
            avg_ignored_anchors,
            family="TargetAssignment",
        )

    def _apply_hard_mining(
        self, location_losses, cls_losses, prediction_dict, match_list
    ):
        ...
        if "anchors" not in prediction_dict:
            prediction_dict["anchors"] = self.anchors.get()
        decoded_boxes, _ = self._batch_decode(
            prediction_dict["box_encodings"], prediction_dict["anchors"]
        )
        decoded_box_tensors_list = tf.unstack(decoded_boxes)
        class_prediction_list = tf.unstack(class_predictions)
        decoded_boxlist_list = []
        for box_location, box_score in zip(
            decoded_box_tensors_list, class_prediction_list
        ):
    ...
    def restore_from_objects(self, fine_tune_checkpoint_type="detection"):
        ...
        if fine_tune_checkpoint_type == "classification":
            ...
            fake_model = tf.train.Checkpoint(_feature_extractor=self._feature_extractor)
    ...
    def updates(self):
        ...
        if self._box_predictor.is_keras_model:
            ...
            update_ops.extend(
                self._box_predictor.get_updates_for(self._box_predictor.inputs)
            )
        if self._feature_extractor.is_keras_model:
            update_ops.extend(self._feature_extractor.get_updates_for(None))
            update_ops.extend(
                self._feature_extractor.get_updates_for(self._feature_extractor.inputs)
            )
        return update_ops
