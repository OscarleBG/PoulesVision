...
def _prepare_groundtruth_for_eval(detection_model, class_agnostic, max_number_of_boxes):
    ...
    if detection_model.groundtruth_has_field(
        ...
        groundtruth[input_data_fields.groundtruth_image_classes] = (
            groundtruth_image_classes + label_id_offset
        )
    ...
    if detection_model.groundtruth_has_field(fields.BoxListFields.keypoint_depths):
        ...
        groundtruth[input_data_fields.groundtruth_keypoint_depth_weights] = tf.stack(
            detection_model.groundtruth_lists(
    ...
    if detection_model.groundtruth_has_field(fields.BoxListFields.densepose_part_ids):
        groundtruth[input_data_fields.groundtruth_dp_part_ids] = tf.stack(
            detection_model.groundtruth_lists(fields.BoxListFields.densepose_part_ids)
        )
    if detection_model.groundtruth_has_field(
...
def provide_groundtruth(model, labels, training_step=None):
    ...
    if fields.InputDataFields.groundtruth_keypoint_depths in labels:
        gt_keypoint_depths_list = labels[
            fields.InputDataFields.groundtruth_keypoint_depths
        ]
        gt_keypoint_depth_weights_list = labels[
            fields.InputDataFields.groundtruth_keypoint_depth_weights
        ]
...
def create_model_fn(
    detection_model_fn, configs, hparams=None, use_tpu=False, postprocess_on_cpu=False
):
    ...
    def model_fn(features, labels, mode, params=None):
        ...
        if mode == tf_estimator.ModeKeys.PREDICT:
            ...
            export_outputs = {
                tf.saved_model.signature_constants.PREDICT_METHOD_NAME: tf_estimator.export.PredictOutput(
                    exported_output
                )
            }
...
def create_train_and_eval_specs(
    ...
    """
    train_spec = tf_estimator.TrainSpec(input_fn=train_input_fn, max_steps=train_steps)
...
def _evaluate_checkpoint(estimator, input_fn, checkpoint_path, name, max_retries=0):
...
def continuous_eval_generator(
    estimator, model_dir, input_fn, train_steps, name, max_retries=0
):
