...
if int(major) < 1 or (int(major == 1) and int(minor) < 14):
    raise RuntimeError(
        "TensorFlow version >= 1.14 is required. Found ({}).".format(tf.__version__)
...
def get_prediction_tensor_shapes(pipeline_config):
    ...
    return {
        BOX_ENCODINGS: prediction_dict[BOX_ENCODINGS].shape.as_list(),
...
def build_graph(
    ...
    (
        ...
        class_predictions_with_background,
        anchors,
    ) = recover_shape(preprocessed_inputs, prediction_outputs, shapes_info)
