...
def export(
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        tensor_info_inputs = {
            input_placeholder_name: tf.saved_model.utils.build_tensor_info(
                placeholder_tensor
            )
        }
        tensor_info_outputs = {
        ...
        builder.add_meta_graph_and_variables(
            sess,
            [tf.saved_model.tag_constants.SERVING, tf.saved_model.tag_constants.TPU],
            signature_def_map={
...
def run_inference_from_saved_model(
    inputs, saved_model_dir, input_placeholder_name="placeholder_tensor", repeat=1
):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        meta_graph = loader.load(
            sess, [tag_constants.SERVING, tag_constants.TPU], saved_model_dir
        )
