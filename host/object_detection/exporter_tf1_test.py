...
class FakeModel(model.DetectionModel):
    def __init__(
        self,
        add_detection_keypoints=False,
        add_detection_masks=False,
        ...
        with tf.control_dependencies(list(prediction_dict.values())):
            postprocessed_tensors = {
                ...
                "detection_multiclass_scores": tf.constant(
                    [[[0.3, 0.7], [0.4, 0.6]], [[0.1, 0.9], [0.0, 0.0]]], tf.float32
                ),
                "detection_classes": tf.constant([[0, 1], [1, 0]], tf.float32),
                "num_detections": tf.constant([2, 1], tf.float32),
                "raw_detection_boxes": tf.constant(
                    [
                        [[0.0, 0.0, 0.5, 0.5], [0.5, 0.5, 0.8, 0.8]],
                        [[0.5, 0.5, 1.0, 1.0], [0.0, 0.5, 0.0, 0.5]],
                    ],
                    tf.float32,
                ),
                "raw_detection_scores": tf.constant(
                    [[0.7, 0.6], [0.9, 0.5]], tf.float32
                ),
            }
...
class ExportInferenceGraphTest(tf.test.TestCase):
    def _save_checkpoint_from_mock_model(
        self, checkpoint_path, use_moving_averages, enable_quantization=False
    ):
        g = tf.Graph()
        with g.as_default():
    ...
    def test_write_inference_graph(self):
        ...
        with mock.patch.object(model_builder, "build", autospec=True) as mock_builder:
            ...
            self.assertTrue(
                os.path.exists(os.path.join(output_directory, "inference_graph.pbtxt"))
            )
    ...
    def test_export_and_run_inference_with_encoded_image_string_tensor(self):
        ...
        with self.test_session(graph=inference_graph) as sess:
            ...
            for image_str in [jpg_image_str, png_image_str]:
                ...
                self.assertAllClose(
                    multiclass_scores_np,
                    [[[0.3, 0.7], [0.4, 0.6]], [[0.1, 0.9], [0.0, 0.0]]],
                )
                self.assertAllClose(classes_np, [[1, 2], [2, 1]])
                self.assertAllClose(keypoints_np, np.arange(48).reshape([2, 2, 6, 2]))
                self.assertAllClose(masks_np, np.arange(64).reshape([2, 2, 4, 4]))
    ...
    def test_raise_runtime_error_on_images_with_different_sizes(self):
        ...
        with self.test_session(graph=inference_graph) as sess:
            ...
            with self.assertRaisesRegexp(
                tf.errors.InvalidArgumentError, "TensorArray.*shape"
            ):
                sess.run(
                    [boxes, scores, classes, keypoints, masks, num_detections],
                    feed_dict={image_str_tensor: image_str_batch_np},
    ...
    def test_rewrite_nn_resize_op_multiple_path(self):
        ...
        graph_def = strip_unused_lib.strip_unused(
            graph_def,
            input_node_names=[
                "nearest_upsampling/Placeholder",
                "nearest_upsampling_1/Placeholder",
            ],
