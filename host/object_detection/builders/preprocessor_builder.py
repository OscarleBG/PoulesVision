...
PREPROCESSING_FUNCTION_MAP = {
    ...
    "adjust_gamma": preprocessor.adjust_gamma,
}
...
def build(preprocessor_step_config):
    ...
    if step_type == "random_rotation90":
        ...
        return (
            ...
            {
                "keypoint_rot_permutation": tuple(config.keypoint_rot_permutation)
                or None,
                "probability": config.probability or None,
            },
        )
    ...
    if step_type == "remap_labels":
        ...
        return (
            preprocessor.remap_labels,
            {"original_labels": config.original_labels, "new_label": config.new_label},
        )
    ...
    if step_type == "ssd_random_crop_pad":
        ...
        if config.operations:
            ...
            min_padded_size_ratio = [
                tuple(op.min_padded_size_ratio) for op in config.operations
            ]
            max_padded_size_ratio = [
                tuple(op.max_padded_size_ratio) for op in config.operations
            ]
            pad_color = [
    ...
    if step_type == "ssd_random_crop_pad_fixed_aspect_ratio":
        ...
        if max_padded_size_ratio:
            ...
            kwargs["max_padded_size_ratio"] = tuple(max_padded_size_ratio)
        if config.operations:
            kwargs["min_object_covered"] = [
                op.min_object_covered for op in config.operations
            ]
            kwargs["aspect_ratio_range"] = [
                (op.min_aspect_ratio, op.max_aspect_ratio) for op in config.operations
            ]
            kwargs["area_range"] = [
                (op.min_area, op.max_area) for op in config.operations
            ]
