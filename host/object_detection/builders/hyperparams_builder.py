...
class KerasLayerHyperparams(object):
    ...
    def __init__(self, hyperparams_config):
        ...
        if not isinstance(hyperparams_config, hyperparams_pb2.Hyperparams):
            raise ValueError(
                "hyperparams_config not of type " "hyperparams_pb.Hyperparams."
            )
...
def build(hyperparams_config, is_training):
    ...
    if hyperparams_config.force_use_bias:
        raise ValueError(
            "Hyperparams force_use_bias only supported by " "KerasLayerHyperparams."
        )

    if hyperparams_config.HasField("sync_batch_norm"):
        raise ValueError(
            "Hyperparams sync_batch_norm only supported by " "KerasLayerHyperparams."
        )
...
def _build_initializer(initializer, build_for_keras=False):
    ...
    if initializer_oneof == "variance_scaling_initializer":
        enum_descriptor = (
            hyperparams_pb2.VarianceScalingInitializer.DESCRIPTOR.enum_types_by_name[
                "Mode"
            ]
        )
        mode = enum_descriptor.values_by_number[
            initializer.variance_scaling_initializer.mode
        ].name
        if build_for_keras:
            if initializer.variance_scaling_initializer.uniform:
                ...
                try:
                    ...
                    truncated_scale = (
                        initializer.variance_scaling_initializer.factor
                        / (truncate_constant * truncate_constant)
                    )
                    return tf.variance_scaling_initializer(
                        scale=truncated_scale, mode=mode.lower(), distribution="normal"
                    )
