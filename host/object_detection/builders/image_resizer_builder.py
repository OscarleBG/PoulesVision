...
def _tf_resize_method(resize_method):
    ...
    """
    dict_method = {
        image_resizer_pb2.BILINEAR: tf.image.ResizeMethod.BILINEAR,
...
def build(image_resizer_config):
    ...
    if not isinstance(image_resizer_config, image_resizer_pb2.ImageResizer):
        raise ValueError(
            "image_resizer_config not of type " "image_resizer_pb2.ImageResizer."
        )
    ...
    if image_resizer_oneof == "keep_aspect_ratio_resizer":
        ...
        if not (
            keep_aspect_ratio_config.min_dimension
        ...
        if keep_aspect_ratio_config.per_channel_pad_value:
            per_channel_pad_value = tuple(
                keep_aspect_ratio_config.per_channel_pad_value
            )
        ...
        pad_to_multiple_resizer_config = image_resizer_config.pad_to_multiple_resizer
