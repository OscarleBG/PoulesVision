...
class CalibrationBuilderTest(test_case.TestCase):
    ...
    @staticmethod
    def _add_function_approximation_to_calibration_proto(
        calibration_proto, x_array, y_array, class_id
    ):
        """Adds a function approximation to calibration proto for a class id."""
        # Per-class calibration.
        if class_id is not None:
            function_approximation = calibration_proto.class_id_function_approximations.class_id_xy_pairs_map[
                class_id
            ]
    ...
    def test_multiclass_function_approximations(self):
        ...
        def graph_fn():
            ...
            # batch_size = 2, num_classes = 2, num_anchors = 2.
            class_predictions_with_background = tf.constant(
                [[[0.1, 0.2], [0.9, 0.1]], [[0.6, 0.4], [0.08, 0.92]]], dtype=tf.float32
            )
            calibrated_scores = calibration_fn(class_predictions_with_background)
            return calibrated_scores
        ...
        calibrated_scores_np = self.execute(graph_fn, [])
        self.assertAllClose(
            calibrated_scores_np, [[[0.5, 0.6], [0.5, 0.3]], [[0.5, 0.7], [0.5, 0.96]]]
        )
    ...
    def test_skips_class_when_calibration_parameters_not_present(self):
        ...
        calibrated_scores_np = self.execute(graph_fn, [])
        self.assertAllClose(
            calibrated_scores_np, [[[0.5, 0.2], [0.5, 0.1]], [[0.5, 0.4], [0.5, 0.92]]]
        )
