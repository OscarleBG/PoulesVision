...
def build_convolutional_keras_box_predictor(
    is_training,
    num_classes,
    conv_hyperparams,
    freeze_batchnorm,
...
def build_weight_shared_convolutional_keras_box_predictor(
    ...
    return convolutional_keras_box_predictor.WeightSharedConvolutionalBoxPredictor(
        is_training=is_training,
        num_classes=num_classes,
        box_prediction_head=box_prediction_head,
        class_prediction_head=class_prediction_head,
...
def build_mask_rcnn_keras_box_predictor(
    is_training,
    num_classes,
    fc_hyperparams,
    freeze_batchnorm,
    ...
    if predict_instance_masks:
        ...
        ] = keras_mask_head.MaskRCNNMaskHead(
            is_training=is_training,
            num_classes=num_classes,
            conv_hyperparams=conv_hyperparams,
            freeze_batchnorm=freeze_batchnorm,
...
def build(
    ...
    if not isinstance(box_predictor_config, box_predictor_pb2.BoxPredictor):
        raise ValueError(
            "box_predictor_config not of type " "box_predictor_pb2.BoxPredictor."
        )
    ...
    if box_predictor_oneof == "mask_rcnn_box_predictor":
        ...
        fc_hyperparams_fn = argscope_fn(
            config_box_predictor.fc_hyperparams, is_training
        )
        conv_hyperparams_fn = None
