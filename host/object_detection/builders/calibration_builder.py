...
def _find_interval_containing_new_value(x, new_value):
    ...
    compare = tf.cast(
        tf.reshape(new_value, shape=(new_value_shape, 1))
...
def _tf_linear_interp1d(x_to_interpolate, fn_x, fn_y):
    ...
    # Interpolate
    alpha = (x_to_interpolate - tf.gather(x_pad, interval_idx)) / (
        tf.gather(x_pad, interval_idx + 1) - tf.gather(x_pad, interval_idx)
    )
    interpolation = (1 - alpha) * tf.gather(y_pad, interval_idx) + alpha * tf.gather(
        y_pad, interval_idx + 1
    )
...
def _function_approximation_proto_to_tf_tensors(x_y_pairs_message):
    ...
    """
    tf_x = tf.convert_to_tensor(
        [x_y_pair.x for x_y_pair in x_y_pairs_message.x_y_pair], dtype=tf.float32
    )
    tf_y = tf.convert_to_tensor(
        [x_y_pair.y for x_y_pair in x_y_pairs_message.x_y_pair], dtype=tf.float32
    )
    return tf_x, tf_y
...
def _get_class_id_function_dict(calibration_config):
    ...
    for class_id in class_id_xy_pairs_map:
        class_id_function_dict[class_id] = _function_approximation_proto_to_tf_tensors(
...
def build(calibration_config):
    ...
    if calibration_config.WhichOneof("calibrator") == "function_approximation":
        ...
        def calibration_fn(class_predictions_with_background):
            ...
            calibrated_class_predictions_with_background = tf.reshape(
                updated_scores, shape=original_detections_shape, name="calibrate_scores"
            )
            return calibrated_class_predictions_with_background

    elif (
        calibration_config.WhichOneof("calibrator")
        ...
        def calibration_fn(class_predictions_with_background):
            ...
            calibrated_class_predictions_with_background = tf.reshape(
                combined_calibrated_tensor, shape=input_shape, name="calibrate_scores"
            )
            return calibrated_class_predictions_with_background

    elif (
        calibration_config.WhichOneof("calibrator") == "temperature_scaling_calibration"
    ):
        ...
        def calibration_fn(class_predictions_with_background):
            ...
            calibrated_class_predictions_with_background = tf.math.divide(
                class_predictions_with_background, scaler, name="calibrate_score"
            )
            return calibrated_class_predictions_with_background
    ...
    # TODO(zbeaver): Add sigmoid calibration.
        ...
        raise ValueError(
            'No calibration builder defined for "Oneof" in ' "calibration_config."
        )
