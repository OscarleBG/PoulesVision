...
class HyperparamsBuilderTest(tf.test.TestCase):
    ...
    def _assert_variance_in_range(self, initializer, shape, variance, tol=1e-2):
        with tf.Graph().as_default() as g:
            with self.test_session(graph=g) as sess:
                var = tf.get_variable(
                    name="test", shape=shape, dtype=tf.float32, initializer=initializer
                )
    ...
    """
        ...
        self._assert_variance_in_range(
            initializer, shape=[100, 40], variance=2.0 / 100.0
        )
    ...
    """
        ...
        initializer = conv_scope_arguments["weights_initializer"]
        self._assert_variance_in_range(
            initializer, shape=[100, 40], variance=2.0 / 40.0
        )
    ...
    """
        ...
        initializer = conv_scope_arguments["weights_initializer"]
        self._assert_variance_in_range(
            initializer, shape=[100, 40], variance=4.0 / (100.0 + 40.0)
        )
    ...
    """
        ...
        initializer = conv_scope_arguments["weights_initializer"]
        self._assert_variance_in_range(
            initializer, shape=[100, 40], variance=0.64, tol=1e-1
        )
...
class KerasHyperparamsBuilderTest(tf.test.TestCase):
    ...
    """
        ...
        self.assertIsInstance(batch_norm_layer, freezable_batch_norm.FreezableBatchNorm)
    ...
    """
        ...
        self.assertIsNone(keras_config.params(include_activation=True)["activation"])
