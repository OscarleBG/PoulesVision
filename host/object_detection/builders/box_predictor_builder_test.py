...
class ConvolutionalBoxPredictorBuilderTest(tf.test.TestCase):
    ...
    """
        ...
        self.assertAlmostEqual(
            ...
            (conv_hyperparams_actual.initializer.truncated_normal_initializer.stddev),
        )
        self.assertAlmostEqual(
            (hyperparams_proto.initializer.truncated_normal_initializer.mean),
            (conv_hyperparams_actual.initializer.truncated_normal_initializer.mean),
        )
        self.assertEqual(
            hyperparams_proto.activation, conv_hyperparams_actual.activation
        )
...
class WeightSharedConvolutionalBoxPredictorBuilderTest(tf.test.TestCase):
    ...
    """
        ...
        box_predictor_proto = box_predictor_pb2.BoxPredictor()
        (
            box_predictor_proto.weight_shared_convolutional_box_predictor.conv_hyperparams.CopyFrom(
                hyperparams_proto
            )
        )
        box_predictor = box_predictor_builder.build(
...
class MaskRCNNBoxPredictorBuilderTest(tf.test.TestCase):
    ...
    """
        ...
        self.assertEqual(
            box_predictor._box_prediction_head._fc_hyperparams_fn, "arg_scope"
        )
        self.assertEqual(
            box_predictor._class_prediction_head._fc_hyperparams_fn, "arg_scope"
        )
    ...
    def test_build_box_predictor_with_mask_branch(self):
        ...
        mock_argscope_fn.assert_has_calls(
            [
                mock.call(
                    box_predictor_proto.mask_rcnn_box_predictor.fc_hyperparams, True
                ),
                mock.call(
                    box_predictor_proto.mask_rcnn_box_predictor.conv_hyperparams, True
                ),
            ],
            any_order=True,
        )
        ...
        self.assertTrue(box_predictor._is_training)
        self.assertEqual(box_head._box_code_size, 4)
        self.assertIn(mask_rcnn_box_predictor.MASK_PREDICTIONS, third_stage_heads)
        self.assertEqual(
            third_stage_heads[
                mask_rcnn_box_predictor.MASK_PREDICTIONS
