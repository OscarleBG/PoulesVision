...
def _read_dataset_internal(
    file_read_func, input_files, num_readers, config, filename_shard_fn=None
):
...
def read_dataset(file_read_func, input_files, config, filename_shard_fn=None):
    ...
    if config.sample_from_datasets_weights:
        ...
        if len(input_files) != len(config.sample_from_datasets_weights):
            raise ValueError(
                "Expected the number of input files to be the same as "
        ...
        for i, input_file in enumerate(input_files):
            ...
            num_readers = math.ceil(
                config.num_readers * weight / sum(config.sample_from_datasets_weights)
            )
            tf.logging.info("Num readers for dataset [%s]: %d", input_file, num_readers)
            if num_readers == 0:
                tf.logging.info("Skipping dataset due to zero weights: %s", input_file)
                continue
            tf.logging.info("Num readers for dataset [%s]: %d", input_file, num_readers)
            records_dataset = _read_dataset_internal(
                file_read_func, [input_file], num_readers, config, filename_shard_fn
            )
            dataset_weights.append(weight)
            records_datasets.append(records_dataset)
        return tf.data.experimental.sample_from_datasets(
            records_datasets, dataset_weights
        )
    else:
        tf.logging.info("Reading unweighted datasets: %s" % input_files)
        return _read_dataset_internal(
            file_read_func, input_files, config.num_readers, config, filename_shard_fn
        )
...
def build(
    ...
    if input_reader_config.WhichOneof("input_reader") == "tf_record_input_reader":
        ...
        if not config.input_path:
            raise ValueError(
                "At least one input path must be specified in " "`input_reader_config`."
            )

        def dataset_map_fn(
            dataset, fn_to_map, batch_size=None, input_reader_config=None
        ):
        ...
        dataset = dataset_map_fn(
            dataset, decoder.decode, batch_size, input_reader_config
        )
        if reduce_to_frame_fn:
            dataset = reduce_to_frame_fn(
                dataset, dataset_map_fn, batch_size, input_reader_config
            )
        if transform_input_data_fn is not None:
            dataset = dataset_map_fn(
                dataset, transform_input_data_fn, batch_size, input_reader_config
            )
        if batch_size:
            dataset = dataset.batch(
                batch_size, drop_remainder=input_reader_config.drop_remainder
            )
