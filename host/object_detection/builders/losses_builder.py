...
def build(loss_config):
    ...
    if loss_config.HasField("hard_example_miner"):
        if (
            loss_config.classification_loss.WhichOneof("classification_loss")
            == "weighted_sigmoid_focal"
        ):
            raise ValueError(
                "HardExampleMiner should not be used with sigmoid focal " "loss"
            )
        hard_example_miner = build_hard_example_miner(
            loss_config.hard_example_miner, classification_weight, localization_weight
        )
    ...
    if loss_config.HasField("random_example_sampler"):
        ...
        random_example_sampler = sampler.BalancedPositiveNegativeSampler(
            positive_fraction=loss_config.random_example_sampler.positive_sample_fraction
        )
