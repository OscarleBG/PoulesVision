...
class SSDMobileNetV2FpnKerasFeatureExtractor(ssd_meta_arch.SSDKerasFeatureExtractor):
    ...
    def build(self, input_shape):
        ...
        self.classification_backbone = tf.keras.Model(
            inputs=full_mobilenet_v2.inputs, outputs=outputs
        )
    ...
    def _extract_features(self, preprocessed_inputs):
        ...
        for level in range(self._fpn_min_level, self._base_fpn_max_level + 1):
            feature_maps.append(
                fpn_features["top_down_{}".format(self._feature_blocks[level - 2])]
            )
        last_feature_map = fpn_features[
            "top_down_{}".format(self._feature_blocks[self._base_fpn_max_level - 2])
        ]
