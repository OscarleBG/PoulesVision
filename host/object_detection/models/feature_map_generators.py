...
class KerasFpnTopDownFeatureMaps(tf.keras.Model):
    ...
    def __init__(
        ...
        if use_bounded_activations:
            self.top_layers.append(
                tf.keras.layers.Lambda(clip_by_value, name="clip_by_value")
            )
        ...
        for level in reversed(list(range(num_levels - 1))):
            ...
            if use_explicit_padding:
                ...
                conv_net.append(
                    tf.keras.layers.Lambda(fixed_padding, name="fixed_padding")
                )
...
def fpn_top_down_feature_maps(
    ...
    with tf.name_scope(scope, "top_down"):
        ...
        with slim.arg_scope(
            ...
            if use_bounded_activations:
                top_down = tf.clip_by_value(
                    top_down, -ACTIVATION_BOUND, ACTIVATION_BOUND
                )
            output_feature_maps_list.append(top_down)
            output_feature_map_keys.append("top_down_%s" % image_features[-1][0])
            ...
            for level in reversed(list(range(num_levels - 1))):
                ...
                if use_bounded_activations:
                    residual = tf.clip_by_value(
                        residual, -ACTIVATION_BOUND, ACTIVATION_BOUND
                    )
                if use_explicit_padding:
...
def pooling_pyramid_feature_maps(
    base_feature_map_depth, num_layers, image_features, replace_pool_with_conv=False
):
    ...
    if replace_pool_with_conv:
        with slim.arg_scope([slim.conv2d], padding="SAME", stride=2):
            for i in range(num_layers - 1):
                feature_map_key = "Conv2d_{}_3x3_s2_{}".format(
                    i, base_feature_map_depth
                )
                feature_map = slim.conv2d(
