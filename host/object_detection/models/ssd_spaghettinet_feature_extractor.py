...
SepConvOp = collections.namedtuple(
    "SepConvOp", ["kernel_size", "stride", "has_residual"]
)
IbnFusedGrouped = collections.namedtuple(
    "IbnFusedGrouped",
    ["kernel_size", "expansion_rate", "stride", "groups", "has_residual"],
)
SpaghettiStemNode = collections.namedtuple(
    "SpaghettiStemNode", ["kernel_size", "num_filters"]
)
SpaghettiNode = collections.namedtuple(
    "SpaghettiNode", ["layers", "num_filters", "edges", "level"]
)
SpaghettiResampleEdge = collections.namedtuple("SpaghettiResampleEdge", ["input"])
SpaghettiPassthroughEdge = collections.namedtuple("SpaghettiPassthroughEdge", ["input"])
SpaghettiNodeSpecs = collections.namedtuple("SpaghettiNodeSpecs", ["nodes", "outputs"])
...
class SpaghettiNet:
    ...
    def _expanded_conv(
        self, net, num_filters, expansion_rates, kernel_size, stride, scope
    ):
    ...
    def _slice_shape_along_axis(self, shape, axis, groups):
        ...
        if isinstance(shape, tf.TensorShape):
            ...
            if shape_as_list[axis] % groups != 0:
                raise ValueError(
                    "Dimension {} must be divisible by {} groups".format(
    ...
    def _ibn_fused_grouped(
        self, net, num_filters, expansion_rates, kernel_size, stride, groups, scope
    ):
        ...
        net = self._quantizable_concat(
            slice_outputs, -1, self._is_training, scope="{}/{}".format(scope, "concat")
        )
        net = slim.conv2d(
            net,
            num_filters,
    ...
    def _upsample(self, net, num_filters, upsample_ratio, scope):
        ...
        if self._use_native_resize_op:
            ...
            node_up = ops.nearest_neighbor_upsampling(node_pre_up, scale=upsample_ratio)
    ...
    def _downsample(self, net, num_filters, downsample_ratio, scope):
        ...
        node_down = slim.max_pool2d(
            ops.fixed_padding(net, downsample_ratio + 1) if add_fixed_padding else net,
    ...
    def _spaghetti_node(self, node, scope):
        ...
        for edge in node_spec.edges:
            if isinstance(edge, SpaghettiPassthroughEdge):
                ...
                edge_outputs.append(
                    self._spaghetti_edge(
                        node, edge.input, "edge_{}_{}".format(edge.input, node)
                    )
                )
        ...
        for idx, layer_spec in enumerate(node_spec.layers):
            if isinstance(layer_spec, IbnOp):
                net_exp = self._expanded_conv(
                    net,
                    node_spec.num_filters,
                ...
                net_exp = self._ibn_fused_grouped(
                    net,
                    node_spec.num_filters,
                    layer_spec.expansion_rate,
                    layer_spec.kernel_size,
    ...
    def _spaghetti_edge(self, curr_node, prev_node, scope):
        ...
        if curr_spec.level < prev_spec.level:
            ...
            output = self._upsample(
                self._nodes[prev_node],
                curr_spec.num_filters,
                2 ** (prev_spec.level - curr_spec.level),
                scope,
            )
            ...
            output = self._downsample(
                self._nodes[prev_node],
                curr_spec.num_filters,
                2 ** (curr_spec.level - prev_spec.level),
                scope,
            )
        else:
            # 1x1
            output = self._no_resample(
                self._nodes[prev_node], curr_spec.num_filters, scope
            )
        return output
    ...
    def _spaghetti_stem_node(self, net, node, scope):
        ...
        self._nodes[node] = slim.conv2d(
            ops.fixed_padding(net, kernel_size) if self._use_explicit_padding else net,
            stem_spec.num_filters,
            [kernel_size, kernel_size],
            stride=2,
            activation_fn=self._activation_fn,
            normalizer_fn=self._normalization_fn,
...
def _spaghettinet_edgetpu_s():
    ...
    nodes["c0n0"] = SpaghettiNode(
        ...
        edges=[SpaghettiResampleEdge(input="n3"), SpaghettiResampleEdge(input="n4")],
    )
    nodes["c0n1"] = SpaghettiNode(
        ...
        edges=[SpaghettiResampleEdge(input="n2"), SpaghettiResampleEdge(input="c0n0")],
    )
    ...
    nodes["c0n3"] = SpaghettiNode(
        ...
        edges=[SpaghettiResampleEdge(input="n5"), SpaghettiResampleEdge(input="c0n0")],
    )
...
class SSDSpaghettinetFeatureExtractor(ssd_meta_arch.SSDFeatureExtractor):
    ...
    def extract_features(self, preprocessed_inputs):
        ...
        with tf.variable_scope(self._spaghettinet_arch_name, reuse=self._reuse_weights):
            with slim.arg_scope(
                [slim.conv2d],
