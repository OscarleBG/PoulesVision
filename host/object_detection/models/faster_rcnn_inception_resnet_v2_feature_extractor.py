        ...
        with slim.arg_scope(
            ...
            with slim.arg_scope([slim.batch_norm], is_training=self._train_batch_norm):
                with tf.variable_scope(
                    "InceptionResnetV2", reuse=self._reuse_weights
                ) as scope:
    ...
    def _extract_box_classifier_features(self, proposal_feature_maps, scope):
        ...
        with tf.variable_scope("InceptionResnetV2", reuse=self._reuse_weights):
            with slim.arg_scope(
                ...
                # Forces is_training to False to disable batch norm update.
                with slim.arg_scope(
                    [slim.batch_norm], is_training=self._train_batch_norm
                ):
                    with slim.arg_scope(
    ...
    def restore_from_classification_checkpoint_fn(
        ...
        for variable in variables_helper.get_global_variables_safely():
            ...
            if variable.op.name.startswith(second_stage_feature_extractor_scope):
                var_name = variable.op.name.replace(
                    second_stage_feature_extractor_scope + "/InceptionResnetV2/Repeat",
