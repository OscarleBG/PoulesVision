...
from object_detection.meta_architectures.center_net_meta_arch import (
...
class CenterNetResnetV1FpnFeatureExtractor(CenterNetFeatureExtractor):
    ...
    def __init__(
        ...
        if resnet_type == "resnet_v1_50":
            self._base_model = tf.keras.applications.ResNet50(
        ...
        outputs = [
            self._base_model.get_layer(output_layer_name).output
            for output_layer_name in output_layers
        ]

        self._resnet_model = tf.keras.models.Model(
            inputs=self._base_model.input, outputs=outputs
        )
        ...
        residual_op = tf.keras.layers.Conv2D(
            filters=256, kernel_size=1, strides=1, padding="same"
        )
    ...
    def preprocess(self, resized_inputs):
        ...
        """
        resized_inputs = super(CenterNetResnetV1FpnFeatureExtractor, self).preprocess(
            resized_inputs
        )
