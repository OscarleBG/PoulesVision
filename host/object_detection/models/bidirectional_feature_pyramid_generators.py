...
def _create_bifpn_input_config(
    fpn_min_level, fpn_max_level, input_max_level, level_scales=None
):
    ...
    for i in range(fpn_min_level, min(fpn_max_level, input_max_level) + 1):
        bifpn_input_params.append(
            {"name": "0_up_lvl_{}".format(i), "scale": level_scales[i - fpn_min_level]}
        )
...
def _create_bifpn_node_config(
    ...
    default_node_params = {
        "num_channels": bifpn_num_filters,
        "combine_method": "fast_attention",
        "input_op": functools.partial(
            _create_bifpn_resample_block, downsample_method="max_pooling"
        ),
        "post_combine_op": functools.partial(
...
def _create_bifpn_resample_block(
    name,
    input_scale,
    input_num_channels,
    output_scale,
    ...
    layers.extend(
        bifpn_utils.create_resample_feature_map_ops(
            input_scale,
            output_scale,
            downsample_method,
    ...
    def __init__(
        ...
        for node_config in bifpn_node_config:
            ...
            if node_config["post_combine_op"]:
                post_combine_block.extend(
                    node_config["post_combine_op"](
