...
class ConvolutionalBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, kernel_size, out_channels, stride=1, relu=True, padding="same"):
...
class ResidualBlock(tf.keras.layers.Layer):
    ...
    def __init__(
        self, out_channels, skip_conv=False, kernel_size=3, stride=1, padding="same"
    ):
        ...
        if skip_conv:
            self.skip = SkipConvolution(out_channels=out_channels, stride=stride)
        else:
...
class InputDownsampleBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, out_channels_initial_conv, out_channels_residual_block):
        ...
        self.conv_block = ConvolutionalBlock(
            kernel_size=7,
            out_channels=out_channels_initial_conv,
            stride=2,
            padding="valid",
        )
...
def _make_repeated_residual_blocks(
    ...
    for i in range(num_blocks - 1):
        ...
        blocks.append(
            ResidualBlock(
                out_channels=residual_channels, stride=stride, skip_conv=skip_conv
            )
        )
    ...
    blocks.append(
        ResidualBlock(out_channels=out_channels, skip_conv=skip_conv, stride=stride)
    )
    ...
    def __init__(
        ...
        if num_stages > 1:
            ...
            self.inner_block = _make_repeated_residual_blocks(
                out_channels=out_channels_downsampled, num_blocks=blocks_per_stage[1]
            )
        ...
        for _ in range(self.num_hourglasses):
            self.hourglass_network.append(
                EncoderDecoderBlock(
                    ...
                    channel_dims=channel_dims_per_stage,
            ...
            self.output_conv.append(
                ConvolutionalBlock(
                    kernel_size=3, out_channels=channel_dims_per_stage[0]
                )
            )
    ...
    def call(self, inputs):
        ...
        for i in range(self.num_hourglasses):
            ...
            if i < self.num_hourglasses - 1:
                secondary_output = self.intermediate_conv1[i](
                    inputs
...
def hourglass_depth(network):
    ...
    intermediate_layers = _layer_depth(network.intermediate_conv2) + _layer_depth(
        network.intermediate_residual
    )

    # network.output_conv is applied before sending input to the later stages
    output_layers = _layer_depth(network.output_conv)

    encoder_decoder_layers = sum(
        _encoder_decoder_depth(net) for net in network.hourglass_network
    )

    return (
        input_conv_layers + encoder_decoder_layers + intermediate_layers + output_layers
    )
