...
class MobilenetV2Test(test_case.TestCase):
    ...
    def _check_returns_correct_shape(
        ...
        image_tensor = np.random.rand(batch_size, image_height, image_width, 3).astype(
            np.float32
        )
    ...
    def _check_returns_correct_shapes_with_dynamic_inputs(
        ...
        height = tf.random.uniform(
            [], minval=image_height, maxval=image_height + 1, dtype=tf.int32
        )
        width = tf.random.uniform(
            [], minval=image_width, maxval=image_width + 1, dtype=tf.int32
        )
        image_tensor = tf.random.uniform(
            [batch_size, height, width, 3], dtype=tf.float32
        )
        model = self._create_application_with_layer_outputs(
            layer_names=layer_names,
            batchnorm_training=False,
    ...
    def test_returns_correct_shapes_128(self):
        ...
        self._check_returns_correct_shape(
            2, image_height, image_width, depth_multiplier, expected_feature_map_shape
        )

    def test_returns_correct_shapes_128_explicit_padding(self):
        image_height = 128
        image_width = 128
        depth_multiplier = 1.0
        expected_feature_map_shape = (
    ...
    def test_returns_correct_shapes_with_dynamic_inputs(self):
        image_height = 128
        image_width = 128
        depth_multiplier = 1.0
        expected_feature_map_shape = (
    ...
    def test_returns_correct_shapes_enforcing_min_depth(self):
        image_height = 299
        image_width = 299
    ...
    def test_returns_correct_shapes_with_conv_defs(self):
        image_height = 299
        image_width = 299
        depth_multiplier = 1.0
        conv_1 = model_utils.ConvDefs(conv_name="Conv_1", filters=256)
        conv_defs = [conv_1]
