...
class NonLocalBlock(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        if pairwise_fn not in ("dot", "embedded_softmax"):
            raise RuntimeError(
                'pairwise_fn must be one of "dot" or ' '"embedded_softmax"'
            )
    ...
    def build(self, input_shape):
        ...
        self.queries_conv = tf.keras.layers.Conv2D(
            filters=self._bottleneck_channels,
            kernel_size=1,
            use_bias=False,
            strides=1,
            padding="same",
        ...
        self.expand_conv = tf.keras.layers.Conv2D(
            filters=channels, kernel_size=1, use_bias=False, strides=1, padding="same"
        )
    ...
    def call(self, inputs):
        ...
        """
        batch, height, width, _ = shape_utils.combined_static_and_dynamic_shape(inputs)

        x = inputs
        if self._add_coord_conv:
            coords_x, coords_y = tf.meshgrid(
                tf.linspace(-1.0, 1.0, height), tf.linspace(-1.0, 1.0, width)
            )
            coords = tf.stack([coords_y, coords_x], axis=-1)
            coords = tf.tile(coords[tf.newaxis, :, :, :], multiples=[batch, 1, 1, 1])
        ...
        if self._pairwise_fn == "dot":
            ...
            scores = (1.0 / normalization) * scores
