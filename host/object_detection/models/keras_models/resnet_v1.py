...
class _LayersOverride(object):
    ...
    def _FixedPaddingLayer(self, kernel_size, rate=1):  # pylint: disable=invalid-name
        return tf.keras.layers.Lambda(lambda x: _fixed_padding(x, kernel_size, rate))
    ...
    def BatchNormalization(self, **kwargs):  # pylint: disable=invalid-name
        ...
        if self._conv_hyperparams:
            return self._conv_hyperparams.build_batch_norm(
                training=self._batchnorm_training, name=name
            )
        else:
...
def resnet_v1_50(
    ...
    **kwargs
    ...
    return tf.keras.applications.resnet.ResNet50(layers=layers_override, **kwargs)
...
def resnet_v1_152(
    ...
    **kwargs
    ...
    return tf.keras.applications.resnet.ResNet152(layers=layers_override, **kwargs)
...
def block_basic(x, filters, kernel_size=3, stride=1, conv_shortcut=False, name=None):
    ...
    if conv_shortcut:
        shortcut = layers.Conv2D(filters, 1, strides=1, name=name + "_0_conv")(preact)
    else:
        shortcut = layers.MaxPooling2D(1, strides=stride)(x) if stride > 1 else x

    x = layers.ZeroPadding2D(padding=((1, 1), (1, 1)), name=name + "_1_pad")(preact)
    x = layers.Conv2D(
...
def stack_basic(x, filters, blocks, stride1=2, name=None):
    ...
    x = block_basic(x, filters, stride=stride1, name=name + "_block" + str(blocks))
    return x
