...
from object_detection.models import (
    bidirectional_feature_pyramid_generators as bifpn_generators,
)
...
class SSDEfficientNetBiFPNKerasFeatureExtractor(ssd_meta_arch.SSDKerasFeatureExtractor):
    ...
    def __init__(
        ...
        if depth_multiplier != 1.0:
            raise ValueError(
                "EfficientNetBiFPN does not support a non-default " "depth_multiplier."
            )
        ...
        logging.info(
            "EfficientDet EfficientNet backbone version: %s", self._efficientnet_version
        )
        logging.info("EfficientDet BiFPN num filters: %d", self._bifpn_num_filters)
        logging.info(
            "EfficientDet BiFPN num iterations: %d", self._bifpn_num_iterations
        )
        ...
        if override_base_feature_extractor_hyperparams:
            efficientnet_overrides[
                "weight_decay"
            ] = conv_hyperparams.get_regularizer_weight()
        if conv_hyperparams.use_sync_batch_norm() and keras_backend.is_tpu_strategy(
