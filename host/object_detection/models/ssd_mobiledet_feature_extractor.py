...
def _swish6(h):
    with tf.name_scope("swish6"):
        return h * tf.nn.relu6(h + np.float32(3)) * np.float32(1.0 / 6.0)
...
def _conv(
    ...
    if activation_fn is None:
        raise ValueError(
            "Activation function cannot be None. Use tf.identity "
...
def _inverted_bottleneck_no_expansion(
    h, filters, activation_fn=tf.nn.relu6, kernel_size=3, strides=1, use_se=False
):
...
def mobiledet_cpu_backbone(h, multiplier=1.0):
    ...
    ibn = functools.partial(_inverted_bottleneck, use_se=True, activation_fn=_swish6)
...
def mobiledet_dsp_backbone(h, multiplier=1.0):
    ...
    h = _inverted_bottleneck_no_expansion(h, _scale(24), activation_fn=tf.nn.relu6)
    endpoints["C1"] = h
...
def mobiledet_edgetpu_backbone(h, multiplier=1.0):
    ...
    h = _conv(h, _scale(32), 3, strides=2, activation_fn=tf.nn.relu6)
    h = tucker(
        h, _scale(16), input_rank_ratio=0.25, output_rank_ratio=0.75, residual=False
    )
    endpoints["C1"] = h
    ...
    h = fused(h, _scale(16), expansion=4)
    endpoints["C2"] = h
    h = fused(h, _scale(40), expansion=8, kernel_size=5, strides=2, residual=False)
...
def mobiledet_gpu_backbone(h, multiplier=1.0):
    ...
    # block 1
    h = tucker(
        h, _scale(16), input_rank_ratio=0.25, output_rank_ratio=0.25, residual=False
    )
    endpoints["C1"] = h
    ...
    # block 3
    h = fused(h, _scale(64), expansion=8, kernel_size=3, strides=2, residual=False)
    ...
    # block 4
    h = fused(h, _scale(128), expansion=8, kernel_size=3, strides=2, residual=False)
    h = fused(h, _scale(128), expansion=4)
    h = fused(h, _scale(128), expansion=4)
    h = fused(h, _scale(128), expansion=4)

    # block 5
    h = fused(h, _scale(128), expansion=8, kernel_size=3, strides=1, residual=False)
    ...
    # block 6
    h = fused(h, _scale(128), expansion=4, kernel_size=3, strides=2, residual=False)
    h = fused(h, _scale(128), expansion=4)
    h = fused(h, _scale(128), expansion=4)
    h = fused(h, _scale(128), expansion=4)
