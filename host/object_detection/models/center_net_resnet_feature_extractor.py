...
class CenterNetResnetFeatureExtractor(CenterNetFeatureExtractor):
    ...
    def __init__(
        ...
        if resnet_type == "resnet_v2_101":
            self._base_model = tf.keras.applications.ResNet101V2(
                weights=None, include_top=False
            )
            output_layer = "conv5_block3_out"
        elif resnet_type == "resnet_v2_50":
            self._base_model = tf.keras.applications.ResNet50V2(
                weights=None, include_top=False
            )
            output_layer = "conv5_block3_out"
        else:
        ...
        self._resnet_model = tf.keras.models.Model(
            inputs=self._base_model.input, outputs=output_layer.output
        )
