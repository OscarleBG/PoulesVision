...
class SsdFeatureExtractorTestBase(test_case.TestCase):
    ...
    def _create_features(
        ...
        feature_extractor = self._create_feature_extractor(
            depth_multiplier, pad_to_multiple, **kwargs
        )
        return feature_extractor

    def _extract_features(self, image_tensor, feature_extractor, use_keras=False):
        if use_keras:
    ...
    def check_extract_features_returns_correct_shape(
        ...
        def graph_fn(image_tensor):
            return self._extract_features(
                image_tensor, feature_extractor, use_keras=use_keras
            )
    ...
    def check_extract_features_raises_error_with_invalid_image_size(
        ...
        def graph_fn():
            feature_maps = self._extract_features(
                preprocessed_inputs, feature_extractor, use_keras=use_keras
            )
            return feature_maps
    ...
    def get_feature_extractor_variables(
        self, depth_multiplier, pad_to_multiple, use_keras=False, use_depthwise=False
    ):
        g = tf.Graph()
        with g.as_default():
            feature_extractor = self._create_features(
