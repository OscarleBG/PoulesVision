...
def _maybe_pad(feature, use_explicit_padding, kernel_size=3):
    return ops.fixed_padding(feature, kernel_size) if use_explicit_padding else feature
...
def _apply_size_dependent_ordering(
    input_feature,
    feature_level,
    block_level,
    expansion_size,
    ...
    if feature_level >= block_level:  # Perform 1x1 then upsampling.
        ...
        node = slim.max_pool2d(
            _maybe_pad(input_feature, use_explicit_padding),
            [3, 3],
...
def _mnasfpn_cell(
            ...
            for i, inp in enumerate(block.inputs):
                with tf.variable_scope("input_{}".format(i)):
                    ...
                    node = _apply_size_dependent_ordering(
                        ...
                        input_level,
                        block_level,
                        multiplier_func(block.expansion_size),
                        use_explicit_padding,
                        use_native_resize_op,
                    )
    ...
    for in_i, out_i in enumerate(out_idx):
        if cell_features[out_i].shape.as_list() == cell_features[in_i].shape.as_list():
...
def mnasfpn(
    ...
    for ci, cell_spec in enumerate(head_def["spec"]):
        with tf.variable_scope("cell_{}".format(ci)):
            features = _mnasfpn_cell(
                features,
                head_def["feature_levels"],
                cell_spec,
                output_channel,
                use_explicit_padding,
                use_native_resize_op,
...
def training_scope(l2_weight_decay=1e-4, is_training=None):
    ...
    with slim.arg_scope(
        ...
        weights_initializer=tf.initializers.truncated_normal(
            stddev=0.536
        ),  # He_normal for 3x3 depthwise kernel.
        weights_regularizer=slim.l2_regularizer(l2_weight_decay),
    ), slim.arg_scope(
        [slim.batch_norm],
...
class SSDMobileNetV2MnasFPNFeatureExtractor(ssd_meta_arch.SSDFeatureExtractor):
    ...
    def _verify_config(self, inputs):
        ...
        base_width = (
            inputs[0].shape.as_list()[1] * 2 ** self._head_def["feature_levels"][0]
        )
        ...
        for cell_spec in self._head_def["spec"]:
            ...
            for i in range(num_inputs):
                if (
                    cell_spec[-num_inputs + i].output_level
                    != self._head_def["feature_levels"][i]
                ):
                    raise ValueError(
                        "Mismatch between node level {} and desired output level {}.".format(
                            cell_spec[-num_inputs + i].output_level,
                            self._head_def["feature_levels"][i],
                        )
                    )
    ...
    def extract_features(self, preprocessed_inputs):
        ...
        with tf.variable_scope("MobilenetV2", reuse=self._reuse_weights) as scope:
            with slim.arg_scope(
                ...
                with slim.arg_scope(
                    training_scope(l2_weight_decay=4e-5, is_training=self._is_training)
                ):
