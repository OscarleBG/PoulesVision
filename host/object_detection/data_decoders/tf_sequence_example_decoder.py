...
class TfSequenceExampleDecoder(data_decoder.DataDecoder):
    ...
    def __init__(
        ...
        self._items_to_handlers = {
            # Context.
            fields.InputDataFields.image_height: slim_example_decoder.Tensor(
                "image/height"
            ),
            fields.InputDataFields.image_width: slim_example_decoder.Tensor(
                "image/width"
            ),
            # Sequence.
            fields.InputDataFields.num_groundtruth_boxes: slim_example_decoder.NumBoxesSequence(
                "region/bbox/xmin"
            ),
            fields.InputDataFields.groundtruth_boxes: slim_example_decoder.BoundingBoxSequence(
                prefix="region/bbox/", default_value=0.0
            ),
            fields.InputDataFields.groundtruth_weights: slim_example_decoder.Tensor(
        ...
        if load_context_features:
            self._context_keys_to_features["image/context_features"] = tf.VarLenFeature(
                dtype=tf.float32
            )
            self._items_to_handlers[
        ...
        if load_context_image_ids:
            ...
            self._items_to_handlers[
                ...
                "image/context_features_image_id_list", default_value=""
            )
    ...
    def decode(self, tf_seq_example_string_tensor):
        ...
        tensor_dict[fields.InputDataFields.groundtruth_boxes].set_shape([None, None, 4])
        tensor_dict[fields.InputDataFields.num_groundtruth_boxes] = tf.cast(
            tensor_dict[fields.InputDataFields.num_groundtruth_boxes], dtype=tf.int32
        )
        ...
        tensor_dict[fields.InputDataFields.groundtruth_weights] = tf.cond(
            tf.greater(
                tf.size(tensor_dict[fields.InputDataFields.groundtruth_weights]), 0
            ),
        ...
        if self._fully_annotated:
            tensor_dict[fields.InputDataFields.is_annotated] = tf.ones_like(
                tensor_dict[fields.InputDataFields.num_groundtruth_boxes], dtype=tf.bool
            )
        else:
