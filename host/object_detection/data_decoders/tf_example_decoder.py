    ...
    def __init__(
        ...
        display_name_to_id_table = hash_table_class(
            initializer=lookup.KeyValueTensorInitializer(
                ...
                values=tf.constant(list(display_name_to_id.values()), dtype=tf.int64),
            ),
            default_value=-1,
        )
        ...
        super(_ClassTensorHandler, self).__init__(
            tensor_key, shape_keys, shape, default_value
        )

    def tensors_to_item(self, keys_to_tensors):
        unmapped_tensor = super(_ClassTensorHandler, self).tensors_to_item(
            keys_to_tensors
        )
        return tf.maximum(
            self._name_to_id_table.lookup(unmapped_tensor),
...
class TfExampleDecoder(data_decoder.DataDecoder):
    ...
    def __init__(
        ...
        self.keys_to_features = {
            ...
            "image/neg_category_ids": tf.VarLenFeature(tf.int64),
            "image/not_exhaustive_category_ids": tf.VarLenFeature(tf.int64),
            "image/class/confidence": tf.VarLenFeature(tf.float32),
        ...
        if use_keypoint_label_map:
            ...
            self.keys_to_features[_KEYPOINT_TEXT_FIELD] = tf.VarLenFeature(tf.string)
            self.items_to_handlers[
        ...
        if load_context_features:
            ...
            self.keys_to_features["image/context_feature_length"] = tf.FixedLenFeature(
                (), tf.int64
            )
            self.items_to_handlers[
        ...
        if num_additional_channels > 0:
            ...
            ] = tf.FixedLenFeature((num_additional_channels,), tf.string)
            self.items_to_handlers[
                fields.InputDataFields.image_additional_channels
            ] = additional_channel_image
        self._num_keypoints = num_keypoints
        if num_keypoints > 0:
            self.keys_to_features["image/object/keypoint/x"] = tf.VarLenFeature(
                tf.float32
            )
            self.keys_to_features["image/object/keypoint/y"] = tf.VarLenFeature(
                tf.float32
            )
            self.keys_to_features[
            ...
            if load_keypoint_depth_features:
                self.keys_to_features["image/object/keypoint/z"] = tf.VarLenFeature(
                    tf.float32
                )
                self.keys_to_features[
                ...
                self.items_to_handlers[
                    ...
                    ["image/object/keypoint/x", "image/object/keypoint/z/weights"],
                    self._reshape_keypoint_depth_weights,
                )

        if load_instance_masks:
            if instance_mask_type in (
                input_reader_pb2.DEFAULT,
            ...
            self.keys_to_features["image/object/mask/weight"] = tf.VarLenFeature(
                tf.float32
            )
            self.items_to_handlers[
        ...
        if load_dense_pose:
            self.keys_to_features["image/object/densepose/num"] = tf.VarLenFeature(
                tf.int64
            )
            self.keys_to_features[
                "image/object/densepose/part_index"
            ] = tf.VarLenFeature(tf.int64)
            self.keys_to_features["image/object/densepose/x"] = tf.VarLenFeature(
                tf.float32
            )
            self.keys_to_features["image/object/densepose/y"] = tf.VarLenFeature(
                tf.float32
            )
            self.keys_to_features["image/object/densepose/u"] = tf.VarLenFeature(
                tf.float32
            )
            self.keys_to_features["image/object/densepose/v"] = tf.VarLenFeature(
                tf.float32
            )
            self.items_to_handlers[
            ...
            self.items_to_handlers[
                ...
                ["image/object/densepose/part_index", "image/object/densepose/num"],
                self._dense_pose_part_indices,
            )
            self.items_to_handlers[
        ...
        if load_track_id:
            self.keys_to_features["image/object/track/label"] = tf.VarLenFeature(
                tf.int64
            )
            self.items_to_handlers[
        ...
        if label_map_proto_file:
            ...
            label_handler = slim_example_decoder.BackupHandler(
                _ClassTensorHandler(
                    "image/object/class/text", label_map_proto_file, default_value=""
                ),
        ...
        if expand_hierarchy_labels:
            if label_map_proto_file:
                (
                    ancestors_lut,
                    descendants_lut,
                ) = label_map_util.get_label_map_hierarchy_lut(
                    label_map_proto_file, True
                )
                ...
                raise ValueError(
                    "In order to expand labels, the label_map_proto_file "
                    "has to be provided."
                )
    ...
    def decode(self, tf_example_string_tensor):
        ...
        tensor_dict[fields.InputDataFields.groundtruth_weights] = tf.cond(
            tf.greater(
                tf.shape(tensor_dict[fields.InputDataFields.groundtruth_weights])[0], 0
            ),
        ...
        if fields.InputDataFields.groundtruth_instance_masks in tensor_dict:
            ...
            tensor_dict[
                ...
                tf.greater(num_gt_instance_mask_weights, 0),
        ...
        if fields.InputDataFields.groundtruth_keypoints in tensor_dict:
            ...
            if self._keypoint_label_map is None:
                ...
                tensor_dict[gt_kpt_fld] = tf.reshape(
                    tensor_dict[gt_kpt_fld], [-1, self._num_keypoints, 2]
                )
                ...
                def true_fn(num_instances):
                    ...
                    kpts_idx = tf.cast(
                        self._kpts_name_to_id_table.lookup(
                            tensor_dict[_KEYPOINT_TEXT_FIELD]
                        ),
                        dtype=tf.int32,
                    )
                    ...
                    instance_idx = tf.tile(
                        tf.expand_dims(
                            tf.range(num_instances, dtype=tf.int32), axis=-1
                        ),
                        [1, num_kpt_texts],
                    )
                    ...
                    gt_kpt_vis = tf.cast(
                        tf.scatter_nd(
                            ...
                            shape=[num_instances, self._num_keypoints],
                        ),
                        dtype=tf.bool,
                    )
                    visibilities_tiled = tf.tile(
                    ...
                    gt_kpt = tf.where(
                        visibilities_tiled, gt_kpt, np.nan * tf.ones_like(gt_kpt)
                    )
                ...
                def false_fn():
                    ...
                    return (
                        tf.zeros([0, self._num_keypoints, 2], dtype=tf.float32),
        ...
        if self._expand_hierarchy_labels:
            ...
            tensor_dict[input_fields.groundtruth_image_confidences] = image_confidences
    ...
    def _reshape_keypoint_visibilities(self, keys_to_tensors):
        ...
        vis = tf.cond(
            tf.equal(tf.size(x), tf.size(vis)),
            true_fn=lambda: vis,
    ...
    def _expand_image_label_hierarchy(self, image_classes, image_confidences):
        ...
        def expand_labels(relation_tensor, confidence_value):
            ...
            expanded_indices = tf.reduce_any(
                (
                    tf.gather(
                        relation_tensor, target_image_classes - _LABEL_OFFSET, axis=0
                    )
                    > 0
                ),
                axis=0,
            ...
            new_groundtruth_image_classes = tf.concat(
                [
            ...
            new_groundtruth_image_confidences = tf.concat(
                [
                    tf.boolean_mask(image_confidences, tf.logical_not(mask)),
                    tf.ones(
                        [tf.shape(expanded_indices)[0]], dtype=image_confidences.dtype
                    )
                    * confidence_value,
                ],
                axis=0,
    ...
    def _expansion_box_field_labels(
        self, object_classes, object_field, copy_class_id=False
    ):
