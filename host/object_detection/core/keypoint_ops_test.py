...
class KeypointOpsTest(test_case.TestCase):
    ...
    def test_clip_to_window(self):
        def graph_fn():
            ...
            expected_keypoints = tf.constant(
                [[[0.25, 0.5], [0.75, 0.75]], [[0.5, 0.25], [0.75, 0.75]]]
            )
    ...
    def test_prune_outside_window(self):
        def graph_fn():
            ...
            expected_keypoints = tf.constant(
                [[[0.25, 0.5], [0.75, 0.75]], [[np.nan, np.nan], [np.nan, np.nan]]]
            )
    ...
    def test_keypoints_to_enclosing_bounding_boxes(self):
        ...
        expected_bboxes = np.array([[3.0, 4.0, 8.0, 20.0], [0.0, 3.0, 5.0, 19.0]])
    ...
    def test_to_normalized_coordinates(self):
        def graph_fn():
            keypoints = tf.constant(
                [[[10.0, 30.0], [30.0, 45.0]], [[20.0, 0.0], [40.0, 60.0]]]
            )
    ...
    def test_rot90_permutation(self):
        def graph_fn():
            ...
            output = keypoint_ops.rot90(keypoints, rotation_permutation=rot_permutation)
            return output, expected_keypoints
    ...
    def test_keypoint_weights_from_visibilities(self):
        ...
        expected_keypoint_weights = [[1.0, 2.0, 0.0], [0.0, 2.0, 0.0]]
        output = self.execute(graph_fn, [])
    ...
    def test_set_keypoint_visibilities_no_initial_kpt_vis(self):
        keypoints_np = np.array(
            [
                [[np.nan, 0.2], [np.nan, np.nan], [-3.0, 7.0]],
                [[0.5, 0.2], [4.0, 1.0], [-3.0, np.nan]],
            ],
            dtype=np.float32,
        )
        ...
        def graph_fn():
            ...
            keypoint_visibilities = keypoint_ops.set_keypoint_visibilities(keypoints)
            return keypoint_visibilities

        expected_kpt_vis = [[False, False, True], [True, True, False]]
        output = self.execute(graph_fn, [])
    ...
    def test_set_keypoint_visibilities(self):
        ...
        initial_keypoint_visibilities_np = np.array(
            [
                [
                    ...
                    False,
                ],  # Will be maintained, even though non-NaN coords.
                [True, False, False],  # Will be maintained, even though non-NaN coords.
            ]
        )
        ...
        expected_kpt_vis = [[False, False, False], [True, False, False]]
        output = self.execute(graph_fn, [])
        self.assertAllEqual(expected_kpt_vis, output)
