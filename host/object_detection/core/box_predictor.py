...
class BoxPredictor(object):
    ...
    def predict(
        self, image_features, num_predictions_per_location, scope=None, **params
    ):
        ...
        if len(image_features) != len(num_predictions_per_location):
            raise ValueError(
                ...
                "be of same length, found: {} vs {}".format(
                    len(image_features), len(num_predictions_per_location)
                )
            )
        if scope is not None:
            with tf.variable_scope(scope):
                return self._predict(
                    image_features, num_predictions_per_location, **params
                )
