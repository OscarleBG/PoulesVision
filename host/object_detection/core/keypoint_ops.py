    ...
    with tf.name_scope(scope, "ToAbsoluteCoordinates"):
        ...
        if check_range:
            ...
            max_assert = tf.Assert(
                ...
                ["maximum keypoint coordinate value is larger " "than 1.01: ", max_val],
            )
            with tf.control_dependencies([max_assert]):
                width = tf.identity(width)
...
def keypoint_weights_from_visibilities(
    keypoint_visibilities, per_keypoint_weights=None
):
    ...
    if per_keypoint_weights is None:
        ...
        per_keypoint_weight_mult = tf.ones(
            (
                1,
                num_keypoints,
            ),
