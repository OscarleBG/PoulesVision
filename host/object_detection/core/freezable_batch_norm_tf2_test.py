...
class FreezableBatchNormTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def _build_model(self, use_sync_batch_norm, training=None):
        ...
        if use_sync_batch_norm:
            norm = freezable_sync_batch_norm.FreezableSyncBatchNorm(
                training=training, input_shape=(10,), momentum=0.8
            )
        else:
            norm = freezable_batch_norm.FreezableBatchNorm(
                training=training, input_shape=(10,), momentum=0.8
            )
    ...
    def _train_freezable_batch_norm(
        self, training_mean, training_var, use_sync_batch_norm
    ):
        ...
        train_data = np.random.normal(
            loc=training_mean, scale=training_var, size=(1000, 10)
        )
    ...
    def _test_batchnorm_layer(
        ...
        out_tensor = norm(
            tf.convert_to_tensor(test_data, dtype=tf.float32), training=training_arg
        )
        out = out_tensor
        out -= norm.beta
        out /= norm.gamma
    ...
    def test_batchnorm_freezing_training_none(self, use_sync_batch_norm):
        ...
        trained_weights = self._train_freezable_batch_norm(
            training_mean, training_var, use_sync_batch_norm
        )
        ...
        test_data = np.random.normal(
            loc=testing_mean, scale=testing_var, size=(1000, 10)
        )

        # Test with training=True passed to the call method:
        training_arg = True
        should_be_training = True
        self._test_batchnorm_layer(
            norm,
            should_be_training,
            test_data,
            testing_mean,
