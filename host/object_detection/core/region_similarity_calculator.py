...
class DETRSimilarity(RegionSimilarityCalculator):
    ...
    def _compare(self, boxlist1, boxlist2):
        ...
        classification_scores = tf.matmul(
            groundtruth_labels, predicted_labels, transpose_b=True
        )
        loss = (
            self.l1_weight * box_list_ops.l1(boxlist1, boxlist2)
            + self.giou_weight * (1 - box_list_ops.giou(boxlist1, boxlist2))
...
class ThresholdedIouSimilarity(RegionSimilarityCalculator):
    ...
    def _compare(self, boxlist1, boxlist2):
        ...
        thresholded_ious = tf.where(
            ious > self._iou_threshold, row_replicated_scores, tf.zeros_like(ious)
        )
