...
def scale_height_width(boxlist, y_scale, x_scale, scope=None):
    ...
    with tf.name_scope(scope, "ScaleHeightWidth"):
        ...
        scaled_boxlist = box_list.BoxList(tf.stack([y_min, x_min, y_max, x_max], 1))
        return _copy_extra_fields(scaled_boxlist, boxlist)
...
def clip_to_window(boxlist, window, filter_nonoverlapping=True, scope=None):
    ...
    with tf.name_scope(scope, "ClipToWindow"):
        ...
        clipped = box_list.BoxList(
            tf.concat([y_min_clipped, x_min_clipped, y_max_clipped, x_max_clipped], 1)
        )
...
def l1(boxlist1, boxlist2, scope=None):
    ...
    with tf.name_scope(scope, "PairwiseL1"):
        ...
        ycenters = tf.abs(
            tf.expand_dims(ycenter2, axis=0)
            - tf.expand_dims(tf.transpose(ycenter1), axis=1)
        )
        xcenters = tf.abs(
            tf.expand_dims(xcenter2, axis=0)
            - tf.expand_dims(tf.transpose(xcenter1), axis=1)
        )
        heights = tf.abs(
            tf.expand_dims(h2, axis=0) - tf.expand_dims(tf.transpose(h1), axis=1)
        )
        widths = tf.abs(
            tf.expand_dims(w2, axis=0) - tf.expand_dims(tf.transpose(w1), axis=1)
        )
        ...
        if check_range:
            ...
            max_assert = tf.Assert(
                ...
                [
                    ...
                    "than %f: " % maximum_normalized_coordinate,
                    box_maximum,
                ],
            )
            with tf.control_dependencies([max_assert]):
...
def refine_boxes_multi_class(
    pool_boxes, num_classes, nms_iou_thresh, nms_max_detections, voting_iou_thresh=0.5
):
...
def box_voting(selected_boxes, pool_boxes, iou_thresh=0.5):
    ...
    scores_assert = tf.Assert(
        tf.reduce_all(tf.greater_equal(scores, 0)), ["Scores must be non negative."]
    )
    ...
    with tf.name_scope(scope, "SelectRandomBox"):
        ...
        def select_box():
            random_index = tf.random_uniform(
                [], maxval=number_of_boxes, dtype=tf.int32, seed=seed
            )
...
def get_minimal_coverage_box(boxlist, default_box=None, scope=None):
    ...
    with tf.name_scope(scope, "CreateCoverageBox"):
        ...
        def coverage_box(bboxes):
            ...
            return tf.stack(
                [y_min_coverage, x_min_coverage, y_max_coverage, x_max_coverage], axis=1
            )
...
def sample_boxes_by_jittering(boxlist, num_boxes_to_sample, stddev=0.1, scope=None):
    ...
    with tf.name_scope(scope, "SampleBoxesByJittering"):
        ...
        box_indices = tf.random_uniform(
            [num_boxes_to_sample], minval=0, maxval=num_boxes, dtype=tf.int32
        )
