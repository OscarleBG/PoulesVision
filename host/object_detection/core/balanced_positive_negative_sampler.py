...
class BalancedPositiveNegativeSampler(minibatch_sampler.MinibatchSampler):
    ...
    def _get_num_pos_neg_samples(self, sorted_indices_tensor, sample_size):
        ...
        valid_positive_index = tf.greater(
            sorted_indices_tensor, tf.zeros(input_length, tf.int32)
        )
    ...
    def _get_values_from_start_and_end(
    ...
    def _static_subsample(self, indicator, batch_size, labels):
        ...
        indicator_idx = tf.where(
            indicator, tf.range(1, input_length + 1), tf.zeros(input_length, tf.int32)
        )
        ...
        sorted_signed_indicator_idx = tf.nn.top_k(
            signed_indicator_idx, input_length, sorted=True
        ).values
        ...
        sampled_idx_indicator = tf.cast(
            tf.reduce_sum(tf.one_hot(sampled_idx, depth=input_length), axis=0), tf.bool
        )

        # project back the order based on stored permutations
        idx_indicator = tf.scatter_nd(
            tf.expand_dims(permutation, -1),
