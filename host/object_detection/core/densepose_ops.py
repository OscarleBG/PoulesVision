...
def prune_outside_window(
    dp_num_points, dp_part_ids, dp_surface_coords, window, scope=None
):
    ...
    with tf.name_scope(scope, "DensePosePruneOutsideWindow"):
        ...
        dp_num_points_tiled = tf.tile(
            dp_num_points[:, tf.newaxis], multiples=[1, num_points]
        )
        range_tiled = tf.tile(
            tf.range(num_points)[tf.newaxis, :], multiples=[num_instances, 1]
        )
        ...
        new_dp_num_points = tf.math.reduce_sum(tf.cast(valid_indices, tf.int32), axis=1)
        ...
        new_dp_part_ids, new_dp_surface_coords = shape_utils.static_or_dynamic_map_fn(
...
def to_normalized_coordinates(
    dp_surface_coords, height, width, check_range=True, scope=None
):
    ...
    with tf.name_scope(scope, "DensePoseToNormalizedCoordinates"):
        ...
        if check_range:
            ...
            max_assert = tf.Assert(
                tf.greater(max_val, 1.01), ["max value is lower than 1.01: ", max_val]
            )
    ...
    with tf.name_scope(scope, "DensePoseToAbsoluteCoordinates"):
        ...
        if check_range:
            ...
            max_assert = tf.Assert(
                ...
                ["maximum coordinate value is larger than 1.01: ", max_val],
            )
            with tf.control_dependencies([max_assert]):
                width = tf.identity(width)
...
class DensePoseHorizontalFlip(object):
    ...
    def __init__(self):
        ...
        uv_symmetry_transforms_path = tf.resource_loader.get_path_to_datafile(
            os.path.join(
                path, "..", "dataset_tools", "densepose", "UV_symmetry_transforms.mat"
            )
        )
        tf.logging.info(
            "Loading DensePose symmetry transforms file from {}".format(
        ...
        for key in ("U_transforms", "V_transforms"):
            ...
            for i in range(data[key].shape[1]):
                ...
                data_normalized = data[key][0, i].astype(np.float32) / 255.0
        ...
        for i, part_name in enumerate(PART_NAMES):
            if b"left" in part_name:
                part_symmetries.append(
                    PART_NAMES.index(part_name.replace(b"left", b"right"))
                )
            elif b"right" in part_name:
                part_symmetries.append(
                    PART_NAMES.index(part_name.replace(b"right", b"left"))
                )
            else:
    ...
    def flip_parts_and_coords(self, part_ids, vu):
        ...
        new_part_ids = tf.reshape(new_part_ids_flattened, [num_instances, num_points])

        # Convert VU floating point coordinates to values in [256, 256] grid.
        vu = tf.math.minimum(tf.math.maximum(vu, 0.0), 1.0)
        vu_locs = tf.cast(vu * 256.0, dtype=tf.int32)
...
def flip_horizontal(dp_part_ids, dp_surface_coords, scope=None):
    ...
    with tf.name_scope(scope, "DensePoseFlipHorizontal"):
        ...
        new_dp_part_ids, new_vu = horizontal_flip.flip_parts_and_coords(dp_part_ids, vu)
