...
def batch_iou(boxes1, boxes2):
    ...
    with tf.name_scope("BatchIOU"):
        ...
        intersection_area = tf.maximum(
            (intersection_xmax - intersection_xmin), 0
...
def _self_suppression(iou, iou_threshold, loop_condition, iou_sum):
    ...
    iou_suppressed = (
        tf.reshape(
            tf.cast(
                tf.reduce_max(can_suppress_others * iou, 1) <= iou_threshold, iou.dtype
            ),
            [1, -1, 1],
        )
        * iou
    ...
    return [
        iou_suppressed,
        iou_threshold,
        tf.reduce_any(iou_sum - iou_sum_new > iou_threshold),
        iou_sum_new,
    ]
...
def _cross_suppression(boxes, box_slice, iou_threshold, inner_idx):
    ...
    """
    new_slice = tf.slice(
        boxes, [0, inner_idx * _NMS_TILE_SIZE, 0], [1, _NMS_TILE_SIZE, 4]
    )
    iou = batch_iou(new_slice, box_slice)
    ret_slice = (
        tf.expand_dims(
            tf.cast(tf.reduce_all(iou < iou_threshold, [1]), box_slice.dtype), 2
        )
        * box_slice
    )
...
def _suppression_loop_body(boxes, iou_threshold, output_size, idx):
    ...
    _, box_slice, _, _ = tf.while_loop(
        ...
        _cross_suppression,
        [boxes, box_slice, iou_threshold, tf.constant(0)],
    )

    # Iterates over the current tile to compute self-suppression.
    iou = batch_iou(box_slice, box_slice)
    mask = tf.expand_dims(
        tf.reshape(tf.range(_NMS_TILE_SIZE), [1, -1])
        > tf.reshape(tf.range(_NMS_TILE_SIZE), [-1, 1]),
        0,
    )
    ...
    suppressed_iou, _, _, _ = tf.while_loop(
        ...
        _self_suppression,
        [iou, iou_threshold, tf.constant(True), tf.reduce_sum(iou, [1, 2])],
    )
    ...
    ) * mask + tf.reshape(boxes, [1, num_tiles, _NMS_TILE_SIZE, 4]) * (1 - mask)
...
def partitioned_non_max_suppression_padded(
    boxes, scores, max_output_size, iou_threshold=0.5, score_threshold=float("-inf")
):
    ...
    pad = (
        tf.cast(tf.ceil(tf.cast(num_boxes, tf.float32) / _NMS_TILE_SIZE), tf.int32)
        * _NMS_TILE_SIZE
        - num_boxes
    )
    ...
    selected_boxes, _, output_size, _ = tf.while_loop(
        ...
        [boxes, iou_threshold, tf.zeros([1], tf.int32), tf.constant(0)],
    )
...
def _validate_boxes_scores_iou_thresh(
    boxes, scores, iou_thresh, change_coordinate_frame, clip_window
):
    ...
    if not (
        shape_utils.get_dim_as_int(boxes.shape[1])
        == shape_utils.get_dim_as_int(scores.shape[1])
...
def _clip_window_prune_boxes(
    sorted_boxes, clip_window, pad_to_max_output_size, change_coordinate_frame
):
    ...
    sorted_boxes = box_list_ops.clip_to_window(
        sorted_boxes, clip_window, filter_nonoverlapping=not pad_to_max_output_size
    )
    ...
    if change_coordinate_frame:
        ...
        if sorted_boxes.has_field(fields.BoxListFields.keypoints):
            ...
            sorted_keypoints = keypoint_ops.change_coordinate_frame(
                sorted_keypoints, clip_window
            )
...
def _clip_boxes(boxes, clip_window):
    ...
    return tf.stack([clipped_ymin, clipped_xmin, clipped_ymax, clipped_xmax], axis=-1)
    ...
    with tf.name_scope(scope, "MultiClassNonMaxSuppression"), tf.device(
        ...
        for class_idx, boxes_idx in zip(range(num_classes), boxes_ids):
            ...
            if pad_to_max_output_size:
                ...
                if use_partitioned_nms:
                    (
                        ...
                        _,
                    ) = partitioned_non_max_suppression_padded(
                ...
                nms_result = box_list_ops.gather(
                    boxlist_and_class_scores, selected_indices
                )
                ...
                if (
                    hasattr(tf.image, "non_max_suppression_with_scores")
                    and tf.compat.forward_compatible(2019, 6, 6)
                    and not use_hard_nms
                ):
                    (
                        selected_indices,
                        selected_scores,
                    ...
                    selected_indices = tf.concat(
                        [
                            selected_indices,
                            tf.zeros(
                                max_selection_size - num_valid_nms_boxes, tf.int32
                            ),
                        ],
                        0,
                    )
                    selected_scores = tf.concat(
                        [
                            selected_scores,
                            tf.zeros(
                                max_selection_size - num_valid_nms_boxes, tf.float32
                            ),
                        ],
                        -1,
                    )
                    ...
                    if soft_nms_sigma != 0:
                        raise ValueError(
                            "Soft NMS not supported in current TF version!"
                        )
            ...
            nms_result.add_field(
                ...
                (
                    tf.zeros_like(nms_result.get_field(fields.BoxListFields.scores))
                    + class_idx
                ),
            )
...
def class_agnostic_non_max_suppression(
    boxes,
    scores,
    score_thresh,
    iou_thresh,
    ...
    with tf.name_scope(scope, "ClassAgnosticNonMaxSuppression"):
        ...
        if boundaries is not None:
            boxlist_and_class_scores.add_field(
                fields.BoxListFields.boundaries, boundaries
            )
        ...
        if pad_to_max_output_size:
            ...
            if use_partitioned_nms:
                ...
                classes_with_max_scores = tf.gather(
                    classes_with_max_scores, argsort_ids
                )
            else:
                (
                    selected_indices,
            ...
            if hasattr(
                ...
                selected_scores = tf.concat(
                    [
                        selected_scores,
                        tf.zeros(max_selection_size - num_valid_nms_boxes, tf.float32),
                    ],
                    -1,
                )
                nms_result = box_list_ops.gather(
        ...
        if max_total_size:
            ...
            num_valid_nms_boxes = tf.where(
                max_total_size > num_valid_nms_boxes,
                num_valid_nms_boxes,
                max_total_size,
            )
        # Select only the valid boxes if pad_to_max_output_size is False.
        if not pad_to_max_output_size:
            sorted_boxes = box_list_ops.gather(
                sorted_boxes, tf.range(num_valid_nms_boxes)
            )
    ...
    with tf.name_scope(scope, "BatchMultiClassNonMaxSuppression"):
        ...
        def _single_image_nms_fn(args):
            ...
            per_image_additional_fields = {
                key: value for key, value in zip(ordered_additional_fields, args[4:-1])
            }
            ...
            if use_static_shapes:
                ...
                per_image_masks = tf.reshape(
                    ...
                    [
                        -1,
                        q,
                        shape_utils.get_dim_as_int(per_image_masks.shape[2]),
        ...
        batch_outputs = map_fn(
            ...
            elems=(
                [boxes, scores, masks, clip_window]
