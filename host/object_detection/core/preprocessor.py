...
def _apply_with_random_selector(x, func, num_cases, preprocess_vars_cache=None, key=""):
    ...
    rand_sel = _get_or_create_preprocess_rand_vars(
        generator_func,
        preprocessor_cache.PreprocessorCache.SELECTOR,
...
def _apply_with_random_selector_tuples(
    x, func, num_cases, preprocess_vars_cache=None, key=""
):
    """Computes func(x, sel), with sel sampled from [0...num_cases-1].
    ...
    rand_sel = _get_or_create_preprocess_rand_vars(
        generator_func,
        preprocessor_cache.PreprocessorCache.SELECTOR_TUPLES,
        preprocess_vars_cache,
        key,
    )
...
def _get_or_create_preprocess_rand_vars(
    generator_func, function_id, preprocess_vars_cache, key=""
):
...
def normalize_image(
    image, original_minval, original_maxval, target_minval, target_maxval
):
    ...
    with tf.name_scope("NormalizeImage", values=[image]):
        ...
        image = tf.multiply(
            image, (target_maxval - target_minval) / (original_maxval - original_minval)
        )
...
def retain_boxes_above_threshold(
    ...
    """
    with tf.name_scope(
        "RetainBoxesAboveThreshold", values=[boxes, labels, label_weights]
    ):
        indices = tf.where(
...
def drop_label_probabilistically(
    boxes,
    labels,
    label_weights,
    label_confidences=None,
    ...
    Returns:
      ...
      retained_keypoints: [num_retained_instance, num_keypoints, 2]
    """
    with tf.name_scope("DropLabelProbabilistically", values=[boxes, labels]):
        indices = tf.where(
            tf.logical_or(
...
def random_horizontal_flip(
    ...
    with tf.name_scope("RandomHorizontalFlip", values=[image, boxes]):
        ...
        if keypoint_visibilities is not None and keypoint_flip_permutation is not None:
        ...
        if keypoint_depths is not None and keypoint_flip_permutation is not None:
            kpt_flip_perm = keypoint_flip_permutation
...
def random_vertical_flip(
    ...
    with tf.name_scope("RandomVerticalFlip", values=[image, boxes]):
        ...
        do_a_flip_random = _get_or_create_preprocess_rand_vars(
            generator_func,
            preprocessor_cache.PreprocessorCache.VERTICAL_FLIP,
            preprocess_vars_cache,
        )
        do_a_flip_random = tf.less(do_a_flip_random, probability)
        ...
        if boxes is not None:
            boxes = tf.cond(
                do_a_flip_random, lambda: _flip_boxes_up_down(boxes), lambda: boxes
            )
            result.append(boxes)
        ...
        if masks is not None:
            masks = tf.cond(
                do_a_flip_random, lambda: _flip_masks_up_down(masks), lambda: masks
            )
            result.append(masks)
...
def random_rotation90(
    ...
    with tf.name_scope("RandomRotation90", values=[image, boxes]):
        ...
        do_a_rot90_random = _get_or_create_preprocess_rand_vars(
            generator_func,
            preprocessor_cache.PreprocessorCache.ROTATION90,
            preprocess_vars_cache,
        )
        ...
        if boxes is not None:
            boxes = tf.cond(
                do_a_rot90_random, lambda: _rot90_boxes(boxes), lambda: boxes
            )
            result.append(boxes)
        ...
        if masks is not None:
            masks = tf.cond(
                do_a_rot90_random, lambda: _rot90_masks(masks), lambda: masks
            )
            result.append(masks)
...
def random_pixel_value_scale(
    image, minval=0.9, maxval=1.1, seed=None, preprocess_vars_cache=None
):
...
def random_image_scale(
    ...
    with tf.name_scope("RandomImageScale", values=[image]):
        ...
        size_coef = _get_or_create_preprocess_rand_vars(
            generator_func,
            preprocessor_cache.PreprocessorCache.IMAGE_SCALE,
            preprocess_vars_cache,
        )
...
def random_rgb_to_gray(image, probability=0.1, seed=None, preprocess_vars_cache=None):
    ...
    with tf.name_scope("RandomRGBtoGray", values=[image]):
        ...
        do_gray_random = _get_or_create_preprocess_rand_vars(
            generator_func,
            preprocessor_cache.PreprocessorCache.RGB_TO_GRAY,
            preprocess_vars_cache,
        )

        image = tf.cond(
            tf.greater(do_gray_random, probability),
            lambda: image,
...
def random_adjust_brightness(
    image, max_delta=0.2, seed=None, preprocess_vars_cache=None
):
    ...
    with tf.name_scope("RandomAdjustBrightness", values=[image]):
        generator_func = functools.partial(
            tf.random_uniform, [], -max_delta, max_delta, seed=seed
        )
...
def random_adjust_contrast(
    image, min_delta=0.8, max_delta=1.25, seed=None, preprocess_vars_cache=None
):
    ...
    with tf.name_scope("RandomAdjustContrast", values=[image]):
        generator_func = functools.partial(
            tf.random_uniform, [], min_delta, max_delta, seed=seed
        )
...
def random_adjust_hue(image, max_delta=0.02, seed=None, preprocess_vars_cache=None):
    ...
    with tf.name_scope("RandomAdjustHue", values=[image]):
        ...
        delta = _get_or_create_preprocess_rand_vars(
            generator_func,
            preprocessor_cache.PreprocessorCache.ADJUST_HUE,
            preprocess_vars_cache,
        )
    ...
    with tf.name_scope("RandomDistortColor", values=[image]):
        if color_ordering == 0:
            ...
            image = random_adjust_hue(
                image, max_delta=0.2, preprocess_vars_cache=preprocess_vars_cache
            )
...
def random_jitter_boxes(boxes, ratio=0.05, jitter_mode="default", seed=None):
    ...
    with tf.name_scope("RandomJitterBoxes"):
        ...
        if jitter_mode in [
            ...
            max_height_distortion = tf.abs(
                tf.minimum((2.0 * ycenter) / height, 2.0 * (1 - ycenter) / height)
            )
            max_width_distortion = tf.abs(
                tf.minimum((2.0 * xcenter) / width, 2.0 * (1 - xcenter) / width)
            )

            if jitter_mode in ["expand_symmetric_xy", "shrink_symmetric_xy"]:
                height_distortion = width_distortion = distortion[:, 0]
                max_height_distortion = max_width_distortion = tf.minimum(
                    max_width_distortion, max_height_distortion
                )
...
def _strict_random_crop_image(
    ...
    with tf.name_scope("RandomCropImage", values=[image, boxes]):
        ...
        if any(t is not None for t in densepose_tensors) and not all(
            t is not None for t in densepose_tensors
        ):
            raise ValueError(
        ...
        new_image = image[
            im_box_begin[0] : im_box_end[0], im_box_begin[1] : im_box_end[1], :
        ]
        ...
        if masks is not None:
            ...
            new_masks = masks_of_boxes_completely_inside_window[
                :, im_box_begin[0] : im_box_end[0], im_box_begin[1] : im_box_end[1]
            ]
        ...
        if keypoint_visibilities is not None:
            kpt_vis_of_boxes_inside_window = tf.gather(
                keypoint_visibilities, inside_window_ids
            )
        ...
        if densepose_num_points is not None:
            ...
            for dp_tensor in densepose_tensors:
                ...
                dp_tensor_completely_inside_window = tf.gather(
                    dp_tensor_inside_window, keep_ids
                )
...
def random_crop_image(
    ...
    if random_coef < sys.float_info.min:
        ...
        do_a_crop_random = _get_or_create_preprocess_rand_vars(
            generator_func,
            preprocessor_cache.PreprocessorCache.CROP_IMAGE,
            preprocess_vars_cache,
        )
        ...
        if densepose_num_points is not None:
            outputs.extend(
                [densepose_num_points, densepose_part_ids, densepose_surface_coords]
            )

        result = tf.cond(
            do_a_crop_random, strict_random_crop_image_fn, lambda: tuple(outputs)
        )
    return result
...
def random_pad_image(
    ...
    if center_pad:
        offset_height = tf.cast(tf.floor((target_height - image_height) / 2), tf.int32)
        offset_width = tf.cast(tf.floor((target_width - image_width) / 2), tf.int32)
...
def random_crop_pad_image(
    ...
    (
        padded_image,
        padded_boxes,
    ) = random_pad_image(  # pylint: disable=unbalanced-tuple-unpacking
        cropped_image,
        cropped_boxes,
        min_image_size=min_image_size,
        max_image_size=max_image_size,
...
def random_crop_to_aspect_ratio(
    image,
    boxes,
    labels,
    label_weights,
    ...
    with tf.name_scope("RandomCropToAspectRatio", values=[image]):
        ...
        orig_aspect_ratio = tf.cast(orig_width, dtype=tf.float32) / tf.cast(
        ...
        target_height = tf.cond(
            orig_aspect_ratio >= new_aspect_ratio, lambda: orig_height, target_height_fn
        )
        ...
        target_width = tf.cond(
            orig_aspect_ratio <= new_aspect_ratio, lambda: orig_width, target_width_fn
        )
        ...
        if center_crop:
            offset_height = tf.cast(
                tf.math.floor((orig_height - target_height) / 2), tf.int32
            )
            offset_width = tf.cast(
                tf.math.floor((orig_width - target_width) / 2), tf.int32
            )
        else:
...
def random_pad_to_aspect_ratio(
    image,
    boxes,
    masks=None,
    keypoints=None,
    ...
    with tf.name_scope("RandomPadToAspectRatio", values=[image]):
        ...
        min_scale = tf.minimum(
            max_scale, tf.maximum(min_height / target_height, min_width / target_width)
        )

        generator_func = functools.partial(
            tf.random_uniform, [], min_scale, max_scale, seed=seed
        )
        ...
        im_box = tf.stack(
            [0.0, 0.0, target_height / image_height, target_width / image_width]
        )
        boxlist = box_list.BoxList(boxes)
...
def random_black_patches(
    ...
    def add_black_patch_to_image(image, idx):
        ...
        box_size = tf.cast(
            tf.multiply(
                tf.minimum(
                    ...
                    tf.cast(image_width, dtype=tf.float32),
                ),
                size_to_image_ratio,
            ),
            dtype=tf.int32,
        ...
        y_min = tf.cast(
            normalized_y_min * tf.cast(image_height, dtype=tf.float32), dtype=tf.int32
        )
        x_min = tf.cast(
            normalized_x_min * tf.cast(image_width, dtype=tf.float32), dtype=tf.int32
        )
        black_box = tf.ones([box_size, box_size, 3], dtype=tf.float32)
        mask = 1.0 - tf.image.pad_to_bounding_box(
            black_box, y_min, x_min, image_height, image_width
        )
    ...
    with tf.name_scope("RandomBlackPatchInImage", values=[image]):
        for idx in range(max_black_patches):
            ...
            image = tf.cond(
                tf.greater(random_prob, probability),
                lambda: image,
...
def random_jpeg_quality(
    ...
    def _adjust_jpeg_quality():
        ...
        quality = _get_or_create_preprocess_rand_vars(
            generator_func,
            preprocessor_cache.PreprocessorCache.JPEG_QUALITY,
            preprocess_vars_cache,
            key="quality",
        )
    ...
    with tf.name_scope("RandomJpegQuality", values=[image]):
        ...
        image = tf.cond(
            do_encoding_random, _adjust_jpeg_quality, lambda: tf.cast(image, tf.float32)
        )

    return image


def random_downscale_to_target_pixels(
    image,
    masks=None,
    ...
    def _resize_image_and_masks_to_target(target_height, target_width):
        # pylint: disable=unbalanced-tuple-unpacking
        new_image, new_masks, _ = resize_image(
            image, masks, target_height, target_width
        )
    ...
    with tf.name_scope("RandomDownscaleToTargetPixels", values=[image]):
        ...
        should_apply_resize = tf.logical_and(
            do_downscale_random, image_larger_than_target
        )
        if masks is not None:
            resize_fn = functools.partial(
                _resize_image_and_masks_to_target, target_height, target_width
            )
            return tf.cond(
                should_apply_resize,
                resize_fn,
...
def random_patch_gaussian(
    ...
    with tf.name_scope("RandomPatchGaussian", values=[image]):
        ...
        image = tf.cond(
            do_patch_gaussian, lambda: _apply_patch_gaussian(image), lambda: image
        )
    return image
...
def resize_to_min_dimension(
    image, masks=None, min_dimension=600, method=tf.image.ResizeMethod.BILINEAR
):
...
def resize_to_max_dimension(
    image, masks=None, max_dimension=600, method=tf.image.ResizeMethod.BILINEAR
):
...
def resize_image(
    ...
    """
    with tf.name_scope(
        "ResizeImage", values=[image, new_height, new_width, method, align_corners]
    ):
        ...
        if masks is not None:
            ...
            masks = tf.cond(
                num_instances > 0, resize_masks_branch, reshape_masks_branch
            )
            result.append(masks)
...
def random_self_concat_image(
    ...
    label_confidences=None,
    multiclass_scores=None,
    concat_vertical_probability=0.1,
    concat_horizontal_probability=0.1,
    seed=None,
    preprocess_vars_cache=None,
):
    ...
    # fixed seeds.
    concat_horizontal = (
        tf.random_uniform([], seed=seed + 1 if seed else None)
    ...
    params = _get_or_create_preprocess_rand_vars(
        gen_func,
        preprocessor_cache.PreprocessorCache.SELF_CONCAT_IMAGE,
        preprocess_vars_cache,
    )
...
def random_square_crop_by_scale(
    ...
    def y_function():
        y = _random_integer(h_border, tf.cast(height, tf.int32) - h_border + 1, seed)
        return y

    def x_function():
        x = _random_integer(w_border, tf.cast(width, tf.int32) - w_border + 1, seed)
        return x
    ...
    if keypoints is not None:
        ...
        keypoints = keypoint_ops.prune_outside_window(keypoints, [0.0, 0.0, 1.0, 1.0])
...
def random_scale_crop_and_pad_to_square(
    ...
    scaled_image = scaled_image[
        offset_y : offset_y + output_size, offset_x : offset_x + output_size, :
    ]
    output_image = tf.image.pad_to_bounding_box(
        scaled_image, 0, 0, output_size, output_size
    )

    # Update the boxes.
    new_window = tf.cast(
        tf.stack([offset_y, offset_x, offset_y + output_size, offset_x + output_size]),
        dtype=tf.float32,
    )
    new_window /= tf.cast(
...
def get_default_func_arg_map(
    ...
    if include_dense_pose:
        groundtruth_dp_num_points = fields.InputDataFields.groundtruth_dp_num_points
        groundtruth_dp_part_ids = fields.InputDataFields.groundtruth_dp_part_ids
    ...
    if include_keypoint_depths:
        groundtruth_keypoint_depths = fields.InputDataFields.groundtruth_keypoint_depths
...
def preprocess(
    tensor_dict, preprocess_options, func_arg_map=None, preprocess_vars_cache=None
):
    ...
    for option in preprocess_options:
        ...
        if func not in func_arg_map:
            raise ValueError(
                "The function %s does not exist in func_arg_map" % (func.__name__)
            )
        arg_names = func_arg_map[func]
        for a in arg_names:
            if a is not None and a not in tensor_dict:
                raise ValueError(
                    "The function %s requires argument %s" % (func.__name__, a)
                )
