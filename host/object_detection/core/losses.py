...
class WeightedGIOULocalizationLoss(Loss):
    ...
    def _compute_loss(self, prediction_tensor, target_tensor, weights):
        ...
        return tf.reshape(
            tf.reshape(weights, [-1]) * per_anchor_iou_loss, [batch_size, num_anchors]
        )
...
class WeightedSoftmaxClassificationAgainstLogitsLoss(Loss):
    ...
    def _compute_loss(self, prediction_tensor, target_tensor, weights):
        ...
        prediction_tensor = tf.divide(
            prediction_tensor, self._logit_scale, name="scale_logits"
        )
...
class BootstrappedSigmoidClassificationLoss(Loss):
    ...
    def _compute_loss(self, prediction_tensor, target_tensor, weights):
        ...
        if self._bootstrap_type == "soft":
            ...
            bootstrap_target_tensor = self._alpha * target_tensor + (
                1.0 - self._alpha
            ) * tf.cast(tf.sigmoid(prediction_tensor) > 0.5, tf.float32)
        per_entry_cross_ent = tf.nn.sigmoid_cross_entropy_with_logits(
...
class HardExampleMiner(object):
    ...
    def __call__(
        self, location_losses, cls_losses, decoded_boxlist_list, match_list=None
    ):
    ...
    def _subsample_selection_to_desired_neg_pos_ratio(
        ...
        max_negatives = tf.maximum(
            ...
            tf.cast(
                max_negatives_per_positive * tf.cast(num_positives, dtype=tf.float32),
                dtype=tf.int32,
            ),
        )
        ...
        return (
            tf.reshape(tf.gather(indices, subsampled_selection_indices), [-1]),
            num_positives,
            num_negatives,
        )
...
class PenaltyReducedLogisticFocalLoss(Loss):
    ...
    def _compute_loss(self, prediction_tensor, target_tensor, weights):
        ...
        positive_loss = tf.math.pow((1 - prediction_tensor), self._alpha) * tf.math.log(
            prediction_tensor
        )
        negative_loss = (
            tf.math.pow((1 - target_tensor), self._beta)
