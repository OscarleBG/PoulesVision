...
class BalancedPositiveNegativeSamplerTest(test_case.TestCase):
    ...
    def test_subsample_selection_no_batch_size(self):
        ...
        def graph_fn(indicator, labels):
            sampler = (
                balanced_positive_negative_sampler.BalancedPositiveNegativeSampler(0.01)
            )
        ...
        self.assertEqual(
            sum(np.logical_and(np.logical_not(numpy_labels), is_sampled_out)), 495
        )
        self.assertAllEqual(
            is_sampled_out, np.logical_and(is_sampled_out, numpy_indicator)
        )
