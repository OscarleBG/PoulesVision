...
if tf_version.is_tf1():
    from object_detection.matchers import (
        bipartite_matcher,
...
class TargetAssigner(object):
    ...
    def __init__(
        self, similarity_calc, matcher, box_coder_instance, negative_class_weight=1.0
    ):
    ...
    def assign(
        ...
        with tf.control_dependencies(
            ...
            match = self._matcher.match(
                match_quality_matrix, valid_rows=tf.greater(groundtruth_weights, 0)
            )
    ...
    def _create_regression_targets(self, anchors, groundtruth_boxes, match):
        ...
        if groundtruth_boxes.has_field(fields.BoxListFields.keypoints):
            ...
            matched_gt_boxlist.add_field(
                fields.BoxListFields.keypoints, matched_keypoints
            )
    ...
    def _create_classification_targets(
        self, groundtruth_labels, unmatched_class_label, match
    ):
...
def create_target_assigner(
    reference, stage=None, negative_class_weight=1.0, use_matmul_gather=False
):
...
def batch_get_targets(
    batch_match,
    groundtruth_tensor_list,
...
def batch_assign_confidences(
    ...
    for anchors, gt_boxes, gt_class_confidences, gt_weights in zip(
        anchors_batch, gt_box_batch, gt_class_confidences_batch, gt_weights_batch
    ):

        if (
            gt_class_confidences is not None
            and len(gt_class_confidences.get_shape().as_list()) > 2
        ):
            raise ValueError(
                "The shape of the class target is not supported. ",
        ...
        cls_weights_without_background = (1 - implicit_class_weight) * tf.cast(
            explicit_example_mask, dtype=tf.float32
        ) + implicit_class_weight

        if include_background_class:
            cls_weights_background = (
                1 - implicit_class_weight
            ) * regression_weights_expanded + implicit_class_weight
...
def max_distance_for_overlap(height, width, min_iou):
    ...
    distance_detection_offset = _smallest_positive_root(
        a=1, b=-(height + width), c=width * height * ((1 - min_iou) / (1 + min_iou))
    )
    ...
    return tf.reduce_min(
        [distance_detection_offset, distance_gt_in_detection, distance_detection_in_gt],
        axis=0,
    )
...
def get_batch_predictions_from_indices(batch_predictions, indices):
    ...
    flattened_inds = tf.linalg.matmul(indices, rev_cum_interior_indices[:, tf.newaxis])[
        :, 0
    ]
...
def _preprocess_keypoints_and_weights(
    out_height,
    out_width,
    keypoints,
    class_onehot,
    ...
    if keypoint_weights is None:
        ...
        keypoint_weights = tf.gather(keypoint_weights, indices=keypoint_indices, axis=1)
...
class CenterNetCenterHeatmapTargetAssigner(object):
    ...
    def assign_center_targets_from_boxes(
        self,
        height,
        width,
        gt_boxes_list,
        ...
        for boxes, class_targets, weights in zip(
            gt_boxes_list, gt_classes_list, gt_weights_list
        ):
            ...
            sigma = _compute_std_dev_from_box_size(
                boxes_height, boxes_width, self._min_overlap
            )
            ...
            if self._box_heatmap_type == "adaptive_gaussian":
                ...
                heatmap = ta_utils.coordinates_to_iou(
                    y_grid, x_grid, boxes, class_targets, weights
                )
            else:
        ...
        return (
            tf.pow(stacked_heatmaps, self._heatmap_exponent)
            if self._heatmap_exponent != 1.0
            else stacked_heatmaps
        )

    def assign_center_targets_from_keypoints(
        self,
        height,
        width,
        gt_classes_list,
        ...
        """
        assert (
            self._keypoint_weights_for_center is not None
            and self._keypoint_class_id is not None
            and self._keypoint_indices is not None
        )
        out_height = tf.cast(tf.maximum(height // self._stride, 1), tf.float32)
        out_width = tf.cast(tf.maximum(width // self._stride, 1), tf.float32)
        # Compute the yx-grid to be used to generate the heatmap. Each returned
        ...
        for keypoints, classes, kp_weights, weights in zip(
            ...
            sigma = _compute_std_dev_from_box_size(
                2 * avg_radius, 2 * avg_radius, self._min_overlap
            )
...
class CenterNetBoxTargetAssigner(object):
    ...
    def assign_size_and_offset_targets(
        self,
        height,
        width,
        gt_boxes_list,
        ...
        for i, (boxes, weights) in enumerate(zip(gt_boxes_list, gt_weights_list)):
            ...
            batch_box_height_width.append(tf.stack([boxes_height, boxes_width], axis=1))
...
class CenterNetKeypointTargetAssigner(object):
    ...
    def __init__(
        ...
        if keypoint_std_dev is None:
            self._keypoint_std_dev = [_DEFAULT_KEYPOINT_OFFSET_STD_DEV] * len(
                keypoint_indices
            )
        else:
    ...
    def assign_keypoint_heatmap_targets(
        self,
        height,
        width,
        gt_keypoints_list,
        ...
        for keypoints, classes, kp_weights, weights, boxes in zip(
            ...
            heatmaps.append(heatmap)
        return (
            tf.stack(heatmaps, axis=0),
            tf.stack(num_instances_list, axis=0),
    ...
    def _get_keypoint_types(self, num_instances, num_keypoints, num_neighbors):
        ...
        tiled_keypoint_types = tf.tile(
            keypoint_types, multiples=[num_instances, 1, num_neighbors]
        )
        return tiled_keypoint_types

    def assign_keypoints_offset_targets(
        self,
        height,
        width,
        gt_keypoints_list,
        ...
        for i, (keypoints, classes, kp_weights, weights) in enumerate(
            ...
            (
                ...
                x_source_neighbors,
                valid_sources,
            ) = ta_utils.get_surrounding_grids(
            ...
            # [num_instance * num_keypoints, num_neighbors]
            valid_keypoints = tf.cast(valid_sources, dtype=tf.float32) * tf.stack(
            ...
            batch_index = tf.fill([num_instances * num_keypoints * num_neighbors, 1], i)
                ...
                batch_indices.append(
                    tf.concat(
                        [
                            batch_index,
                            indices,
                            tf.reshape(tiled_keypoint_types, [-1, 1]),
                        ],
                        axis=1,
                    )
                )
    ...
    def assign_keypoints_depth_targets(
        self,
        height,
        width,
        gt_keypoints_list,
        ...
        for i, (
            ...
            #   [num_instances, num_keypoints]
            keypoint_depths = tf.gather(keypoint_depths, self._keypoint_indices, axis=1)
            ...
            tiled_depth_weights = (
                tf.reshape(
                    valid_keypoints, [num_instances, num_keypoints, num_neighbors]
                )
                * keypoint_depth_weights
            )
            ...
            final_keypoint_depths = tf.where(
                invalid_depths,
                tf.zeros_like(tiled_keypoint_depths),
                tiled_keypoint_depths,
            )
            final_keypoint_depth_weights = tf.where(
                invalid_depths, tf.zeros_like(tiled_depth_weights), tiled_depth_weights
            )
            ...
            if self._per_keypoint_depth:
                ...
                batch_indices.append(tf.concat([batch_index, indices], axis=1))
            batch_weights.append(tf.keras.backend.flatten(final_keypoint_depth_weights))
    ...
    def assign_joint_regression_targets(
        self,
        height,
        width,
        gt_keypoints_list,
        ...
        for i, (keypoints, classes, boxes, kp_weights, weights) in enumerate(
            ...
            (
                ...
                tf.keras.backend.flatten(x_center_tiled),
                self._peak_radius,
            )
            ...
            batch_indices.append(
                tf.concat(
                    [batch_index, indices, tf.reshape(tiled_keypoint_types, [-1, 1])],
                    axis=1,
                )
            )
            batch_offsets.append(offsets)
...
def _resize_masks(masks, height, width, method):
    ...
    masks = tf2.image.resize(
        masks[:, :, :, tf.newaxis], size=(height, width), method=method
    )
    return masks[:, :, :, 0]
...
class CenterNetMaskTargetAssigner(object):
    ...
    def assign_segmentation_targets(
        ...
        _, input_height, input_width = shape_utils.combined_static_and_dynamic_shape(
            gt_masks_list[0]
        )
            ...
            if gt_boxes is not None and gt_mask_weights is not None:
                ...
                segmentation_weight_for_image = (
                    ta_utils.blackout_pixel_weights_by_box_regions(
                        ...
                        boxes_scale=self._boxes_scale,
                    )
                )
                segmentation_weights_list.append(segmentation_weight_for_image)
            else:
                segmentation_weights_list.append(
                    tf.ones((output_height, output_width), dtype=tf.float32)
                )

            gt_masks = _resize_masks(
                gt_masks, output_height, output_width, mask_resize_method
            )
...
class CenterNetDensePoseTargetAssigner(object):
    ...
    def assign_part_and_coordinate_targets(
        self,
        height,
        width,
        ...
        for i, (num_points, part_ids, surface_coords, weights) in enumerate(
            ...
            num_points_tiled = tf.tile(
                num_points[:, tf.newaxis], multiples=[1, max_sampled_points]
            )
            range_tiled = tf.tile(
                tf.range(max_sampled_points)[tf.newaxis, :], multiples=[num_boxes, 1]
            )
            ...
            batch_indices.append(
                tf.stack(
                    [batch_index, indices_y, indices_x, part_ids_flattened], axis=1
                )
            )
...
class CenterNetTrackTargetAssigner(object):
    ...
    def assign_track_targets(
        self, height, width, gt_track_ids_list, gt_boxes_list, gt_weights_list=None
    ):
...
def filter_mask_overlap_min_area(masks):
    ...
    return tf.cond(num_instances > 0, _filter_min_area, lambda: masks)
...
class CenterNetCornerOffsetTargetAssigner(object):
    ...
    def assign_corner_offset_targets(self, gt_boxes_list, gt_masks_list):
        ...
        y_grid, x_grid = tf.meshgrid(
            tf.range(output_height), tf.range(output_width), indexing="ij"
        )
        ...
        for gt_masks, gt_boxes in zip(gt_masks_list, gt_boxes_list):
            ...
            foreground_target = tf.cast(
                tf.reduce_sum(gt_masks, axis=0) > 0.5, tf.float32
            )
        ...
        return (tf.stack(corner_targets, axis=0), tf.stack(foreground_targets, axis=0))
...
class CenterNetTemporalOffsetTargetAssigner(object):
    ...
    def assign_temporal_offset_targets(
        self,
        height,
        width,
        gt_boxes_list,
        ...
        for i, (boxes, offsets, match_flags, weights) in enumerate(
            zip(gt_boxes_list, gt_offsets_list, gt_match_list, gt_weights_list)
        ):
            boxes = box_list.BoxList(boxes)
            boxes = box_list_ops.to_absolute_coordinates(
                boxes,
...
class DETRTargetAssigner(object):
    ...
    def batch_assign(
        ...
        """
        pred_box_batch = [
            box_list.BoxList(pred_box) for pred_box in tf.unstack(pred_box_batch)
        ]
        gt_box_batch = [box_list.BoxList(gt_box) for gt_box in tf.unstack(gt_box_batch)]
        ...
        for (
            ...
            (cls_targets, cls_weights, reg_targets, reg_weights) = self.assign(
                pred_boxes,
                gt_boxes,
                pred_class_batch,
                gt_class_targets,
    ...
    def assign(
        ...
        match_quality_matrix = self._similarity_calc.compare(gt_boxes, pred_boxes)
        match = self._matcher.match(
            match_quality_matrix, valid_rows=tf.greater(gt_weights, 0)
        )

        matched_gt_boxes = match.gather_based_on_match(
            gt_boxes.get(), unmatched_value=tf.zeros(4), ignored_value=tf.zeros(4)
        )
        matched_gt_boxlist = box_list.BoxList(matched_gt_boxes)
        ...
        reg_weights = match.gather_based_on_match(
            gt_weights, ignored_value=0.0, unmatched_value=0.0
        )
        cls_weights = match.gather_based_on_match(
            gt_weights, ignored_value=0.0, unmatched_value=1
        )

        # convert cls_weights from per-box_pred to per-class.
        class_label_shape = tf.shape(cls_targets)[1:]
        weights_multiple = tf.concat([tf.constant([1]), class_label_shape], axis=0)
        cls_weights = tf.expand_dims(cls_weights, -1)
        cls_weights = tf.tile(cls_weights, weights_multiple)
