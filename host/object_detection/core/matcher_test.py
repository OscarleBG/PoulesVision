    ...
    def test_scalar_gather_based_on_match(self):
        def graph_fn():
            ...
            gathered_tensor = match.gather_based_on_match(
                input_tensor, unmatched_value=100.0, ignored_value=200.0
            )
    ...
    def test_multidimensional_gather_based_on_match(self):
        def graph_fn():
            ...
            input_tensor = tf.constant(
                [[0, 0.5, 0, 0.5], [0, 0, 0.5, 0.5]], dtype=tf.float32
            )
            match = matcher.Match(match_results)
            gathered_tensor = match.gather_based_on_match(
                input_tensor, unmatched_value=tf.zeros(4), ignored_value=tf.zeros(4)
            )
            return gathered_tensor
