...
class ModelLibTest(tf.test.TestCase):
    ...
    def test_experiment(self):
        ...
        experiment = model_lib.populate_experiment(
            run_config, hparams, pipeline_config_path, train_steps=10, eval_steps=20
        )
...
class UnbatchTensorsTest(tf.test.TestCase):
    def test_unbatch_without_unpadding(self):
        ...
        tensor_dict = {
            fields.InputDataFields.image: image_placeholder,
            fields.InputDataFields.groundtruth_boxes: groundtruth_boxes_placeholder,
            fields.InputDataFields.groundtruth_classes: groundtruth_classes_placeholder,
            fields.InputDataFields.groundtruth_weights: groundtruth_weights_placeholder,
        }
        ...
        with self.test_session() as sess:
            unbatched_tensor_dict_out = sess.run(
                ...
                feed_dict={
                    image_placeholder: np.random.rand(2, 4, 4, 3).astype(np.float32),
                    groundtruth_boxes_placeholder: np.random.rand(2, 5, 4).astype(
                        np.float32
                    ),
                    groundtruth_classes_placeholder: np.random.rand(2, 5, 6).astype(
                        np.float32
                    ),
                    groundtruth_weights_placeholder: np.random.rand(2, 5).astype(
                        np.float32
                    ),
                },
            )
        ...
        with self.test_session() as sess:
            unbatched_tensor_dict_out = sess.run(
                ...
                feed_dict={
                    ...
                    num_groundtruth_placeholder: np.array([3, 3], np.int32),
                },
            )
        for image_out in unbatched_tensor_dict_out[fields.InputDataFields.image]:
            self.assertAllEqual(image_out.shape, [4, 4, 3])
