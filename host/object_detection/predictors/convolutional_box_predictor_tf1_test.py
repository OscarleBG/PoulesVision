...
class ConvolutionalBoxPredictorTest(test_case.TestCase):
    ...
    def test_get_predictions_with_feature_maps_of_dynamic_shape(self):
        ...
        conv_box_predictor = box_predictor_builder.build_convolutional_box_predictor(
            is_training=False,
            num_classes=0,
            conv_hyperparams_fn=self._build_arg_scope_with_conv_hyperparams(),
            min_depth=0,
        ...
        with self.test_session() as sess:
            ...
            (box_encodings_shape, objectness_predictions_shape) = sess.run(
                ...
                feed_dict={
                    image_features: np.random.rand(4, resolution, resolution, 64)
                },
            )
            actual_variable_set = set([var.op.name for var in tf.trainable_variables()])
            self.assertAllEqual(box_encodings_shape, [4, expected_num_anchors, 1, 4])
            self.assertAllEqual(
                objectness_predictions_shape, [4, expected_num_anchors, 1]
            )
...
class WeightSharedConvolutionalBoxPredictorTest(test_case.TestCase):
    ...
    def test_other_heads_predictions(self):
        ...
        def graph_fn(image_features):
            ...
            other_heads = {
                other_head_name: mask_head.WeightSharedConvolutionalMaskHead(
        ...
        feature_ht = 8
        feature_wt = 8
        image_features = np.random.rand(batch_size, feature_ht, feature_wt, 64).astype(
            np.float32
        )
        ...
        self.assertAllEqual(
            box_encodings.shape, [batch_size, num_anchors, box_code_size]
        )
        self.assertAllEqual(
