...
class RfcnKerasBoxPredictor(box_predictor.KerasBoxPredictor):
    ...
    def __init__(
        ...
        class_feature_map_depth = (
            self._num_spatial_bins[0] * self._num_spatial_bins[1] * self._total_classes
        )
    ...
    def _predict(self, image_features, proposal_boxes, **kwargs):
        ...
        if len(image_features) != 1:
            raise ValueError(
                "length of `image_features` must be 1. Found {}".format(
                    len(image_features)
                )
            )
        ...
        box_encodings = tf.reshape(
            box_encodings,
            [batch_size * num_boxes, 1, self.num_classes, self._box_code_size],
        )
        ...
        class_predictions_with_background = ops.batch_position_sensitive_crop_regions(
            class_net,
            boxes=proposal_boxes,
            crop_size=self._crop_size,
            num_spatial_bins=self._num_spatial_bins,
