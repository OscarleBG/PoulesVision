    ...
    def build(self, input_shapes):
        ...
        if len(input_shapes) != len(self._prediction_heads[BOX_ENCODINGS]):
            raise ValueError(
                ...
                % (len(self._prediction_heads[BOX_ENCODINGS]), len(input_shapes))
            )
...
class WeightSharedConvolutionalBoxPredictor(box_predictor.KerasBoxPredictor):
        ...
        if inserted_layer_counter >= 0:
            use_bias = (
                ...
                if (
                    ...
                    and not self._conv_hyperparams.force_use_bias()
                )
                else True
            )
    ...
    def _compute_base_tower(self, tower_name_scope, feature_index):
        ...
        use_bias = (
            False
            if (self._apply_batch_norm and not self._conv_hyperparams.force_use_bias())
            else True
        )
    ...
    def build(self, input_shapes):
        """Creates the variables of the layer."""
        feature_channels = [
            shape_utils.get_dim_as_int(input_shape[3]) for input_shape in input_shapes
        ]
    ...
    def _predict(self, image_features, **kwargs):
        ...
        for (index, image_feature) in enumerate(image_features):
            ...
            box_tower_feature = _apply_layers(
                self._base_tower_layers_for_heads[BOX_ENCODINGS][index], image_feature
            )
