...
class ConvolutionalBoxPredictor(box_predictor.BoxPredictor):
    ...
    def _predict(self, image_features, num_predictions_per_location_list):
        ...
        for (image_feature, num_predictions_per_location, box_predictor_scope) in zip(
            image_features, num_predictions_per_location_list, box_predictor_scopes
        ):
            net = image_feature
            with box_predictor_scope:
                with slim.arg_scope(self._conv_hyperparams_fn()):
                    with slim.arg_scope([slim.dropout], is_training=self._is_training):
                        ...
                        depth = max(
                            min(features_depth, self._max_depth), self._min_depth
                        )
                        tf.logging.info(
...
class WeightSharedConvolutionalBoxPredictor(box_predictor.BoxPredictor):
    ...
    def _insert_additional_projection_layer(
        self, image_feature, inserted_layer_counter, target_channel
    ):
    ...
    def _compute_base_tower(self, tower_name_scope, image_feature, feature_index):
        ...
        for i in range(self._num_layers_before_predictor):
            ...
            if self._apply_batch_norm:
                net = slim.batch_norm(
                    net,
                    scope="{}/conv2d_{}/BatchNorm/feature_{}".format(
                        tower_name_scope, i, feature_index
                    ),
                )
            net = tf.nn.relu6(net)
        return net
    ...
    def _predict(self, image_features, num_predictions_per_location_list):
        ...
        if len(set(num_predictions_per_location_list)) > 1:
            raise ValueError(
                ...
                "feature maps, found: {}".format(num_predictions_per_location_list)
            )
        feature_channels = [
        ...
        for head_name in self._other_heads.keys():
            predictions[head_name] = []
        for feature_index, (image_feature, num_predictions_per_location) in enumerate(
            zip(image_features, num_predictions_per_location_list)
        ):
            with tf.variable_scope(
                "WeightSharedConvolutionalBoxPredictor", reuse=tf.AUTO_REUSE
            ):
                with slim.arg_scope(self._conv_hyperparams_fn()):
