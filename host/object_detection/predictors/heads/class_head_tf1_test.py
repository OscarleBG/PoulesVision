...
class MaskRCNNClassHeadTest(test_case.TestCase):
    ...
    def test_scope_name(self):
        expected_var_names = set(
            ["""ClassPredictor/weights""", """ClassPredictor/biases"""]
        )
        ...
        with g.as_default():
            ...
            class_prediction_head.predict(
                features=image_feature, num_predictions_per_location=1
            )
...
class WeightSharedConvolutionalClassPredictorTest(test_case.TestCase):
    ...
    def test_softmax_score_converter(self):
        ...
        def graph_fn():
            class_prediction_head = class_head.WeightSharedConvolutionalClassHead(
                num_class_slots=num_class_slots, score_converter_fn=tf.nn.softmax
            )
            image_feature = tf.random_uniform(
                [batch_size, height, width, 1024],
                minval=-10.0,
                maxval=10.0,
                dtype=tf.float32,
            )
        ...
        exp_class_predictions_sum = np.ones((batch_size, num_anchors), dtype=np.float32)
        self.assertAllEqual(
            (batch_size, num_anchors, num_class_slots), class_predictions_out.shape
        )
