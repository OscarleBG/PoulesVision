...
class WeightSharedConvolutionalMaskPredictorTest(test_case.TestCase):
    ...
    def test_prediction_size(self):
        mask_prediction_head = mask_head.WeightSharedConvolutionalMaskHead(
            num_classes=20, mask_height=7, mask_width=7
        )
        image_feature = tf.random_uniform(
            [64, 17, 19, 1024], minval=-10.0, maxval=10.0, dtype=tf.float32
        )
    ...
    def test_class_agnostic_prediction_size(self):
        mask_prediction_head = mask_head.WeightSharedConvolutionalMaskHead(
            num_classes=20, mask_height=7, mask_width=7, masks_are_class_agnostic=True
        )
        image_feature = tf.random_uniform(
            [64, 17, 19, 1024], minval=-10.0, maxval=10.0, dtype=tf.float32
        )
